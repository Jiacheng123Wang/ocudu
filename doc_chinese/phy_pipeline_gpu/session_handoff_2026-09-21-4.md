...
**工作树 HEAD = `8a8453b631`**（= 第 2 步接线 + 回滚 + 第 3 步的 1/2/4 小步 + 池子泄漏修复 + **方案 A 的机制与接线** + **重新武装**；快照提交跟在它后面）。
**开机第一件事就是自查戳记**：
```bash
...
| **8** | **★ 下一件事：上腿对**（`gpu` 对照 / `gpu` + 旋钮），**第一次能看数据面**：PUCCH `metric/sinr`、CRC 分层比、`real-time failures` | 腿对 | ⬜ **下一步** |
...
**今天三条腿揭出三个缺陷（接收池泄漏已修；网格的宿主消费者 PUCCH 是结构性的；机制落地时单测又抓到自死锁与
"栅栏/完成回调无先后"两个真缺陷），用户裁定走 A，**A 的机制与接线都已完成、离线验过、交出已重新武装**（§5.9.13/§5.9.14）。
下一步就是上腿对——**这次第一次能看到数据面**（PUCCH 的 metric/sinr 与 CRC）。**
