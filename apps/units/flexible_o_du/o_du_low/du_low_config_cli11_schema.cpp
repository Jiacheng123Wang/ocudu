...
static void configure_cli11_expert_phy_args(CLI::App& app, du_low_unit_expert_upper_phy_config& expert_phy_params)
{
  ...
  auto ldpc_decoder_type_check = [](const std::string& value) -> std::string {
    if ((value == "auto") || (value == "generic") || (value == "neon") || (value == "avx2") ||
        (value == "avx512") || (value == "metal") || (value == "metal_nms")) {
      return {};
    }
    return "Invalid PUSCH LDPC decoder type. Accepted values [auto,generic,neon,avx2,avx512,metal,metal_nms]";
  };
  ...
  add_option(app,
             ...
             "PUSCH LDPC decoder type: auto, generic, neon, avx2, avx512, metal and metal_nms.")
      ->capture_default_str()
