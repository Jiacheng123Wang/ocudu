...
**① 已归档（用户裁定：留作以后优化，本轮不动；设计文档 §5.9.28）**
* 饱和爬坡那档异常的**度量口径**（71.6 ms 样本的 `stale=` 分流、"起点槽→完成槽"槽距）；
* 判读约定：A/B 取**稳态饱和**窗口，避开阶跃后 1–2 s 的自适应瞬态。

**② 本轮在做的两件（爬坡拒收 91 次 + 爬坡最坏端到端，同源）**
1. **第二档余量（已做，一行）**：`nof_ul_rg` 3 帧 → **6 帧** ⇒ 判据见 §5.9.30，等 `s42` 腿；
2. **结构性修法（方案已定，未实现）**：设计文档 **§5.9.29** ——
   **方案 A（先做，便宜）**：`uplink_processor_fsm` 加 `pending_slot_tasks` + `on_release_slot_ownership()`，
   `start_new_slot()` 只要"队列 0 + 槽作用域任务 0"，并在 `pusch_proc->process()` **返回后**立刻释放
   （此后 `pdu` 不再被解引用、网格已读完）⇒ **LDPC 解码不再阻止下一个槽**；
   **方案 B（不够时再做）**：按槽上下文的小池子（仓库 + 网格 + FSM）。
