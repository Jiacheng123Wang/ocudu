...
**下一步（§3.3 已开工，不需要飞腿也不需要用户裁决）**：**单 kernel 离线微基准** —— 照 `doc_chinese/phy_latency/wip/dft_kernel_cost.mm` 的形状，
把 **CE（`reformat`/`pilots_lse`/`pilots_cfo`/`corr_a`/`corr_rhp`/`apply_lse`）/ 均衡 / 解映射** 各自的**算力（µs）与窗口**一次量出来，
并**单独量一次"空 kernel 派发"的固定开销**（把"派发成本"和"算力成本"分开——这正是本会话暴露出来的、标尺一直含混的那一项）。
**理由**：`merged_hop` 533.7 µs 里**非派发的 ~450 µs 比剩余派发预算（≈30–42 µs）大一个数量级** ⇒ 该砍的是 kernel，不是再加派发臂。
...
⇒ **派发这条矿脉还剩 ≈30–42 µs**（不是 §6.63④ 说的"见底"），但 `merged_hop` 里**非派发的 ~450 µs 大一个数量级**
⇒ **主战场仍是 §3.3**（单 kernel 离线微基准 → 砍最贵的 kernel）；派发臂按"顺手做掉"的节奏继续即可。
