...
> ⚠ **本文件已被 `session_handoff_2026-09-20-4.md` 取代（那份是入口）**；本文件保留为同一会话的**过程记录**。
>
> 上一份：`session_handoff_2026-09-20-2.md`（P0 收尾 + P1 起步）。
...
**★★ 仪器的坏消息（§5.8.14）**：给权重链每段加的**保值重复探针**（`OCUDU_CE_*_REPEAT`，n 次编同一 dispatch，
dump 逐位不变 ✓）第一次就暴露了**仪器本身**：同一配置连跑 6 次，`ch_wt` = 571 / 554 / **203** / 606 / 625 / **142** µs
（`--repeat 20` 仍 579 / 476 / 608）⇒ **离线的 `busy split` 不能用来判优化**（散布远大于要判的效果）。
**⇒ §5.8.12 的离线 bisection 整张撤回**（CORR_DEV=0 的 −445 µs 等全是噪声），**"相关矩阵前缀是最大单项"没有证据**，
**P5 当初的靶子因此也是未被测量的**。仍然成立的是**空口腿**量出来的东西：链条三段分解、`ch_wt` 383–394 µs（四条腿 ±2%）、
...
要重启这条线，**先装逐 dispatch 的 GPU 计时**（或做"路由钉住"的空壳 bisection），别再直接改 kernel。
**两条纪律**：① 改 kernel **先跑 `wip/neutral_vs_baseline.sh`**——本会话里它抓到了错值，而 `ctest -R metal` 9/9 全过；
② **判读门前先确认产物是刚构建的**（坑 35 又踩了一次：回退后全量构建没重链 `ul_chain_replay`，
拿旧二进制跑门得到"还是全红"的假象）。
