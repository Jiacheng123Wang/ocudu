...
> ⚠ **2026-09-25 后记：本 memo 的两项任务都已做（先 P0-5、再 P2-E）。P0-5 完成并提交；P2-E 实现完成，
> 但其机制被平台证伪（本机只在命令缓冲完成时发布"中途"事件信号）⇒ 需要用户裁决。最新状态请看
> **`gpu_phy_latency_optimization_design_and_implementation.md`（§6.3/§6.4/§8）与 `high_level_status_and_plan.md`**；
> 2026-09-25 那一份会话 memo 已按用户要求并入开发文档（不再单独存在）。下文保持原文（当时的计划与判据）。**
...
**下一步的顺序（用户 2026-09-24 指定）：先做 P0-5**——把相位探针与车道探针**键在同一事件上**（见 **§6.0**），**做完再实现 P2-E**（计划已写好；2026-09-25 起并入开发文档 §6.4.1，见 §6.1）。
...
| P1 | 单变量臂（P1-8 并发度已跑；其余见开发文档 §7）| P1-8 已跑出结果（§5.3）|
| **P2-E** | **输入缓冲寿命解耦** | **P0-5 之后做**（计划：开发文档 §6.4.1，见本 memo §6.1）|
...
### 5.1 未解释 → **已定位**的 n1 退化（开发文档 §8 与 §7.5）
...
### 6.1 第二优先：**实现 P2-E**（完整计划见开发文档 §6.4.1；本省的结果见该文档 §6.4.3）
...
| 文件 | 内容 |
|---|---|
| `gpu_phy_latency_optimization_design_and_implementation.md` | **开发文档**（2026-09-25 起 `00–05` 已全部并入）：现象/预算/机制/V1–V5/Q1–Q11 + 实施记录 |
| `high_level_status_and_plan.md` | 高层：阶段进度、下一步、待裁决（V4 裁决见开发文档 §7.4）|
| `README.md` | 三类文档的分工 + 旧名映射 |
| 开发文档 §6.1/§6.2 | P0-6 / P0-1 的读数、读法、命名陷阱、臂身份、缺陷与修法 |
