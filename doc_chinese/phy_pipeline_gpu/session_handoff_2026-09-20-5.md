# 交接（入口） — S14：**离线 GPU 时间仪器定案 + K1 画像**——计数器永久关闭、等待点实测、`busy split` 有协议、`ch_wt` 分段量出、**K1 是最大单项且已排除三条微调路线**
...
| 候选 | 依据 | 第一步 |
|---|---|---|
| **① 回答"K1 那 197 µs 到底是什么"** | §5.8.16 已经排除三条：**不是 barrier**（2→1 更慢；K1b barrier 少 9 倍却贵 2.1 倍）、**不是算术量**（n=18/36/54 拟合 ≈ n^1.45，n³ 会差 27 倍）、**不是几何**（默认已最优，最好的替代只值 1.1%）。剩下的是"每 system 一份的固定开销"或"逐元素线程组访存" | **固定 n、扫 `nof_systems`**（或反过来），用 §7 的协议；结构事实：`mmse_inv` grid = `MTLSizeMake(nof_systems,1,1)`，调用点传 **`2 * nof_layers`**（单层 UE 只有 **2 个线程组**）⇒ GPU 基本空着 |
| **② 结构性改法（不是微调）** | K1 = 每跳 **197 µs = `ch_wt` 的 31%（空口几何）**，而它**与分配宽度无关**（4 PRB 与 25 PRB 都是 197 µs）⇒ 一整个核干一份固定的活，其余核全空 | 方向：**一个 system 用多个线程组**（列分块 + 多次 dispatch，逐元素运算次序不变 ⇒ 仍可逐位保值）或换算法；**先做 ① 定出收益再动手** |
| **③ 修 `OCUDU_CE_INVERT_FIRST`** | 它现在产出 **NaN**（§4.2）；修好后 K1 能单独占一条命令缓冲，比 `INV_REPEAT=3` 更干净（后者只对奇数有效，且 `_ce.txt` 差 7 字节）| 把 `engine_run()` 里那次 `engine->invert(gpu_a, …)` 移到**相关矩阵前缀之后**；判据：四个 dump 与默认臂逐字节相同，且 `busy split` 出现独立的求逆项 |
| **④ 查那 163 µs 未归属（`ch_wt` 的 26%，按空口几何）** | 第二大桶，且是**纯开销**（y scatter、barrier、编码/发射）| 用同一协议逐项关掉/重复（`OCUDU_CE_DEV_Y=0`、`EDGE_FUSE=0`、去掉 scatter）|
...
| **★ K1 画像（§5.8.16）** | ✅ **三条微调路线已排除；两次改动量输并回退** | ① K1 = **197 µs/跳**，**与分配宽度无关**（4 / 10 / 25 PRB 都是 197.3 / 198.3 / 197.5）；② 几何默认 **(64,16) 已最优**（最好的替代 128x8 只值 **1.1%**，几何逐位保值：7 种 × 4 dump = 0 字节）；③ **不是 barrier**：2→1 barrier/pivot 反而更慢（197→216），K1b barrier 少 9 倍却贵 **2.1×**（422 vs 197）；④ 结构：grid = `MTLSizeMake(nof_systems,1,1)`、调用点传 `2 * nof_layers` ⇒ **单层 UE 只有 2 个线程组** |
