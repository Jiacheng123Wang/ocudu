...
| **CE** | **K1 矩阵反演 `A⁻¹`** | **CPU（主机）**——**临时**，见 §48.86 | 它**应该**在 GPU：回退 CPU 的原始理由是**性能**（§48.68：设备 555µs vs 主机 ~10µs），后来被我**用错误的精度论据**维持（§48.85）。精度不是障碍（设备 `W` 误差 7.67e-4 ⇒ ≈0.003 dB）。**真正障碍**：设备 K1 在 `run_async()` 的 CB 里**完全不执行**（A 前后逐字节相同），`OCUDU_CE_GPU_INVERT=1` 可复现。**这是功能缺陷，修好即回 GPU** |
