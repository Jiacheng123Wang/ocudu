...
> ⚠ **2026-09-25 实测结果：本计划的两处锚点已实现（开关 `OCUDU_DFT_RELEASE_TOKENS_EARLY`，默认关），
> 但本平台（macOS 26.6.2 / Apple Silicon）把"编码在 encoder 边界之后的 `MTLSharedEvent` 信号"推迟到
> 整条命令缓冲完成才发布 ⇒ `notifyListener` 无法在整跳结束前释放 tokens，本计划的机制在这台机器上不生效。**
> 三种测法与对照、以及"为什么这不只是调参"、需要用户裁决的选项，见 **`05_p2e_platform_finding.md`**。
> 本文保留原计划原文（判据先写死、不许为过关而改），下文即当时的计划。
