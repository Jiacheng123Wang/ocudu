...
| 判据 | 阈值 | 现状 | 出处 |
|---|---|---|---|
| **V1** `[ul_gpu_pipeline]` 中位 | ≤2150 µs | ✅ **1408.2 µs**（`p33`，新最好；基线 2675.1 ⇒ **−47.4%**）| §6.46/§6.48–§6.50/§6.52 |
| **V2** 池 | `starved_events=0` 且 `held_max<pool` | ⚠ **`p33` 读红**：`held_max=16=pool`、`free_min=0`、`starved_events=2`（但 `pop_blocking` max **249 µs**、`dropped=0`、无 park）；`p31`/`p32`/`p32b` 仍是 10–12 / <16 / **0** | §6.37–§6.39、§6.52 ④ |
| **V4** 提交数 | `cbs/lane ≤2.00`、`dropped=0` | ✅ `p33` 仍 **2.00 (max=2)**、crossings `0.00+0.00`/跳（第四次确认）| §6.30/§6.46/§6.49/§6.52 ⑤ |
| **V5** 不回归 | 契约 8/8、0 crossings | ✅ **`p33` 契约 MET 8/8、`gaps=0`**；`p32`/`p32b` 是 7/8（各 1 gap）| §6.49 ⑤、§6.52 ①⑤ |
...
**gap 问题已收口成因、补了仪器、并跑了第一条腿**（§6.51/§6.52）：**每一次 gap 都是电台接收环溢出**（11 条腿 `overflow` 次数 == `gaps` 次数）；
`p33`（环 64→256 + RX 仪器）**预登记三项等价 0==0==0 成立、契约 8/8、V1 1408.2（最好）**，
且 **Q17 有答案：堵点在传输侧、不在宿主**（`recv` 657 次 >1 ms、`loop` 仅 1 次 >1 ms）。
⚠ **但环与仪器同提交 ⇒ "环是否有效"还没有对照**（我的实验设计错误），且 **`p33` 的 V2 读红**
（`held_max=16=pool`、`starved_events=2`；但 `pop_blocking` 249 µs、`dropped=0`，且 `p29` 在改动前就有 15/1/1）。
**下一步 = §3.1c：同二进制、只把 `num_recv_frames` 改回 64 飞 `p34-n78-ring64`**（§6.52 ③⑥）。
...
### 3.1c ⏳（待飞一条**配置对照**腿）：接收环 64 vs 256
