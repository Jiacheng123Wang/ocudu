...
| **9** | **★ 下一件事：把注册表的键从【地址】改成【地址 + 接收槽】**（+ 未认领的块被替换时提交它而不是丢掉 + 补三个计数）| 机制单测 + 新腿对 | ⬜ **下一步，§5.9.15 ④ 已写到位与函数** |
...
### 4.3 腿法（**⚠ 交出已再次关掉**，见 §5.9.15；接线修好后才重新武装）
...
**五条腿一共揭出五个缺陷，前四个都修掉了**（接收池泄漏；PUCCH 是宿主消费者的结构性约束；锁内释放最后一个引用的
自死锁；栅栏与完成回调无先后）。**`s34` 是最好的一版但仍是坏的**：所有结构性问题都关掉了
（实时失败 53135→1、PRACH→0、契约 8/8、干净退出、`dft commits`→1 = 合并成功），
**剩下的是一条【键】的缺陷**——注册表用网格地址做键，而池子会复用地址，于是晚一个槽来问的消费者拿到的是**后一个槽的块**，
它自己那份网格从来没写（§5.9.15）。**修法已写到位与函数；交出已再次关掉。**
