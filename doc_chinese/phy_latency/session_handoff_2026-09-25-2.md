...
## 0. 一句话现状（★ 根因已结案、修复 A 已落地并**空口验证通过**）

**腿 `p15-conc2`（修复后，同配方）：`handshake waits=24, timeouts=0, max=165us`（窗口真被踩到 24 次）、
`waiter-committed-first=0 / 106,016 waits`、判据 **23/23 全绿**、契约 **8/8**、**0 gaps**、满速上行 **105 s 无停顿**、
四个"5 秒"读数全部落回 ms 级（`input hold` 5004.7 → **17.5 ms**、`pop_blocking` 4997.6 → **22 µs**）⇒ **不是"没触发"**（§6.22）。
...
### 3.1 第一件：`p15-conc2` ✅ 已飞（§6.22）；**建议再飞 1–2 条收口**（可选 `iperf3 -R -t 300` 加大暴露）

（下面这条命令留作复跑模板；`p15` 的报告已完整落盘。）
...
| 读数 | p15 实测（通过）|
|---|---|
| **D1/D3/D4**（判据，阈值不动）| **全绿**：`input hold` max 17.5 ms、`pop_blocking` max 22 µs、**0 gaps** |
| **D13（新）** | **`waits=24`、`timeouts=0`、`max=165 µs`** ⇒ 窗口被踩到 24 次、全部关掉 |
