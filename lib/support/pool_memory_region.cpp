...
#else
  // Platform without explicit huge page support (e.g. macOS): report the mapping as unavailable so the caller falls
  // back to the heap-backed resource, rather than returning memory with weaker guarantees than the caller asked for.
  (void)aligned_size;
#endif
