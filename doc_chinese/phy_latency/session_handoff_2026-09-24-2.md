...
2. **Q9：腿 `q9-conc2` 已飞，第一环成了直接读数**（开发文档 §6.8）：**`input hold` max = 61.4 s**（210 个 token > 1 s），
   上行 iperf3 中段 **~40 s 归零**、UL 吞吐 166 跳/s（并发 1 是 555）；同一次会话里 ping 与下行正常 ⇒ **只有 UL 数据面死**。
   并发 2 的**收益**也再次确认：`[ul_gpu_pipeline]` 中位 **2331 µs**（并发 1 是 5248）、`stale=0`。
   **下一步 = P0-7**（逐块生命周期：deposit → 谁认领（跳/sweep）→ 完成），**已落地**（开发文档 §6.9）：
   下一条腿读 `[metal_stats] block lifecycle (P0-7)` 的 `oldest unclaimed age max` / `deposit->completion max`
   （与 `input hold` 对照）即可判"是不是没人认领的块按住了池"；设备侧等待目标（P0-7b）仍未做。
   ⚠ 该腿的 `[ul_rx_pool]` 两行被**我自己的 atexit 缺陷**吃掉（已修 + 已加离线回归，§6.8 ⑤）；池侧直接读数可随 P0-7 再飞一条补。
