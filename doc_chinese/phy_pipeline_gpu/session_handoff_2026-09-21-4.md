# 交接（入口，**已被 `session_handoff_2026-09-21-5.md` 取代**）

> **⚠ `-5.md` 撤回了本文件 §4.1 的第 14/15 行**（"网格是一个对象、每槽复用" —— 证据读错了）。
> 本文件其余内容（s36 腿的读数、教训、流程）仍然有效，保留不删。

## S20：**空口腿否证了 D1 第 2 步的接线：变换不能比输入活得久，而"宿主等待"正是输入生命期的锚**（已回滚）
...
**工作树 HEAD = `d9083299d0`**（= 第 2 步接线 + 回滚 + 第 3 步 + 池子泄漏修复 + 方案 A 的机制与接线 + **s34 腿的诊断与再次关掉交出**；快照提交跟在它后面）。
**开机第一件事就是自查戳记**：
```bash
...
| **7** | **把网格产出等待接到宿主消费者上**（PUCCH ×2 + SRS，各在自己的执行器里）**+ 重新武装交出** | 钩子已安装断言；门全过 | ✅ **§5.9.14** |
| **8** | 上腿对（`s34`）：结构性缺陷**全部关掉**（实时失败 53135→1、PRACH→0、契约 8/8、干净退出、`dft commits`→1）| 腿对 | ✅ **§5.9.15** |
| **9** | 把键从【地址】改成【地址 + 接收槽】；未认领的块被替换时**补提交**；记录活到"已产出"；补四个计数 | 机制单测第 7 条臂（同存储两槽）| ✅ **§5.9.16** |
| **10** | 上腿对（`s35`）：键是对的，但**没有任何消费者的槽永不顶掉** ⇒ 池子又枯竭 | 腿对 | ✅ **§5.9.17** |
| **11** | **给"没人认领"加期限（扫掠：落后 2 个槽就由注册表自己提交）** + 仪表改名/拆计数 | 机制单测第 8 条臂（孤儿槽）| ✅ **§5.9.17** |
| **12** | 上腿对（`s36`）：池子/凭据/栅栏全部健康（实时失败 29219→7），数据仍错 | 腿对 | ✅ **§5.9.18** |
| **13** | **★ 决定性离线实验**：把武装后的交出放进"逐字节对照 CPU 参考"的测试 ⇒ **网格产出 0/17808 不匹配** | 离线 | ✅ **§5.9.18 ②** |
| **14** | **★ 第五个结构障碍（真正的拦路石）**：资源网格是**一个对象、每槽复用** ⇒ 任何晚于自己槽的消费者读到的都是**下一个槽的内容**（§5.9.19 有完整代码证据；**它同时解释对照臂那 35% 的 PUCCH 失败**）| 代码证据 | ✅ **已定位** |
| **15** | **★ 下一件事：给槽自己的网格**（小池子，按引用计数归还），让 PUCCH/SRS/跳持有自己槽的那一份 | **可离线判**（`uplink_processor` 单测：迟任务不得读到下一个槽的网格）| ⬜ **下一步** |
...
**八条腿 / 五个结构障碍。D1 的机制侧已全部做完并离线验过**（八条臂 + 网格 `0/17808` 逐字节），
**而真正的拦路石是第五个：资源网格是一个对象、每槽复用**（§5.9.19）——它同时是对照臂那 35% PUCCH 失败的根因。
原先（第七个不是 D1 的：**上层 PHY 的 PUCCH/SRS 任务读"按槽复用"的状态**，
一"等"就跨槽——它在 D1 之前就潜伏着：对照臂的 PUCCH 也只有 65% 好）。
**而 D1 的这条链已被离线证明正确**：武装后的交出，网格 **0/17808 不匹配**（§5.9.18 ②）。（第六个是"键修对了之后才浮出来"的：没有消费者的槽永不顶掉 ⇒ 凭据占住接收缓冲；修法是**扫掠**：
落后最新 deposit 超过 2 个槽的未认领块由注册表自己提交）。（接收池泄漏；PUCCH 是宿主消费者的结构性约束；锁内释放最后一个引用的自死锁；
栅栏与完成回调无先后；以及 `s34` 那条【键】的缺陷——注册表用网格地址做键，而池子会复用地址）。
**`s34` 证明合并本身是成功的**（实时失败 53135→1、PRACH→0、契约 8/8、干净退出、`dft commits`→1），
**键已改成【存储 + 接收槽】并离线验过**（§5.9.16），交出已重新武装。
**下一步就是再上腿对**——判 PUCCH/PUSCH 的 `sinr` 中位数（对照 +24/+26 dB）、CRC，
以及六个计数：`handed / taken / fallback / late / not_found / timeouts`（外加 `unproduced` 与 `keepalives` 必须相等）。
