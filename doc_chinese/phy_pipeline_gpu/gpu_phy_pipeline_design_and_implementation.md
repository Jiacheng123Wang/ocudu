...
> **★ 更正（2026-09-21，P0-④）**：那 45/135 **是陈旧构建的假象**（我只重建了 `ul_chain_replay`，没重建 metallib 与相关库——
...
**P0-④ 结果（2026-09-21）：链式探针把病灶钉到**两个**独立缺陷，其中**一个已修**（且既有路线逐字节不变）。**
...
**P0-⑤ 结果（2026-09-21）：缺陷② 的判决性证据——"权重读到的不是槽位最终的内容"，且**不是时序问题**。**
...
**P0-⑥ 结果（2026-09-21）：探针抓到硬事实——"权重读 A 时，A 整槽还没被写"；并发现一条车道序线索。**
