...
| **Q18** | 接收环 **64 → 256 帧**到底做了什么？（`p33` 上 0 丢样 + 一次 19.45 ms 积压被吸收，但**环与仪器同提交**，不是对照）| ⚠ **需要对照腿**（§6.52 ③）：同二进制、只把 `num_recv_frames` 改回 64 飞一条（`p34-n78-ring64`），读 `rx_overflows`/`gaps`/`slip`/`held_max`/`starved_events`。**且** `p33` 的 V2 读红（`held_max=16=pool`、`starved_events=2`，但 `pop_blocking` 249 µs、`dropped=0`）在改动前也出现过（`p29`：15/1/1）⇒ 归因未定 |
