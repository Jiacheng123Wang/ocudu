# 交接（入口） — S14：**离线 GPU 时间仪器定案 + K1 画像**——计数器永久关闭、等待点实测、`busy split` 有协议、`ch_wt` 分段量出、**K1 的微调路线全部量掉（剩下的是改数值契约还是换靶子的裁定）**
...
| 候选 | 依据 | 第一步 |
|---|---|---|
| **① ★ 决定：K1 要不要为它改【数值契约】** | §5.8.16 ⑦ 已把"微调"这条路走死：几何最优（差 1.1%）、barrier 不是成本（两次否证）、算术量不是主导（n^1.27）、**并行度免费**（`nof_systems` 1→8 成本一动不动）⇒ 剩下的是 **Gauss-Jordan 逐 pivot 的串行临界路径**，在一次 54×54 上 **≈3.7 µs/pivot、共 197 µs/跳（`ch_wt` 的 31%，且与分配宽度无关）**。在"逐元素次序不变"下它**砍不动** | 这是个**用户裁定**：换算法（如 Newton–Schulz 两三次矩阵乘迭代：高度并行、barrier 极少，但同一 A 的**舍入不同**）⇒ 判据从"逐字节不变"变成"重定基线 + 空口腿按 CRC/契约判"，即为一个**值变更**跑一次 §5.8.2。**先估收益再决定**（离线可估：改一版算法，量 K1 降到多少，再看空口腿值不值得跑） |
| **② ★ 转向另外三块（它们可能还有【逐位保值】的余量）** | K1 之外：重排 **96.7 µs（17.4%）**、K2 权重 **59.8 µs（10.8%）**、未归属 **163 µs（26%）** | 未归属那块先查（`OCUDU_CE_DEV_Y=0`、`EDGE_FUSE=0`、去掉 scatter，看它是不是"每 dispatch 固定成本 × dispatch 数"）；重排与 K2 用同一协议量斜率 |
