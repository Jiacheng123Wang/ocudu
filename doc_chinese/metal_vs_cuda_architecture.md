...
**当前不建议推进"与 CUDA 共同抽象"这件事**，理由：

1. **能被 CUDA 共享的那一层，恰好是我们不需要的那一层**（§5.3）；
2. **上游的 CUDA 尚未接入任何应用**（只被单元测试调用），他们**还没有跨模块 device 数据流转的痛点**；
3. **我们的进度在生产化程度上领先**：我们已接入应用、有 CLI 旋钮、已上真机；
   上游 CUDA 目前是"基础设施 + 单模块 + 单测"。

**值得与上游（DeepSig）对话的时机**：**当上游把第二个 CUDA 模块落地、开始需要跨模块 device 数据流转时。**
那时他们会遇到我们今天遇到的同样问题（中间态回传、同步点数量、栅栏语义），
而**"device-addressable view"（而不是"device memory allocation"）**才是双方都能接受的收敛点。

**在此之前，我们应当做的是把 LLDP（low-level design point）层面的抽象对齐到我们的架构**，
而不是对齐到上游：
