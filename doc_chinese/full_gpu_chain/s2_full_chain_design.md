...
| OFDM 解调 → 频域网格 | **CPU** | ← 待搬 |
| DMRS 导频提取 / LS 估计 / EPRE（→ `pilots_lse_view`） | **CPU** | ← 待搬 |
...
| 前段 | DMRS 导频提取 / EPRE / LS / CFO 组装（→ `pilots_lse_view`） | **CPU，但实测 0.46–1.24 µs** | 曾按原样搬上设备的方案（K0-a，`200c0b9010`）**被实测否掉**：省 0.46 µs 却多一次 dispatch（10–25 µs）。**注意：那是否决理由是"性能"，不是硬约束**（见 §48.84(a0) 规则 2）|
