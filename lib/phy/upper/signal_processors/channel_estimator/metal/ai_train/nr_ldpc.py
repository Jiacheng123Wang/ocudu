#!/usr/bin/env python3
"""5G NR LDPC encoder (3GPP TS 38.212, section 5.3.2).

Self-contained, numpy-only implementation for offline training-data generation.

Contents
--------
* The base graphs BG1 (46x68) and BG2 (42x52) exactly as in 3GPP TS 38.212
  Tables 5.3.2-2 / 5.3.2-3.  Because the shift coefficient of an entry can
  depend on the lifting-size *set index* ``i_LS`` (0..7), each non-null entry
  is stored as an 8-tuple of shifts, one per set index.  ``-1`` / a missing
  entry denotes a null (zero) block.  Data source: NVIDIA Sionna reference
  CSVs ``5G_bg1.csv`` / ``5G_bg2.csv`` (themselves "taken from 38.212"),
  cross-checked digit-by-digit against the 3GPP text and the known structural
  invariants.
* The lifting-size table (38.212 Table 5.3.2-1): the standard eight sets
  ``Zc = a * 2^j`` for ``a`` in {2,3,5,7,9,11,13,15} (set indices 0..7),
  each already truncated to ``Zc <= 384`` as in the spec.  The same table is
  used for both base graphs.
* Base-graph selection and code-block segmentation (38.212 5.2.2 / 6.2.2),
  via :func:`select_bg_and_zc`.
* Quasi-cyclic encoding :func:`ldpc_encode` and parity-check helpers
  :func:`build_H`, :func:`dense_H`, :func:`verify`.

Conventions (as in 38.212 5.3.2)
--------------------------------
A base-matrix entry ``s >= 0`` is lifted to the ``Zc x Zc`` identity matrix
cyclically shifted *right* by ``s`` positions (the 1 in row ``i`` sits at
column ``(i + s) mod Zc``).  The full codeword ``c`` (length
``n_base_cols * Zc``) satisfies ``H @ c == 0 (mod 2)``, where ``H`` is the
lifted parity-check matrix (``-1`` entries replaced by ``Zc x Zc`` zero
blocks).  Bit ordering is the standard 3GPP order: information columns first
(columns ``0 .. Kb-1``), then the four core parity columns, then the
extension parity columns.

Structure notes (verified against the spec)
-------------------------------------------
* Columns 0 and 1 are the *punctured* systematic columns (the first ``2*Zc``
  codeword bits are not transmitted).  In the 3GPP base graphs these are the
  *highest-degree* variable nodes (BG1: 30 / 28; BG2: 22 / 23), **not**
  weight-1 columns.
* The core (rows 0..3) parity part is: first parity column (BG1 col 22,
  BG2 col 10) of weight 3, then three weight-2 dual-diagonal columns
  (BG1 cols 23..25, BG2 cols 11..13).  The dual-diagonal entries have shift
  0; the degree-3 column uses the table's actual shifts.
* Each extension row (rows 4..mb-1) adds exactly one single-weight
  (degree-1, shift-0) parity column.
* Every row of BG1 and BG2 has at least one non-null entry.
"""
...
# ---------------------------------------------------------------------------
# Lifting size table (38.212 Table 5.3.2-1).  Eight sets, index i_LS = 0..7,
# already truncated to Zc <= 384.  Used identically by BG1 and BG2.
# ---------------------------------------------------------------------------
LIFTING_SIZE_TABLE = (
...
# Base-graph geometry: (num_rows mb, num_cols nb, num_info_cols Kb).
_BG_PARAMS = {
    1: (46, 68, 22),
    2: (42, 52, 10),
}
...
def _info_columns(bg, K):
    """Number of information columns Kb used to pick Zc (38.212 5.2.2)."""
    if bg == 1:
        return 22
    if K > 640:
        return 10
...
def _select_zc(bg, K):
    """Minimum lifting size Zc with Kb * Zc >= K (38.212 5.2.2)."""
    kb = _info_columns(bg, K)
    best = None
    for zs in LIFTING_SIZE_TABLE:
        for z in zs:
            if kb * z >= K and (best is None or z < best):
                best = z
    if best is None:
        raise ValueError(f"no lifting size satisfies Kb*Zc >= K for K={K}, bg={bg}")
    return best


def _i_ls_of(Zc):
    """Set index i_LS whose lifting set contains Zc."""
    for i, zs in enumerate(LIFTING_SIZE_TABLE):
        if Zc in zs:
            return i
    raise ValueError(f"Zc={Zc} is not a valid lifting size (see Table 5.3.2-1)")


def _base_matrix(bg, i_ls):
    """Return the (mb x nb) integer base matrix for set index ``i_ls``
    (-1 = null)."""
    mb, nb, _ = _BG_PARAMS[bg]
    bm = np.full((mb, nb), -1, dtype=np.int64)
    entries = _BG1_ENTRIES if bg == 1 else _BG2_ENTRIES
    for e in entries:
        bm[e[0], e[1]] = e[2 + i_ls]
    return bm
...
# ---------------------------------------------------------------------------
# Base graphs (38.212 Tables 5.3.2-2 / 5.3.2-3).
# Each tuple = (row, col, v0, v1, ..., v7); v_k is the shift for set index k.
# ---------------------------------------------------------------------------
_BG1_ENTRIES = [
...
def select_bg_and_zc(tbs_bits, E, allow_bg1_shortening=True):
    """Select base graph, lifting size and number of code blocks.

    Parameters
    ----------
    tbs_bits : int
        Transport-block (payload) size in bits, ``A``.
    E : int
        Number of bits available for the coded transmission (used to form the
        code rate ``R = A / E``).
    allow_bg1_shortening : bool
        When True (default), BG1 may be used even if the transport block is
        small enough that BG1 would carry filler (shortened) bits.  When
        False, a small block (``A <= 3824``) is forced onto BG2.

    Returns
    -------
    (bg, Zc, n_cb) : tuple
        ``bg`` in {1, 2}, lifting size ``Zc``, and the number ``n_cb`` of
        code blocks.

    Notes
    -----
    Base-graph selection follows 38.212 (6.2.2 / the LDPC base-graph rule):

    * ``A <= 292``                      -> BG2
    * ``A <= 3824`` and ``R <= 2/3``    -> BG2
    * ``R <= 1/4``                      -> BG2
    * otherwise                         -> BG1

    Code-block segmentation uses the maximum code-block sizes K_cb = 8448
    (BG1) / 3840 (BG2) and a 24-bit code-block CRC when a block is split.
    """
    if tbs_bits < 1:
        raise ValueError("tbs_bits must be a positive integer")
    tbs_bits = int(tbs_bits)
    E = int(E)
    R = (tbs_bits / E) if E > 0 else 1.0

    # --- base graph selection -------------------------------------------------
    if tbs_bits <= 292 or (tbs_bits <= 3824 and R <= 2.0 / 3.0) or R <= 0.25:
        bg = 2
    else:
        bg = 1

    if bg == 1 and not allow_bg1_shortening and tbs_bits <= 3824:
        bg = 2

    # --- code block segmentation (24-bit CRC per block when split) -----------
    k_cb = 8448 if bg == 1 else 3840
    if tbs_bits <= k_cb:
        n_cb = 1
    else:
        n_cb = (tbs_bits + (k_cb - 24) - 1) // (k_cb - 24)

    K = (tbs_bits + n_cb - 1) // n_cb      # info bits per code block
    if n_cb > 1:
        K += 24                            # code-block CRC
...
def _apply_shift(v, s):
    """Right-shift permutation P_s applied to a length-Zc vector.

    ``P_s`` is the identity cyclically shifted right by ``s``, i.e. the 1 in
    row ``i`` is at column ``(i + s) mod Zc``, so ``(P_s @ v)[i] = v[(i+s) mod Zc]``
    which equals ``numpy.roll(v, -s)``.
    """
    return np.roll(v, -int(s))


def ldpc_encode(bg, Zc, u_bits):
    """Encode information bits into a full 5G NR LDPC codeword.
    ...
    bg : int
        Base graph (1 or 2).
    Zc : int
        Lifting size (an entry of :data:`LIFTING_SIZE_TABLE`).
    u_bits : array_like of 0/1
        ``K`` information bits, ``0 <= K <= Kb * Zc`` where ``Kb`` is the base
        graph's information-column count (22 for BG1, 10 for BG2).

    Returns
    -------
    codeword : numpy.ndarray (uint8), shape ``(n_base_cols * Zc,)``
        The full codeword including the punctured ``2*Zc`` prefix and the
        (non-transmitted) filler bits, in 3GPP order: information columns,
        core parity columns, extension parity columns.

    Encoding is the standard dual-diagonal back-substitution: the first core
    parity column (weight 3) and the three dual-diagonal core parity columns
    are solved from the four core rows, then each extension row yields its
    single-weight parity column.  Filler bits are 0 and are appended at the
    end of the information section (columns are shortened from the last
    information column downward).
    """
    if bg not in _BG_PARAMS:
        raise ValueError(f"bg must be 1 or 2, got {bg!r}")
    mb, nb, kb = _BG_PARAMS[bg]
    i_ls = _i_ls_of(Zc)

    u = np.asarray(u_bits, dtype=np.uint8).ravel()
    K = u.size
    if K > kb * Zc:
        raise ValueError(f"{K} information bits exceed capacity Kb*Zc={kb * Zc}")

    # Pad with filler (zero) bits to the base-graph information capacity.
    s = np.zeros(kb * Zc, dtype=np.uint8)
    s[:K] = u
    S = s.reshape(kb, Zc)          # S[j, :] = segment of information column j

    bm = _base_matrix(bg, i_ls)

    # --- core syndromes (info contribution to core rows 0..3) ---------------
    lam = np.zeros((4, Zc), dtype=np.uint8)
    for r in range(4):
        acc = np.zeros(Zc, dtype=np.uint8)
        for j in range(kb):
            sh = bm[r, j]
            if sh >= 0:
                acc ^= _apply_shift(S[j], sh)
        lam[r] = acc

    # --- core parity (back-substitution) ------------------------------------
    # bm_b = bm[0:4, kb:kb+4]; A = bm_b[0,0]; B = bm_b[1,0] (BG1) / bm_b[2,0]
    # (BG2).  In both graphs the degree-3 column has shift A on rows 0 and 3
    # and shift B on row 1 (BG1) / row 2 (BG2).
    A = int(bm[0, kb])
    B = int(bm[1, kb]) if bg == 1 else int(bm[2, kb])

    p = np.zeros((4, Zc), dtype=np.uint8)
    p0 = np.roll(lam[0] ^ lam[1] ^ lam[2] ^ lam[3], int(B))  # P_B^-1 = roll(+B)
    p[0] = p0
    if bg == 1:
        p[1] = lam[0] ^ np.roll(p0, -A)
        p[2] = lam[1] ^ np.roll(p0, -B) ^ p[1]
        p[3] = lam[2] ^ p[2]
    else:
        p[1] = lam[0] ^ np.roll(p0, -A)
        p[2] = lam[1] ^ p[1]
        p[3] = lam[2] ^ np.roll(p0, -B) ^ p[2]

    # --- extension parity (one single-weight column per extension row) ------
    nb_ext = mb - 4
    p_ext = np.zeros((nb_ext, Zc), dtype=np.uint8)
    for r in range(4, mb):
        acc = np.zeros(Zc, dtype=np.uint8)
        for j in range(kb):
            sh = bm[r, j]
        ...
        for m in range(4):
            sh = bm[r, kb + m]
            if sh >= 0:
                acc ^= _apply_shift(p[m], sh)
        p_ext[r - 4] = acc

    return np.concatenate([s, p.ravel(), p_ext.ravel()])


def build_H(bg, Zc):
    """Return the lifted parity-check matrix in compact QC form.

    Returns
    -------
    list of (row, col, shift) tuples
        ``row`` / ``col`` are *base-graph* indices (row in 0..mb-1,
        col in 0..nb-1) and ``shift`` is the cyclic right-shift
        (``s mod Zc``) of the corresponding ``Zc x Zc`` identity block.
        Null entries are omitted.  The full matrix is ``H`` such that every
        valid codeword ``c`` satisfies ``H @ c == 0 (mod 2)``.
    """
    if bg not in _BG_PARAMS:
        raise ValueError(f"bg must be 1 or 2, got {bg!r}")
    mb, nb, _ = _BG_PARAMS[bg]
    bm = _base_matrix(bg, _i_ls_of(Zc))
    tuples = []
    for r in range(mb):
        for c in range(nb):
            s = int(bm[r, c])
            if s >= 0:
                tuples.append((r, c, s % Zc))
    return tuples


def dense_H(bg, Zc):
    """Dense lifted parity-check matrix ``H`` (numpy, uint8).

    Intended for verification with small ``Zc`` (memory is
    ``(mb*Zc) x (nb*Zc)``).
    """
    mb, nb, _ = _BG_PARAMS[bg]
    bm = _base_matrix(bg, _i_ls_of(Zc))
    H = np.zeros((mb * Zc, nb * Zc), dtype=np.uint8)
    for r in range(mb):
        for c in range(nb):
            s = int(bm[r, c])
            if s >= 0:
                cols = (np.arange(Zc) + (s % Zc)) % Zc
                H[r * Zc + np.arange(Zc), c * Zc + cols] = 1
    return H


def verify(codeword, bg, Zc):
    """Return True iff ``codeword`` satisfies ``H @ c == 0 (mod 2)``.

    Computes the syndrome using the QC structure (no dense matrix), so it is
    efficient for any lifting size.  A length mismatch yields False.
    """
    if bg not in _BG_PARAMS:
        raise ValueError(f"bg must be 1 or 2, got {bg!r}")
    mb, nb, _ = _BG_PARAMS[bg]
    ...
    if c.size != nb * Zc:
        return False
    C = c.reshape(nb, Zc)          # C[j, :] = segment of column j
    bm = _base_matrix(bg, _i_ls_of(Zc))
    for r in range(mb):
        acc = np.zeros(Zc, dtype=np.uint8)
        for col in range(nb):
...
if __name__ == "__main__":
    ...
    print("== (a) encode/verify consistency, dense H cross-check ==")
    for bg, zc_list in ((1, [2, 3, 4, 8, 16]), (2, [2, 3, 6, 8, 12])):
        mb, nb, kb = _BG_PARAMS[bg]
        for Zc in zc_list:
            for trial in range(4):
                u = rng.integers(0, 2, size=kb * Zc).astype(np.uint8)
                c = ldpc_encode(bg, Zc, u)
                ok_len = c.size == nb * Zc
                ok_verify = verify(c, bg, Zc)
                # dense cross-check for small Zc only
                if Zc <= 8:
                    H = dense_H(bg, Zc)
                    ok_dense = bool(np.all((H @ c) % 2 == 0))
                else:
                    ok_dense = True
                check(f"bg={bg} Zc={Zc} trial={trial}",
                      ok_len and ok_verify and ok_dense,
                      f"len={ok_len} verify={ok_verify} dense={ok_dense}")

    print("== (b) base-matrix structural invariants ==")
    for bg in (1, 2):
        mb, nb, kb = _BG_PARAMS[bg]
        bm = _base_matrix(bg, 0)
        check(f"bg={bg} shape {mb}x{nb}", bm.shape == (mb, nb))
        # punctured columns 0,1 are the two highest-degree columns (spec)
        colw = (bm >= 0).sum(axis=0)
        check(f"bg={bg} punctured cols 0,1 highest degree",
              colw[0] >= 2 and colw[1] >= 2 and colw[0] > colw[2] and colw[1] > colw[2],
              f"col0={colw[0]} col1={colw[1]} min={colw.min()}")
        # every row has at least one non-null entry
        check(f"bg={bg} every row non-empty", int((bm >= 0).sum(axis=1).min()) >= 1)
        # core parity columns weight-3 + dual-diagonal weight-2
        core = bm[0:4, kb:kb + 4]
        w = (core >= 0).sum(axis=0)
        check(f"bg={bg} core parity weights [3,2,2,2]", list(w) == [3, 2, 2, 2], f"w={list(w)}")
        # dual-diagonal (cols 1..3) shifts are all 0
        check(f"bg={bg} dual-diagonal shifts are 0",
              int((core[:, 1:][core[:, 1:] >= 0]).sum()) == 0)
        # extension parity columns single-weight shift 0
        ext = bm[:, kb + 4:]
        extw = (ext >= 0).sum(axis=0)
        check(f"bg={bg} extension parity single-weight",
              int(extw.min()) == 1 and int(extw.max()) == 1)
        check(f"bg={bg} extension parity shift 0",
              int((ext[ext >= 0]).sum()) == 0)

    print("== (c) all-zero input -> all-zero codeword ==")
    for bg, kb in ((1, 22), (2, 10)):
        Zc = 2
        c = ldpc_encode(bg, Zc, np.zeros(kb, dtype=np.uint8))
        check(f"bg={bg} Zc=2 K={kb} all-zero codeword", bool((c == 0).all()))

    print("== (d) parity bits roughly half set (rate spot check) ==")
    for bg, kb in ((1, 22), (2, 10)):
        Zc = 8
        fracs = []
        for _ in range(20):
            ...
            c = ldpc_encode(bg, Zc, u)
            parity = c[kb * Zc:]
            fracs.append(float(parity.mean()))
        m = float(np.mean(fracs))
        check(f"bg={bg} Zc=8 parity fraction ~0.5", 0.35 < m < 0.65, f"mean={m:.3f}")
