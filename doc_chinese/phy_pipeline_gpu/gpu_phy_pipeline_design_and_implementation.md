...
| **S13-P2c** | 窄跳（分配 < 1 个块）的相关矩阵：宿主建的 A **没有对角加载** ⇒ 2 PRB 恒 0% CRC | ✅ **离线判定完成**（§5.6.7）：20 条真实 1–2 PRB 捕获上 `KO×16 → OK×14`（多数 1 次迭代，SINR +10…+22 dB）；机制由 `HOST_SCALARS=1` 逐值相同证明；27 捕获逐字节不变；穿越 `1.00 写 → 0.00`。**空中腿待跑** |
...
```bash
sudo -E bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu narrow-fix
bash doc_chinese/phy_pipeline_gpu/wip/leg_report.sh \
     doc_chinese/phy_pipeline_gpu/wip/logs/gnb_gpu_narrow-fix_*.log.stderr
```

盯：契约 **8/8**（跨越 `0.00 读 + 0.00 写`/跳、**两张分项表都空**）、`refusals=<none>`、
**2 PRB 分层的 CRC 从 0.0% 起来**、1 PRB 与其他宽度不回归、RTF 0、`[ul_gpu_lane] dropped/carried=0`。
