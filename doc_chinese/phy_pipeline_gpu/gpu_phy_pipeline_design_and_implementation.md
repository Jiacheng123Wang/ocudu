...
2. **车道的 `busy split` 把交出的块整条记成 `eq_demap`**：合并路线下那条命令缓冲**就是**交出的块，
   里面**也装着 DFT 的变换**；而 `stage::dft` 的注释本来就写着 *"not registered yet, see the file comment"*
   —— **它不是新洞，但翻默认让它变得要紧**：现在 DFT 的时间既不进前端系列，也不进车道的 stage 归属，
   `busy split` 的 `100% of busy` 是把两件事算成了一件。
   ⇒ **要压设备的 26% 时，先得能归属它**；否则 `eq_demap` 这个数是虚高的。

**④ #13 的下一步（按代价从低到高，都还没动手）**

1. **补齐归属**（上面 ③ 的两条，只改计数）：让交出的块把 DFT 的那份记到 `stage::dft`。
   **⇒ 这是继续做 #13 的前提，不是可选项**（否则优化的对象是错的）。
