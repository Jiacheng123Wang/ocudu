...
> **技术细节全在常驻设计文档 `gpu_phy_pipeline_design_and_implementation.md` 的 §5.8.15 – §5.8.30 与 §5.9**
> ——**本会话新增的是 §5.8.28**（K1 的墓志铭）、**§5.8.29**（`[ul_time_frequency]` 为什么是 ~1000 µs）、
> **§5.8.30**（收包粒度腿对：不是杠杆）、**§5.9**（**D1 的范围分析，开工前必读**）。
...
> **★ 先读设计文档 §5.9（本会话新写的 D1 范围分析）**——它把 D1 的目标、提交预算和一个
> **比预期更硬的排序约束**都查清了，**不读它会低估工作量**。

### 5.1 为什么是它（不是收益，是排序标准）

* **排序标准 = "功能正确前提下 CPU 还剩几个参与点"**。目标两半（数据面 ✅ / 控制面 ✅）都已达成，
  **D1 是最后一个目标性差距**；
* **提交预算（`s14p3-merged` 实测）**：**DFT 前端 ≈ 1.83 次提交/跳** + 估计器 ~0 + 车道 1.00
  ⇒ **一跳合计 ≈ 2.83 次 CPU 提交**，**D1 要拿掉那 1.83**；
* 设计文档原话：DFT 是 "**the stage that moves onto the lane's queue when the fused pipeline lands**"。

### 5.2 ★★ 开工前必须知道的三个硬事实（否则会做出错误的设计）

1. **顺序是循环的**：车道的整跳命令缓冲**由提取打开**，提取要读导频 ⇒ 只能在**网格就绪之后**开始，
   而网格是 **DFT 的输出**。⇒ **"把 DFT 编码进车道那条缓冲"在时间上是循环的**。
   今天两者靠 **`front_end_wait()` 事件栅栏**跨队列排序，而**该栅栏已经在用**
   （`ocudu_metal_burst.mm:142`、`ocudu_metal_mmse_engine.mm:772`），且 **DFT 已经做到"每槽 14 个变换
   共用一条命令缓冲"**（`begin_block()`/`end_block()`，`OCUDU_DFT_OPEN_BLOCK` 默认开）；
2. **搬进去会失去重叠**：实测 DFT 队列 `busy 415.7 µs` 与车道 `busy 591.8 µs` **本来在并行跑**；
   并入同一条缓冲后**串行**。**P3 已量过同类代价：整跳一条缓冲让后端跨度 +131 µs**（§5.8.27 ②）。
   ⇒ **D1 的净值未测，不能假定为正**；
3. **收包粒度不是杠杆**（本会话刚量完，§5.8.30，**已更正槽长**）：**每槽等待在两条腿之间不变
   （997.9 vs 997.4 µs）**，端到端只 −1.6%。可达粒度是 **0.5 slot**（`half_slot`），
   `OCUDU_UL_RX_SYMBOLS` 那条旋钮**没生效**。**别再回头做这条**。
   （遗留待解释：**每槽等待 ≈ 998 µs ≈ 整整一个 1 ms slot**，且腿2 的 `[ul_rx_wait]` min=1.0 µs，
   说明宿主领先量逐块变化；要查清需**单槽时间线**而非分布。）

### 5.3 ⇒ 建议的第一步（写在 §5.9 ⑤，最小、可判、不押架构）

**先量"跨队列栅栏本身值多少"**：让 DFT 与车道**同队列**（去掉跨队列栅栏），**保持两次提交**，
跑一条腿。判据（**都不需要 GPU 计时**）：`[ul_gpu_lane]` 的 `gap`、`queue: burst commit -> burst start`、
`dft residency` 三项 + 端到端 `[ul_pipeline]`。

* **栅栏几乎不花钱** ⇒ 路①那个大改动的收益只剩"1.83 次提交"，**优先级应下调**；
* **栅栏显著** ⇒ 再评估路①（让车道缓冲从"样本到齐"就打开）。

**目的：把 D1 的收益拆成"栅栏"与"提交数"两块，避免为一个没量过的收益做架构手术。**

### 5.4 顺手要修的（口径）

* `merged` 下 `burst dispatches` 的 `channel_estimator=0` 是**纯计数缺口**（`merged` 分支提前返回，
  没走到 `end_stage_async()` 里那次 `count_dispatch`）——**纯计数**，改 D1 会碰同一段代码；
* **`commit` 是"谁调用"的动作**，各引擎各记自己的账：`merged` 下 `mmse_ce commits=2`、
  `equalizer/demapper commits=0` **都不是"没干活"**（§5.8.27 ⑤）。
...
| `OCUDU_UL_RX_SYMBOLS=N` | 整符号收包（**实验性**，有已知停机竞态）。**实测无法把块压到 1 slot 以下**（§5.8.30）|
