...
| `platform_portability_design_rules.md` | **The design constraints that make the software portable once another candidate platform appears** (v1.1). Follows from the point above: what we depend on is the *required hardware capabilities* plus *software applicability*, not Apple Silicon or macOS. **C1-C13** is a thirteen-item capability contract phrased vendor-neutrally, each with what it degrades into when missing - written as a questionnaire for a candidate platform. **S1-S6** is the applicability half ("is the stack usable *today*"). **R1-R24** are the design rules, each tied to the measured failure it came from (e.g. R3: an ordering token must be scoped to the producer-consumer pair, not to "the newest thing on the queue" - the cross-lane stall measured as `commit->start = 5.0028 s`; R4: hazard tracking keyed on resource objects rather than addresses means the caller must canonicalise identity at the address level, measured at 200/200 pass vs 200/200 fail on an aliased pair; R18: every kernel must terminate for arbitrary parameters, because a GPU hang here needs a **forced power-off**). **Section 4.5 is fifteen evidence rules** - they belong in a portability document because on a new platform you lose the ability to *judge* performance before you lose performance. It names the five patterns this repository already gets right (`dft_processor_grid_write`, `resource_grid_device_view`, the `grid_ready`-family hooks, `phy_pipeline_mode` + the tri-state contract, and the four-line engine A/B loop in `ai_train/convert_coreml.py`), then keeps an honest ledger in **two halves**: 16 naming/registration debt items (no blockers; about half are one zero-behaviour-change commit) and **12 semantic leaks, all blockers** - `#if` that changes what the *program means* rather than which implementation is compiled, including five of the eight pipeline-contract checks being compiled out behind a Metal-named build option, and the `gpu` mode's own crossing criterion being registered only from Metal code. Its opening criterion - *a port may only touch files under `*/metal/`* - **fails today**, and the ledger says exactly where. |
