...
| **跨实现比较**（CPU ↔ 设备/GPU）| **不要求逐字节** —— 两套实现不同，逐字节既不可能也不必要。要求：**解码结果同分布**（哪些跳解得出来）+ **指标差落在写明的容差内**（SINR / NMSE 之差，容差必须写出来）| 拿一套实现当另一套的参考时（§5.3.7 的 CPU 参考链就是这条）|
...
判据是"同分布 + 容差内的差"，不是字节。§5.3.7 的 CPU 对比按这条写。
...
1. **回退在 `mode=gpu` 里是缺陷，不是健壮性。** §5.3.1 的 18 个 `mmse_refusal` 理由与 §5.3.2 的站点表
   ...
   **★ 判定规则（从既有 18 个理由的分类直接推出，不需要新概念）**：
   ...
   这条规则同时保证 §5.3.3 的八个旋钮自证臂**不会**被 strict 破坏。
...
2. **判据措辞随之修正：不再用"收益 / 适用范围"给一项工作定优先级。** 排序标准是**目标完成度**：
   ...
   | 面 | 现状 | CPU 的参与点 |
   |---|---|---|
   | **数据面（IQ→LLR 的样本与数据）** | ✅ **已清零**：空口 16,067 跳 **0.00 读 + 0.00 写/跳**、分项表空（§5.3.8）；离线复测不带 dump 也是 0.00 + 0.00 | **0** |
...
§5.3.7 那句"59 次全是 capture 自己要宿主网格"现在有了机制上的对应物。
...
> 皆空、契约 8/8、0 RF failure**（§5.5.3 S13-P2、§5.3.8 S13-P2c，腿台账 §12.1）。
> **本节 §2.1–§2.5 记的是 2026-09-19 的历史基线**（`ota-b2-recon`/`b3b`/`b4` 那一代计数器口径），
> 保留它是为了 §2.2 的逐跳构成与 §2.4 的口径警告。**引用"当前"数字请引用 §5.5.3 / §5.3.8，不要引用本节。**
...
> **现行状态是契约 8/8**（§5.3.8，`6742e2f271`）。这一段保留下来，是为了 §3.2 第 2 条限定
> （"曾经有过一次被撤回的 8/8"）有据可查。
...
> ⚠ **2026-09-23 里程碑审计对这张表的更正（树 `29686076d2`，逐条读代码核对；读这张表之前先读这里）**
>
> 1. **第 2 行 `dft radio inputs` 的"通过规则"已作废 —— 这是本表最危险的一行。**`radio*100 ≥ transforms*99`
>    是 §5.9.99 修正**之前**的旧式（它比的是两条不同口径的计数器：一个是交棒路由的变换数，一个是普通路由的）。
>    今天的判决是 **`staged == 0`**（`ocudu_dft_metal_engine.mm:302`；`staged` = `wrap_copies`，唯一累加点 `:764`），
>    `radio_inputs`/`plain_*` 现在**只打印、不判**（`:268`）。照本表第 2 行去核 `8/8`，核的是一条不再存在的判据。
> 2. **行号已漂移**（表里给的是当时的位置）：第 1 行计数器现在在 `gaps :685`、`blocks :697`（表里 `:468/:480`）；
>    第 4 行 `device :867`、`host :873`（表里 `:844/:850`）；第 5 行登记点 `port_channel_estimator_metal_mmse_impl.cpp:743`
>    （表里 `:647`），判决在 `phy_pipeline_crossings.h:514`（表里 `:346-401`）。
> 3. **表里没写、但宣布 `8/8` 之前必须一起读的三件事**：
>    ① **六条可以是 `nullopt`**（第 1/2/3/4/5/8 行的 `nullopt` 条件逐个在代码里），而**第 6、7 行永远适用**
>    —— 它们**零分子也算过**（`:205`、`:219`），不构成正面证据；
>    ② **分母今天是"可缩"的、不是假想的**：`ENABLE_METAL_STATS=OFF`（Apple 默认 configure）时**只有第 5 行**
>    会登记（它是唯一无守卫的那条）⇒ `MET (1 of 1)`；`mode=cpu/cpu_gpu` 时第 5 行不登记 ⇒ `MET (7 of 7)`；
>    ③ **第 8 行 `host sample assembly` 到今天仍既无臂也无配方**（§5.9.114 只覆盖了七条）—— 八条里唯一如此的一条。
...
2. **曾经有过一次被撤回的 `8/8`。** 腿 `gnb_gpu_s5_crossings0_0918_2135`（`a656133d70`）也报过
   ...
   有逐跳 `refusals` 理由计数、设备跳计数器修掉了"回退路由上分母为 0"的缺陷（§5.3.4）。
...
| **S13-P1** | 回退路径的可见性（仪表）| ✅ **全部完成**：第一批（`b3f72deadb`）基类虚钩子 `account_host_grid_read`（默认空 ⇒ CPU 车道不变）+ `ce: rx pilots staged (host)` —— 正是它们**照出了那条每跳往返**；**剩余部分见 §5.3**：`refusals=` 拒绝计数（18 个理由，§5.3.1）+ 读侧分项表 + A/R_hp / y / qy 站点（§5.3.2）+ 八个旋钮臂的自证（§5.3.3，**顺带修掉"设备跳"分母在回退路由上为 0 的缺陷**）；**空中腿 `s13p1b_0919_2322` 通过并当场抓到 §5.3.5 那条窄分配的每跳写** |
| **S13-P2c** | 窄跳（分配 < 1 个块）的相关矩阵：宿主建的 A **没有对角加载** ⇒ 2 PRB 恒 0% CRC | ✅ **完成并空中验证**（`b1ab17c937`，腿 `narrow-fix_0920_0508`，§5.3.7/§5.3.8）：**2 PRB `0.0% → 77.5%`**、SINR 中位数 **−10.7 → +13.0 dB**；契约 **8/8**、两张分项表都空、`refusals=<none>`、**RF failure 0**；27 捕获逐字节不变；机制由 `HOST_SCALARS=1` 与 `dev` **20/20 逐字节相同**证明；**真 `--cpu` 参考链与 `dev` 逐条同分布（OK × 18/20）**（§5.3.7 勘误修正了这一批原来的"CPU 全 KO"）|
...
> ⚠ **2026-09-23 里程碑审计：本表是 s47 的一条腿，其中两行已经不是今天的默认读数 —— 引用本表前先读这里。**
> 1. **`cbs/lane=1.00 (max=1)` 已不成立**：雷的修复（fenced 相关构建进自己的命令缓冲，§5.9.91 ①）把它变成
>    **`2.00 (max=2)`**，这是**已登记的代价**（D1 那条"每槽一次提交"的性质被让出），硬门也已随之改成
>    `cbs/lane ≤ 2.00 (max=2)`（§5.9.96）。今天 7 条默认腿一致读 `2.00/2/d0`。
> 2. **`RF 实时失败 = 0` 是"一条腿的断言"**：它只在 `s47` 与 `s67` 成立，而 `s62=2 / s63=4 / s64b=21 / s65=8 /
>    s66=40 / s69=33`（`[RF] [W] Real-time failure in RF: underflow|late`），**`leg_gate` 里没有这条判据**
>    （2026-09-23 起它打一行 INFO，但**故意不判**——阈值未登记）。**宣布里程碑时只能说 `gaps=0`（131/132 条腿成立），
>    不能说"0 RF 失败"。**
> 3. 其余各行（穿越 `0.00+0.00`、契约 8/8、`handed/taken`、`dropped=0`）到今天的默认腿上仍然成立；
>    但**每条腿的出处要连"该腿是否自证无旋钮"一起读**（provenance 只从 s63 起存在，见 §5.9.120 ⑥）。
...
**⑦ ⚠⚠ 更正 ⑥ 的比值，并把问题问准：s64 里 DFT **每个变换各自 commit+wait 一次**（D1 的反面）**

⑥ 里我把 s63 的比值写成"392686/392714 ≈ 99.99%"——**那是从 keepalive 字段推的，错了**。原文是：

```
s62 (n1, 忙)   dft radio inputs: 384846 of 1 transforms read the radio buffer -> OK
s63 (n1, 忙)   dft radio inputs: 392714 of 1 transforms read the radio buffer -> OK
s64 (n78, 空载) dft radio inputs: 2352 of 51265 transforms read the radio buffer -> FAILED
```

配合 `[metal_stats] dft` 行（同一份 stderr，两个计数器都在这一行）：

| 腿 | `commits` / `transforms` / `waits` | `radio_inputs` | 读法 |
|---|---|---|---|
| s62 | **1 / 1 / 1** | 384846 | = **27489** 个交棒栅格 × 14（`handed=27489`）|
| s63 | **1 / 1 / 1** | 392714 | = **28051** × 14（`handed=28051`）|
| s64 | **51265 / 51265 / 51265** | 2352 | 只有 **168** 个栅格（×14 = 2352）走了交棒 |

⇒ **真实的差别不是"99% 还是 4.6%"，而是：**
* n1 的两条腿：DFT 的变换**全部**骑在交棒/融合路线上，引擎自己 **0 次独立提交**（`commits=1` 是那个零头）；
* **s64：51265 次变换各自 commit + wait 一次**（`commits=transforms=waits=51265`）——
  这正是 **D1 要消灭的"每个变换一次 CPU 提交"**，而只有 168 个栅格（2352 个变换）走了融合路线。

**⇒ 所以 s64 是一条"DFT 掉出融合路线"的腿**，而且它发生在**空载**状态（整条腿只有 18 个 device hop、
20 个 PUSCH，手机在最后 1.5 s 才刚开始接入）。这让它**同时**有两个身份：接入失败的证据（用 §② 的射频数字，仍然成立），
以及一个**独立的、更值得追的问题**：**空载时 DFT 是否会掉出融合路线？**

**要钉死需要两条腿，而不是推断**（这条腿里"空载"与"n78 几何"是混在一起的）：
1. **空载 n1**（同一份配置、不接手机）—— 若也掉出去 ⇒ 与几何无关，是**空载**这件事；
2. 修好增益后的 **n78 带话务** —— 若回到"全部骑交棒" ⇒ 确认是空载（或接入前的状态），不是 n78。
判据沿用契约本身：`dft radio inputs` + `commits/transforms/waits` 三个数是否 1:1:1。

**顺带记一条仪表缺陷（与 §3.4 同族）**：这个检查的分母**不是总数** —— 它在一条有 392714 次
"读了射电缓冲"的腿上印 **`of 1`**（`transforms` 只在独立提交那条路上累加）。也就是说它拿两个**不同口径**的
计数器在比（`radio_inputs` vs 独立提交数）。本次它的 FAIL **判对了**（独立提交占绝对多数确实是坏事），
但**"OK"是不可信的**：分母为 1 时它恒过。要么把 `transforms` 改成真正的总数，要么把这条检查改名为
"独立提交占比"。

#### 5.9.98 ★★ n78 宽腿（`rx_gain` 40→70 之后）：**接入恢复、契约 8/8、大跳把每跳成本翻倍**；并解开 §5.9.97 ⑦ 的悬案

**① 三件事同时成立**

* **接入问题解决**：PRACH `rssi = −36.5 dB`（gain 40 时 −56.0；n1 工作腿 −34.4）、`power_dB = −43.57`（原 −64.65）
  ⇒ 回到工作腿量级；手机于 t≈21 s 完成 `UE Create` + `InitialULRRCMessageTransfer`。
* **契约 8-of-8 MET** ✓（`radio sample continuity: 0 gaps over 264753 blocks`、crossings `0.00+0.00`、`host sample assembly` 全部就地读）。
* **这是第一条"大跳"腿**：51 PRB（n1 是 25）、UL TBS 中位 **2880 B**（n1 是 96 B）、≥1000 B 的 grant **65.3%**、
  12:57 那一分钟 **20.35 MB / 2.7 Mbit/s**（7209 个 grant，256QAM 到 6021 B）。

**② §5.9.97 ⑦ 的悬案：掉出融合路线**不是**空载造成的，是"车道不接的那些 UL 槽"**

| 腿 | `dft commits` / `transforms` / `waits` | `radio_inputs` | 读法 |
|---|---|---|---|
| n1 s62 / s63（忙）| **1 / 1 / 1** | 384846 / 392714 | 变换全骑交棒 |
| n78 s64（空载）| 51265 / 51265 / 51265 | 2352（= 168 栅格）| 几乎全走独立提交 |
| **n78 s64b（7458 跳）** | **158845 / 158845 / 158845** | 301658（= 21547 栅格）| **仍有 35% 走独立提交** |

⇒ **有话务也照样掉出去**：交棒 `handed=21547` 而车道只 `taken=7458`（`evicted=21291`），
**n78 TDD 下大量 UL 槽没有 hop**，那些槽的变换只能走独立路径。这是**几何/流量形态**的性质，
**不是本次修法的回归**（同二进制的 n1 忙腿 8/8 且 `commits=1`）。
⚠ 同时坐实了检查的缺陷：它的分母只数独立提交那条路，于是**同一结构在两条 n78 腿上给出相反判决**
（空载 FAIL、忙腿 OK，还印出 `301658 of 158845` = 190%）。**它的 OK 不可信。**

**③ 大跳把每跳成本翻倍 —— H1/H2 的第一份真数据**

| 量 | n1（25 PRB, TBS 96 B）| n78（51 PRB, TBS 2880 B）| 比 |
|---|---|---|---|
| `ch_wt`（围栏那条相关构建 = 纯算术）| 23.8 µs | **40.1 µs** | ×1.68 |
| `merged_hop`（整跳）| 572.2 µs | **1185.2 µs** | ×2.07 |
| residency 中位 | 552.0 µs | **1337.1 µs** | ×2.42 |
| `CE mean total` | 24.7 µs | **36.5 µs** | ×1.48 |
| `defer_wait` 中位 | 675.7 µs | **1455.9 µs** | ×2.16 |
| `[ul_gpu_pipeline]` 中位 | 1811.6 µs | **3044.1 µs** | ×1.68 |

* ⇒ **H1（算术型）在"每跳成本"这一层成立**：PRB ×2、TBS ×30 ⇒ 每跳占用 ×2.4；
* ⇒ **一跳已装不进一个 30 kHz 时隙**（1337 µs vs 500 µs）⇒ n78 上"一跳一槽"不可能，余量只能来自**上行占空比**；
* ⇒ 硬门仍过：`cbs/lane=2.00 (max=2)`、`dropped=0`、`carried=0` ⇒ **合批在大跳下没有洞**（§5.9.96 登记的那一条）。

**④ ★ 余量的正确口径（已修进 `wip/ul_load.sh`）**

`余量 = 1 − residency/slot`（§5.9.92 的口径）**只在"一跳约一槽"时成立**：在 n78 上它印出 **−167%**
（上行占空比 2.8%，一跳每 ~18 ms）—— 我第一版工具就印了这个数，是**分母选错**，不是车道坏了。现在印三个**与配置无关**的量：

* **墙钟占用率 = lanes × mean residency / 墙钟** ⇒ s62 **10.7%（余量 89.3%）**、n78 **3.7%（余量 96.3%）**；
* **可持续速率 = 1 / mean residency** ⇒ n1 **1516 跳/s**、n78 **760 跳/s**（实际 162 / 28 跳/s）；
* **突发比 = residency 中位 / period 中位** ⇒ n1 52.4%、n78 **86.5%**（最紧的到达形态）；
* 5.9.92 那个口径**带条件**保留：只有 `period/slot ∈ [0.75, 1.5]` 时才印数字，否则印 `n/a` ——
  这样老数字在它有意义的地方仍逐位可比（s62 仍印 44.8%）。

⇒ **n78 的正确结论**：吞吐余量很大（96.3%），但**每跳实时成本已占 30 kHz 时隙的 2.7 倍**，
所以约束在**占空比**：可持续 ~760 跳/s ≈ **全部时隙的 38%**，超过它车道开始排队。

**⑤ ⚠ 新问题：12:57 的 KO 突发（记账，不下结论）**

* **现象**：那 60 s 内 7209 个 grant、**1025 个 `crc=KO`（14.2%）**，全是 **256QAM 大 grant**（5763/5252/6021 B…），
  `iter=6.0`（打满上限）而 `sinr=20–24 dB`；同一批里 **454 个同样 5763 B 的 grant 是 OK 的**。
* **OK 的解码只要 1–2 次迭代**（2533×`iter=2.0`、2151×`iter=1.0`）⇒ **失败的那些确实是链路差**，
  不是迭代上限卡住（上限来自配置的 `pusch_dec_max_iterations: 6`）。
* **与 RF 欠载不重合**：21 次 `Real-time failure in RF: underflow`（UHD 的 **TX** 欠载）在
  12:56:15（接入时）与 12:57:37 之后（突发已停），而 KO 窗口是 **12:57:20–27**
  ⇒ "下行 DCI 丢失 ⇒ 手机按旧 grant 发 ⇒ gNB 解出垃圾"这条机制**不成立**。
* **时间形态**：突发前 3 s **0 KO**（601/598/600 OK），随后 ~8 s 稳定在 **10–23%**，突发一停 KO 也停。
* **最可能的解释**：顶格 256QAM + 大 TBS 下链路骑在门限上，而 gNB 的 `sinr` 是 **DM-RS 估计、偏乐观**
  ⇒ 落在 §4 刚解锁的 **`#12` LA** 范畴。**这条腿不足以定论。**
* **登记的下一步 A/B**（四条互相独立、都便宜）：① 重跑同一次上传，先看"前 3 s 干净、之后 ~20%"是否复现；
  ② 抄上 n78 缺的 **GPSDO 三项 + `clock: gpsdo`/`sync: internal`**（n1 有）⇒ 若 KO 消失则是时钟/CFO；
  ③ 打开配置里被注释掉的 **`pusch_channel_estimator_cfo_compensation: true`**（该配置自己的注释就在警告 CFO）；
  ④ 把 **`pusch_dec_max_iterations: 6`** 放回默认（更大）⇒ 若 KO 大减，说明是上限而非链路。
  **判据（跑前登记）**：同一分钟内 `mod=256QAM` 且 `tbs ≥ 2880` 的 KO 率（本次 **14.2%**），四臂只比这一个数。

#### 5.9.99 ★★ A1-2 判定 + A1-1 修正：`dft radio inputs` **从来量的不是它声称的东西**；DFT 的独立提交是**覆盖缺口**

**① A1-2：n78 上 35% 的变换走独立提交 —— 不是设计意图，是车道覆盖缺口**

机制（代码级）：block 累积（头文件承诺的"每槽 14 个变换共用一条命令缓冲"）**只在
`ofdm_demodulator_impl.h:150-161` 的 `set_lane_slot()` 里打开**，而 `set_lane_slot()` 由**车道**驱动
（`dft_processor_metal::set_lane_slot` → 引擎）。n78 TDD 下**大量 UL 槽没有 hop**（车道不认领），
那些槽的变换只能走 `submit()` 的逐变换路径 ⇒ 每个变换一条命令缓冲 ⇒ s64b 的
`commits=transforms=waits=158845`。

* 证据链：`handed=21547 / taken=7458 / evicted=21291`（车道只认领有 hop 的槽）；
  **同一二进制的 n1 忙腿 `commits=1`**（几乎所有 UL 槽都有 hop）；block API 的存在理由就是消掉逐变换缓冲
  （头文件：一条命令缓冲 ~12.7 µs，"装 1 个还是 14 个变换都一样"）。
* 代价（估算）：n78 上 ~11346 槽 × 14 ≈ **158845 次 commit+wait ≈ 1200/s**；按 12.7 µs/缓冲
  ≈ **1.5% 的单线程时间**。不大，但它正是 D1 要消灭的"每变换一次 CPU 提交"。
...
`transforms` = 普通提交路由的变换数。而**两条路由都是零拷贝**（`submit_at()` 同样走 `wrap_buffer()`）
⇒ **它比的不是"零拷贝 vs 宿主拷贝"**。三条腿的原文把这件事暴露得很干净：

| 腿 | 旧式打印 | 旧式判决 | 真相 |
|---|---|---|---|
| n1 s62 / s63 | `384846 of 1` / `392714 of 1` | OK | **分母恒为 1**（只有普通提交那条路累加）⇒ **恒过** |
| n78 s64（空载）| `2352 of 51265` | FAILED | 两条路由都是零拷贝 |
| n78 s64b（忙）| `301658 of 158845` | OK | 190% —— 比值毫无意义 |

**修正后**：判据 = **`wrap_copies == 0`**，即"零拷贝 wrap 被拒、退回 `newBufferWithBytes` 真拷贝"的
计数点（同一处也计入 crossings 的 host write）。消息把**两条路由的占比**当信息印出来（值得盯，但不是判决），
并且**单位分开**：`staged` 数的是 **wrap 次数**（一次运行会 wrap 输入和输出），不与变换数相减 ——
旧式混了口径，**混单位就是同一种错误**。

**③ 反向臂（`lib/phy/generic_functions/metal/test/dft_processor_metal_unit_test.cpp`，放在 `main()` 最前）**

* **控制臂**：按引擎真实请求（**整批 196608 B**）页对齐分配 ⇒ `2 transform(s) … 0 buffer wrap(s) … -> OK`；
* **反向臂**：同一个请求、只给 **1 页**（走生成代码里写明的"请求放不进这块分配 ⇒ 暂存拷贝"分支）
  ⇒ `6 … 1 buffer wrap(s) … -> FAILED`；
* **两条一起才有意义**：旧式在这两跑里**都是 OK** ⇒ 这个臂本身就是"旧判据测不出东西"的证明。
* 踩到的三个坑（记下来，免得重踩）：① `compat::page_size()` 在本机是 **16 KiB**，按 4 KiB 算的缓冲不是页整数倍
  ⇒ 被登记的分配"可用空间向下取整为 0" ⇒ **连控制臂都被迫拷贝**；② 引擎的 wrap 请求是**整批**
  （1536×16×8 = 196608 B），**不是本次提交的变换数** —— 我按"1 个变换"分配，于是**控制臂自己成了反向臂**；
  ③ 这个单测**前面的小节自己的非页对齐缓冲也会触发拷贝**（实测 2 次），所以这一块必须跑在 `main()` 最前，
  否则"控制臂读 OK"根本不成立。
* 复跑：ctest **36/36**、value_net **47/0**、probe **6/6**。
...
**① A1-3：n1 重上行腿 —— 命令与判据（判据仍是 §5.9.96 预登记的那两条，未改）**

s63 **无效**的原因不是配置，是**手机侧没有产生上行流量**（§5.9.97 ①）。所以重跑的全部要求是"把上血压上去"：

```bash
# 手机侧：持续【上传】（上传测速 / iperf3 上行 / 大文件上传），≥ 90 s
sudo -E bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu s65-heavy-ul
# 停：只按一次 Ctrl-C，等收尾块打完
bash doc_chinese/phy_pipeline_gpu/wip/ul_load.sh s62-default-fenced s65-heavy-ul
```

**有效性（先读 LOAD 块，不达标这条腿不说话）**：UL 载荷 **≥ 2.0 Mbit/s**（s62 = 0.39）且**有上行授权的槽 ≥ 50%**（s62 = 16.2%）。
**硬门**（任一不过 ⇒ 回归报告，停止读余量）：契约 8-of-8 `(mode=gpu)`、`stale=0`、crossings `0.00+0.00`、
`dropped=0`、gaps=0、`cbs/lane ≤ 2.00 (max=2)`。
**读数**（工具已带新口径）：墙钟占用率 + 可持续跳率 + 突发比（§5.9.98 ④），以及 **`rx pool verdict`**（§5.9.100）。
...
* **现象**（§5.9.98 ⑤）：12:57 那一分钟 7209 个 grant、**1025 个 `crc=KO`（14.2%）**，全是 256QAM 大 grant
  （5763/5252/6021 B…），`iter=6.0` 打满而 `sinr=20–24 dB`；同尺寸 5763 B 有 454 个 OK；
  OK 的解码只要 1–2 次迭代（⇒ 失败的确链路差）；KO 窗口（12:57:20–27）与 21 次 TX underflow **不重合**
  ⇒ "丢 DCI 按旧 grant 发"不成立。
* **判据（跑前登记，唯一）**：同一分钟内 **`mod=256QAM` 且 `tbs ≥ 2880` 的 KO 率** —— 本次基线 **14.2%**。
* **四个已登记 A/B**：① 重跑同一上传（先要可重复性）；② n78 抄上 GPSDO 三件套 + `clock:`/`sync:`（n1 有、n78 没有）；
  ③ 打开配置里注释掉的 `pusch_channel_estimator_cfo_compensation: true`；④ `pusch_dec_max_iterations: 6` 放回默认。
* **为什么可以延后**：它**不影响管线契约**（那条腿契约 8/8），只影响顶格 MCS 的吞吐；且 §5.9.98 ④ 已证明
  车道在大跳下仍有余量（96.3% 墙钟占用）。
* **归属**：与 `#12 LA 错配`同源（`sinr` 是 DM-RS 估计、偏乐观 ⇒ 外环把 MCS 推到门限上），做 `#12` 时一并。

#### 5.9.102 ★★★ 全面测试与检查审计（六遍；第 5 遍=空中端到端，待 `s65-heavy-ul`）

**第 1 遍 回归面**（相对空口已确认的 `3e987189f1`）：9 文件 / **+1392 / −10**，其中**生产路径只有 2 个文件**：

| 文件 | 改动 | 位置 |
|---|---|---|
| `ocudu_dft_metal_engine.mm` | **+32 / −8** | **全部落在 `register_dft_contract_check()` 内**（一个检查的判据与消息，§5.9.99）|
| `lower_phy_baseband_processor.cpp` | **+16 / −0** | `rx_pool_accounting` +4、`rx_pool_note_taken()` +12（一次性 WARN，§5.9.100）|

逐行核对：新增行里出现 `commit/dispatch/wait/memcpy/newBuffer/encode/barrier/submit` 的 **26 行全部在注释或字符串里**；
唯一的 "wait" 代码在**单测文件**中（Test 15 的 (d) 断言）⇒ **生产改动纯仪表，无行为变更**。
其余 7 文件 = 单测 2 + `wip/` 脚本 3 + 记录 2。

**第 2 遍 离线门（在 `6afafa95c0` 上实跑）**

| 门 | 读数 |
|---|---|
| ctest（36 项）| **36 / 36** |
| `value_net.py` | **47 / 0** |
| `ul_pipeline_probe_test` | **6 / 6**（内含四条反向臂）|
| `lower_phy_test` | **528 / 528** |
| handover 臂 / hop 臂 | **5** / **4× differing=0** |
| `edge_block_arms.sh 6` | 默认 **6/6 绿**；`=0` 臂 **6/6 红**（5 次由 (d) `0 of 480`，1 次 Test 3 先红）|

**第 3 遍 断言反向臂普查**（每条给出"怎么红"或"为什么不能红"）

| 断言 | 怎么让它红 | 实测 |
|---|---|---|
| Test 3 / 9 / 11 | `OCUDU_CE_CORR_FENCED=0`（前缀形态）| Test 3 = 1/8…2/20、Test 9 = 4/8…4/20 |
| Test 15 **(a)–(c) 值判据** | 同上（雷是 ~1/19000 跳的竞态）| 2/8 抓到真爆点（band 63 / 143）|
| **Test 15 (d) 结构判据** | 同上；**确定性** | **6/6 红**（`0 of 480 hops waited`）|
| **`dft radio inputs`** | 分配过小 ⇒ wrap 被拒 | 本会话**双臂**：`OK` → `FAILED`（§5.9.99 ③）|
| 其余 6 项契约 | 各有天然路径（例：`ce device estimates` 用 `OCUDU_CE_CPU_CE=1` ⇒ gpu 模式下 device=0 ⇒ 应红）| **未逐项实跑**（登记为待办，见第 4 遍的 ④）|
| **RX 池 EMPTY WARN** | **做不到**：池大小有硬下限 8（§5.9.100 ④）| **登记为待观测**（下次 `free_min=0` 必须恰好一次）|
| `ul_load.sh` 的判决行 | **是读数，不是断言**（不参与任何门）| — |

**第 4 遍 契约八项的**定义**审计**（不读 OK/FAIL，读判据表达式）

| # | 项 | 判据 | 审计结论 |
|---|---|---|---|
| 1 | `radio sample continuity` | `gaps == 0` | 直接可信 |
| 2 | **`dft radio inputs`** | **`wrap_copies == 0`**（本轮修）| 旧式比的是两个**不同口径**的计数器；已修 + 双臂 |
| 3 | `zero-copy wraps` | `replaces==0 && failures==0 && …`，无 wrap 时 n/a | 可信 |
| 4 | `ce device estimates` | offloaded：`device > 0` | **范围有意收窄**（只查"设备路径没整个消失"），**代码注释已声明**；真实分工在 `[mmse_time_sum] hops_gpu/hops_no_gpu/fb_blocks`。**建议**（不阻塞）：gpu 模式下收紧为 `host == 0 && device > 0`，**反向臂现成**（`OCUDU_CE_CPU_CE=1`）|
| 5 | `host device data crossings` | 每跳读数 ≤ 模式允许值（gpu = 0）| 可信 |
| 6 | `cfo compensation` | `(round_trips == 0) \|\| (cfo_hz != 0)` | 可信 |
| 7 | `baseband metrics` | `(measured == 0) \|\| consumed` | 可信 |
| 8 | `host sample assembly` | `assembled*100 <= symbols`（1% 容差，注释给理由）| 可信 |

**第 5 遍 空中端到端**：**待跑**（命令与判据见 §5.9.101 ①）。审计前的空中基线 = s62 / s63 / s64b 三条腿
（契约 8/8、`stale=0`、crossings `0.00+0.00`、`cbs/lane=2.00`）。

**第 6 遍 记录遗留项普查**：记录里共 **48 处** OPEN/遗留/未解释标记（近 8 千行内 12 处，其余属 §5.7.x / §17.x / §20.x 旧工作流）。

* **当下开放面已被覆盖**：`session_handoff_2026-09-23-2.md` §5 的表 + 本轮 A1 清单 = 全部**知情**项
  ⇒ **没有"仍开但无人知道"的项**；
* **记账卫生发现**：旧标记里有多处**已被后续工作超越却没更正**（例：§20.6 仍写"空口契约 7/8"，今天是 8/8；
  §17.6 的"发布路径尚未接"）⇒ 登记**待办**：给这些行加"已被 §5.9.x 取代"的注记（追加、不改原行），
  或明确宣布该工作流关闭。**本轮不做**（它是一次独立小普查）。

**审计结论**：① 生产改动只有两处**仪表**，无行为变更；② 离线门八项全绿；③ 每个断言要么有**实测**反向臂、
要么**写明做不到的原因**；④ 契约八项里只有 `dft radio inputs` 曾经名不副实（已修 + 双臂），
`ce device estimates` 是**有意收窄且已声明**；⑤ 空中一遍待跑；⑥ 记录**无隐形开放项**，但有旧标记需补注。

#### 5.9.103 ★★ 第 6 遍（记录遗留项普查）的**裁决表**：把 50 处标记逐条判掉

§5.9.102 第 6 遍只做了普查、把补注登记成待办。本节把它做完 —— 逐条裁决，**不改原行**（记录是追加式的）。

**① 先更正口径：50 处里有假阳性**

grep 关键字会命中两类**不是开放项**的行：① **标识符**里的 "OPEN"（`OCUDU_DFT_OPEN_BLOCK`，L2738 / L8974）；
② 叙述句里的"尚未/仍未"（如 L4922 的"ue=8192 = 尚未成为 UE"、L9876 的坑条目）。
⇒ **真正的"开放项陈述"约 30 处**。下表按四类裁决。

**② 裁决表**

| 类别 | 行 | 内容 | 依据 |
|---|---|---|---|
| **已被取代** | L7830 / L7833（§5.9.87）| "机制尚未落到一行代码上"、"翻默认是用生产性能换一个尚未解释的修法" | **§5.9.88**（逐跳链条）+ **§5.9.93**（默认已翻、门达成）|
| | L8003（§5.9.88 末）| "默认仍未翻" | **§5.9.93** |
| | L8638（§5.9.97③）| s64 留下的"尚未开的独立问题" | **§5.9.98 ②**（机制=车道不接的 UL 槽）+ **§5.9.99 ①**（判定=覆盖缺口）|
| | L11901（§20.6）| "契约现在在空口上是 7/8" | 今天 **8/8**（s62/s63/s64b 三条腿实测）|
| | L330（§3 代码地图）| `ocudu_demod_metal_engine.mm` "仍未经审计" | 契约的 crossings 行已把 **demapper** 列为已审计模块（四条 reporter 之一）|
| | L5440（§5.9.54 #5）| 前端栅栏代际 + `wait_all()` 不覆盖交出的块 | **机制已退役**：`dft_processor_metal_unit_test.cpp` 的 §5.9.65 NOTE（交棒成默认后前端队列无提交可关联，全部空中腿该栅栏 signals=0 waits=0）|
| | L5442（§5.9.54 #7）| 探针 `stale=` 分流 | 已落地并被所有腿使用（`[ul_pipeline] stale=`）|
| | L6267（§5.9.67）| #4 单车道并发 1→2 "仍在性能线待定" | **已降级**：一槽至多 1 PUSCH（99.97%，§5.9.47 ⑤）|
| | L5024（§5.9.48）| D1 翻默认的 p95 代价"数值待定" | **§5.9.94** 的空中三腿 A/B 给出成本（槽级 span 反而不差）|
| **仍开（知情）** | L5438 / L5445 / L5446 / L5447 / L5448 / L5451（§5.9.54 #3/#10/#11/#12/#13/#16）| 认领宽限期 / 下行口径 / TA fence / LA 错配 / 延迟批次等待 / Ubuntu 同步 | = `session_handoff_2026-09-23-2.md` **§5 的表**逐条在册（#3/#10/#11/#12/#13/#16）|
| | L4298 / L4309 / L4353 / L4354（§5.9.36/40）| 设备侧等待的第二条反向臂 OPEN，**harness 无法证伪** | 原因已查清并写在原处（跨队列 + 空载 ⇒ **负载相关 hazard**）；离线优先在这里碰到有解释的墙 |
| | L4497 / L4592 / L4609（§5.9.44）| 开放项 #1 重开；"K 跳在 1 线程下能否共享一条命令缓冲"仍未判 | 前置条件当时写清；仍开 |
| | L4208 / L4243 / L4258（§5.9.38）| `hostfirst` 臂一次"槽 2 估计器标量差异"偶发 | 按**偶发未解释**记（harness 也照 OPEN 打印，不假装 PASS）；今日 hop 臂 4×differing=0 未复现 |
| | L6404 / L6491（§5.9.68/69）| trace 把饥饿放大 ~200× 的机制未证实 | 判别实验仍值得做；低优先 |
| | L9098 / L9103（§5.9 D1）| `t2f` 的 ~500/~580 拆分未单独计时 | 原处已写"**记为遗留项，不追**" |
| | L298（§2.5）| 读侧 2.37 的构成"空中尚未逐点确认" | 老基线说明；仍开（低优先）|
| **未核实（需单独看）** | L873（§5.8.5）| `end_stage(WEIGHTS_STAGE)` 被调用 16 次的旁证未解释 | 老工作流；本轮未核 |
| | L2163（§5.8.27）| "机制上说得通但尚未单变量隔离"的重传代价 | 老工作流；本轮未核 |
| | L2406（§5.8.30）| "一个仍未解释的数" | 老工作流；本轮未核 |
| | L5443（§5.9.54 #8）| `burst dispatches` 的 `channel_estimator=0` 缺口 | 本轮未核 |
| | L5449（§5.9.54 #14）| `evicted` 计数语义 | **很可能已闭合**（§5.9.98 用 `handed/taken/evicted` 解释了 n78 的交棒），但未单独核代码 |
| | L5450（§5.9.54 #15）| `--self-test` 门的确切命令（找不到二进制）| 本轮未核 |
| | L5437（§5.9.54 #2）| `not_found` 的小洞（无记录可等时直接读网格）| 本轮未核；`memo -2` §5 未收录（可能已闭）|
| | L9681（§5.7.3）/ L9930（§8.2）/ L10797（§17.6）/ L11144（§17.10.5）| 4 处老工作流的"待定/未定位/未做/仍未解释" | 属 §5.7/§8/§17 三段已封存的工作流；本轮未核 |
| **非开放项（口径假阳性）** | L2738 / L8974 | `OCUDU_DFT_OPEN_BLOCK` 的标识符 | 关键字误命中 |
| | L4922（§5.9.47）| "结案：不做" 段里的叙述句 | 该节本身就是**结案**记录 |
| | L9876（§7 坑条目 #35）| "ctest 不构建"的教训 | 是**纪律条目**，不是开放项 |
| | L8930 / L8935 | §5.9.102 自身的普查叙述 | 自指 |

**③ 结论**

1. **没有"仍开但无人知道"的项**：所有真正的开放项要么在 `memo -2` §5 的表里，要么在本表里（两条清单合起来 = 权威清单）。
2. **9 处已被取代却没更正**（多为"默认仍未翻""契约 7/8"这类**状态句**，后续工作早已越过）——
   本节就是那 9 处的**追加式更正**，原行保持不动。
3. **13 处未核实**：全部属 **§5.7 / §8 / §17 / §5.9.38–54 这些已封存的工作流**，不在本轮范围；
   它们不阻塞任何当前判据，但**下次碰这些工作流时先读本节**，别把里面的状态句当现状。
4. 口径教训（写下来）：**关键字普查会命中标识符与叙述句**（"OPEN"、"尚未"）⇒ 计数不可直接当工作量，
   必须逐条看内容。这跟 §6.6 的"别被 `max` 骗"是同一类错误。

#### 5.9.104 ★ 审计的**最终态复验**（收口提交之后）

* `6afafa95c0..1cd7a76e28` 的 **`lib/` 改动为空**（区间内只有记录 +122 与 `wip/leg_gate.sh` +131）
  ⇒ §5.9.102 第 2 遍那次**全门**（含四条反向臂与 `edge_block_arms`）对**最终 HEAD 仍然成立**——
  二进制除 stamp 串外相同。**这是论证，不是假设**。
* 在 `1cd7a76e28` 上**复跑快门**：ctest **36/36**、value_net **47/0**、probe **6/6**、lower_phy **528/528**；
  `stamp == HEAD` ✅。
* §5.9.102 第 4 遍最后一项（`host device data crossings`）的判据已从源码读全：mode 已发布且 `hops > 0` 时判决为
  **`(reads == 0) && (writes == 0)`** ⇒ 至此**八项判决表达式全部从源码核对过**（不是只看 OK/FAIL）。
* **唯一未闭环项**：第 5 遍（`s65-heavy-ul` 空中端到端，待跑；判据见 §5.9.101 ①/①b）。

#### 5.9.105 ★ 第 5 遍（空中端到端）的**阻塞记账**与"等价性论证"

**① 阻塞面（三条独立证据，2026-09-23 复核）**

1. **本机 USB 上没有 USRP**：`system_profiler SPUSBDataType` 里没有任何 Ettus / USRP / B200 / B210 / NI 条目
   （配置用的是 `type=b200`，USB 设备）；
2. **`sudo` 需要密码**（`sudo -n true` ⇒ `a password is required`），本会话拿不到；
3. **`run_leg.sh` 本身就拒绝非 root**（`run_leg.sh:76`），所以即便有设备也必须先过这一关；
4. `s65-heavy-ul` 的日志**不存在**（`wip/logs/` 里 0 条）。

⇒ **第 5 遍在本会话内无法执行**，不是"没做"，是**做不到**（与 §5.9.100 ④ 的 RX 池反向臂同一类记账）。

**② 等价性论证：第 5 遍要证的东西，有多少已经被证过**

自空口已确认的 `3e987189f1` 起，**生产路径只动了两处，且都在仪表上**（§5.9.102 第 1 遍逐行核对）：

| 改动 | 运行期影响 |
|---|---|
| `register_dft_contract_check()` 里的**判据与消息**（§5.9.99）| **零**：该 lambda 只在契约打印时求值 |
| `rx_pool_note_taken()` 里的**一次性 WARN**（§5.9.100）| 每次 take 多一次原子 load/compare；**至多一次** logger 调用 |

⇒ **管线本身（派发/提交/等待/拷贝/交棒）逐字节未变**，所以 s62 / s63 / s64b 三条空中腿所证的**行为**仍然成立。
**但**第 5 遍要的三件事里有两件**只有真腿能给**：① `dft radio inputs` 的**新格式**在真腿上首次出现（且 n1 忙腿应读 ~99.99% 交棒占比）；
② 若某条腿把池抽空，那条 EMPTY WARN 必须**恰好出现一次**（§5.9.100 ④ 的待观测项）。**等价性论证不能替代这两条。**

**③ 两条收口路径（用户裁定）**

* **A**：跑 §5.9.101 ① 那条 `s65-heavy-ul`（≥90 s 真上行），然后 `bash wip/leg_gate.sh s65-heavy-ul`
  ⇒ 第 5 遍按预登记判据一次判完，审计闭环；
* **B**：**免跑**，并把理由写成"两处生产改动是纯仪表 + 管线逐字节未变（本节 ② 的表）"，
  同时把 ② 里"只有真腿能给"的两条移入**下一轮空口腿的观察清单**（下一个做空口腿的人顺手核）。

#### 5.9.106 ★★★ 第 5 遍（空中端到端）**判读：审计六遍闭环**；而 A1-3 的"重上行"在 n1 上**不可答**（原因是链路，不是流量）

腿：`gnb_gpu_s65-heavy-ul_0923_2131`（145932 槽 = **145.9 s**，基线 s62）。判据由 `wip/leg_gate.sh` 逐条机械求值：
**7 of 9 通过**，两条不通过的都是**有效性**（不是硬门）。

**① 第 5 遍 = PASS（审计闭环）**

| 硬门 | 读数 |
|---|---|
| 契约 | **MET (8 of 8 checks applicable) (mode=gpu)** |
| `stale` / crossings / gaps | **0** / **0.00 + 0.00 每跳** / **0 gaps over 145932 blocks** |
| `cbs/lane` | **2.00（max=2）**，`dropped=0` |

**两条"只有真腿能给"的观察项 —— 一条确认、一条得到反向对照**：

1. **★ `dft radio inputs` 新格式在真腿上首次出现，并给出预期读数**：
   `517693 transform(s) went through the two submit routes: the hand-over route carried 517692 → 100.0%`
   ⇒ 忙 n1 腿上**交棒路由占 99.9998%**。**旧式会印成 `of 1`**（§5.9.99 ② 的假通过），所以这一条正是对那次修正的空中确认。
2. **RX 池：负向对照成立**。`free_min=2 starved_takes=0 starved_events=0`（`held_max=6/8`）⇒ 池没被抽空
   ⇒ **EMPTY WARN 正确地没有打**（`WARN count=0`）。**正向触发仍待观测**（池大小硬下限 8，无法逼出，§5.9.100 ④）。

**与 s62 的复现性**（同配置、同无 knob 默认路径）：`residency` 中位 **540.9 vs 552.0 µs**、
5.9.92 旧口径 **54.1% vs 55.2%（余量 45.9% vs 44.8%）**、`ch_wt` 23.5 vs 23.8、`merged_hop` 564.2 vs 572.2、
`[ul_gpu_pipeline]` 中位 1798.1 vs 1811.6、`defer_wait` 685.8 vs 675.7、`CE mean total` 24.3 vs 24.7、
`crc KO` **2.6% vs 4.2%** ⇒ **六项车道/延迟读数全部在噪声内复现**。

**② A1-3：重上行腿第三次"有效性不达标"，而这次查清了**为什么****

| 有效性判据（§5.9.96 预登记，未改）| 门槛 | s65 实测 |
|---|---|---|
| UL 载荷 | ≥ 2.0 Mbit/s | **0.28**（s62 = 0.39）|
| 有上行授权的槽 | ≥ 50% | **11.0%**（s62 = 16.2%）|

按预登记规则，这条腿**不能用来谈余量**。但这次拿到了机制，而不是第三次"没压上去"：

* 上传发生在 **13:33 UTC 那一分钟**（本地 21:33）：**13899 个 grant（232/s，基线 162/s）**、0.55 Mbit/s，
  **9279 个 grant 占满 25 PRB** ⇒ **手机确实在全缓冲上传**，不是没数据；
* 但 **UL SINR 只有 5.4–5.9 dB**（对照：n78 腿 22–25 dB），于是调度器被钉在最低 MCS：
  **8838 个 grant 的 TBS 只有 96 B（25 PRB + QPSK/MCS0）**，调制分布 QPSK 11466 / 16QAM 2151 / 256QAM 2701；
* ⇒ **限制在 n1 的上行链路预算，不在话务生成器**。同样的手机同样的距离，n78 那条腿能给出 TBS 中位 2880 B。

⇒ **A1-3 的收口方式（如实）**：
1. **"重上行"这个问题在 n1 小區上不可答** —— 该小区产不出重上行（链路预算），继续跑只会重复第三次的结果；
2. **等价问题已由 n78 腿回答**（§5.9.98）：TBS 中位 2880 B、51 PRB、2.7 Mbit/s ⇒ **每跳负载 ×2.4**，
   而车道墙钟占用仍只 **3.7%（余量 96.3%）**、`cbs/lane` 不变；
3. **残留（写清，不掩盖）**：n1 上的**占空比**压力（每秒很多个满 TBS 跳）仍未测 —— 要造它得先抬 n1 的上行链路
   （UE 位置/天线/增益），**不是换流量工具能解决的**。这条与 §5.9.98 ④ 的"可持续 760 跳/s ≈ 时隙的 38%"互为补充：
   速率上限已量化，缺的只是 n1 上的实测占空比样本。

**③ 审计六遍至此全部闭环**

| 遍 | 状态 |
|---|---|
| 1 回归面 / 2 离线门 / 3 反向臂普查 / 4 契约八项定义 / 6 遗留项裁决 | ✅（§5.9.102–104）|
| **5 空中端到端** | ✅ **本节**：硬门全过 + 新仪表在真腿确认 + 池 WARN 负向对照成立 |

**④ 当前开放面（供下一刀）**：`session_handoff_2026-09-23-2.md` §5 的表，加上本轮的
**A1-2 覆盖缺口**（让没有 hop 的 UL 槽也进 block/release）、**RX 池 EMPTY WARN 的正向触发待观测**、
以及 **n1 上行链路预算**（它现在是一条被量到的、挡在"n1 重上行腿"前面的墙）。

#### 5.9.107 ★★★ 更正 §5.9.106 ②：n1 的"上行链路预算差"**不成立**；真机制是**调度器的带宽/阶数取舍**（`#12` LA）

**① 更正什么**：§5.9.106 ② 写"限制在 n1 的上行链路预算，证据是 UL SINR 只有 5.4–5.9 dB（n78 是 22–25 dB）"。
**这个对比混淆了 MCS 构成** —— PUSCH 的 `sinr` 读数**随调制阶数单调上升**，两个小区都一样：

| 腿 | 25 PRB QPSK | 51/25 PRB 16QAM | 64QAM | 5 PRB 256QAM | DL 主调制 | UL 主调制 |
|---|---|---|---|---|---|---|
| n1 s62 / s63 / s65 | 5.3–5.9 dB | 6.3–8.6 | — | **23.8** | 256QAM | **QPSK** |
| n78 s64b | 7.1 / 9.9 | 11.9 / 13.7 | 21.6 / 22.7 | 21.5 | 64QAM | **256QAM** |

⇒ 用"中位 SINR"跨腿比"链路好不好"是**错的**（n1 中位 5.9 是因为它的授权大多是 QPSK，n78 中位 22.2 是因为大多是 256QAM）。
**同调制下**的真实差距只有 **2–5 dB**（n1 25 PRB QPSK 5.3 vs n78 25 PRB QPSK 9.9），**不是 16 dB**。

**② 真正量到的机制（s65，同一腿同一手机）**

| 分钟 | 手机状态 | 授权形态 | 结果 |
|---|---|---|---|
| 13:32 | 缓冲**空**（`SBSR bs=0` 627 条）| **5–8 PRB @ 256QAM**（TBS 560–848 B）| 大多 `crc=OK` |
| 13:33 | **满缓冲上传**（SBSR 只剩 63 条）| **25 PRB @ QPSK MCS0–4**（TBS 96/157/480 B）| 0.55 Mbit/s |

* **手机不是功率受限**：上传那一分钟 `SE_PHR: ph=[13..14)dB` 为主（10–15 dB 余量），空闲分钟 14–18 dB；
* 两组读数的**分布形状**差异极大（n=8755 vs 147）：

| 组 | p10 | 中位 | p90 | max |
|---|---|---|---|---|
| 25 PRB QPSK（全带宽）| 3.1 | **5.3** | 7.2 | 12.9 |
| 5 PRB 256QAM（低段窄带）| 16.8 | **24.2** | 31.2 | 41.9 |

⇒ **满缓冲时调度器选择"宽带宽 + 最低阶"，缓冲空时选择"窄带宽 + 最高阶"**，而窄带那组落在频带低段
（`prb=[0,5)` / `[1,8)` / `[11,16)`）。**这才是 n1 产不出重上行的原因**，它属于**链路自适应/调度**（`#12`），
不属于车道、不属于话务工具、也（已证）不属于手机功率。

**③ 两个仍开的机制 + 判别方式（不猜）**

* **候选 A（信道频率选择性）**：全带宽 25 PRB 的 SINR 确实低 ⇒ 宽带授权必须照顾最差的 PRB，而调度器在满缓冲时
  **宁要带宽不要阶数**，在这条信道上就是错的取舍；
* **候选 B（读数口径）**：宽带估计与窄带估计**不是同一个量**（两组差 19 dB，且 25 PRB 组异常紧：90% 落在 3.1–7.2 dB），
  ⇒ MCS 选择被一个偏悲观的数字驱动；
* **判别（便宜、下一步）**：看 25 PRB QPSK 组的 `iter` 与 BLER —— 若它在"5 dB + MCS0"下仍大量 `iter=1` 且 `crc=OK`，
  那 5.3 dB 就不是这条链路的真实能力（候选 B）；否则候选 A 成立。

**④ 对 A1-3 的最终影响**：结论**不变**（n1 这条腿产不出重上行腿），但**理由换了**——
从"链路预算差 16 dB"（已撤）换成"**满缓冲时被调度在 MCS0 的 25 PRB 上**（`#12` LA 的题目）"。
⇒ "重上行"的等价问题仍由 n78 腿回答（§5.9.98），而 n1 这条**留给 `#12`**：为什么同一条腿里 5 PRB 能到 256QAM、
25 PRB 却只能 QPSK。

**⑤ 一天之内第二个"读数误导"（记账）**：上午是 `dft radio inputs` 的**分母口径**（§5.9.99），
下午是 PUSCH `sinr` 的**跨 MCS 不可比**（本节）。两条都是"看数字前先问这个数字是什么"的同一类错误。

**⑥ 追加（代码级）：日志里那个 `sinr` 到底是什么 —— 它与调度器的输入**不是同一个量**

* **日志的读数**：`PUSCH: … sinr=` 由**解调器**的逐符号统计给出 ——
  `pusch_processor_notifier_adaptor.h:152`：`csi.set_sinr_dB(sinr_type::post_equalization, stats.sinr_dB)`，
  其中 `stats` 是 `demodulation_stats`；同一函数里还有一条 **EVM 换算**分支：`sinr_dB = -20*log10(evm) - 3.7`
  （那个 `-3.7` 是固定的经验偏移）。⇒ 它是**解调侧、逐符号、带经验修正**的量。
* **调度器的输入**：`ue_event_manager.cpp:740` 里 `sinr_dB = convert_power_to_dB(||h||² / nv)` ——
  **信道估计的 Frobenius 范数除以噪声方差**。**这才是外环/链路自适应看的数**，它与日志里那个不是同一个东西。
* ⇒ 两个后果：① §5.9.106 的"n1 的 UL SINR 5.9 dB ⇒ 链路差"用错了量（用的是解调侧统计）；
  ② 要回答"**为什么 n1 把满缓冲的 UE 放在 MCS0**"，必须读**调度器那个** SINR，而不是日志里这个。
* **仍未读全（诚实边界）**：解调器 `demodulation_stats::sinr_dB` 的计算式我还没读（它是否按调制阶数归一，
  正好解释同一腿里 5.3 vs 24.2 dB 的 19 dB 差）；**记为下一步的第一次代码阅读**。
...
#### 5.9.109 ★★★ LA 成对腿（s66 n1 / s67 n78）的判读：**机制 B 被否**、量到**宽度效应**、并剩三个候选（判别只需一次正确的 debug 腿）

**① 先说我的指令错（记账）**：§5.9.108 ③ 让跑的 `--log.phy_level=debug` **开错了 logger** —— `LA UL MCS` 用的是
**`SCHED`**（`ue_link_adaptation_controller.cpp:16`：`fetch_basic_logger("SCHED")`）。两条腿因此各带 7–9 万条 PHY debug、
却**一条 LA 行都没有**（`LA UL MCS=0`）。正确的旗标是 **`--log.sched_level=debug`**（只开它，日志也小得多）。

**② 但这两条腿仍然有产出 —— 它们是"同手机同位置"的成对腿**

| 量 | n1 s66（10245 授权）| n78 s67（5093 授权）|
|---|---|---|
| 主调制组 | 25 PRB QPSK **6.1 dB**（n=7757）、14 PRB QPSK 8.1、16QAM 6.2–8.5 | 51 PRB QPSK **6.7**（n=2074）、51 PRB 16QAM **14.3**、25 PRB 16QAM 14.5、**5 PRB 256QAM 21.4**、51 PRB 64QAM 16.8 |
| **最高的调制** | **只有 QPSK / 16QAM**（无 64/256QAM）| **到 256QAM** |
| 环路初值（最早 8 条）| 3 PRB Msg3，全 `crc=KO iter=6`，`sinr` = inf / **−2.5 / −17.1 / −14.9 / −14.8** … | 3 PRB Msg3 全 KO，`−20.5 / −19.2 / −16.0`，随后 24 PRB QPSK tbs=528 全 KO，`−14.8 … −20.8` |
| PHR（下界众数）| **10–12 dB** | **5–9 dB** |

⇒ **两边的环路初值一样差**（都是负 SINR 的 Msg3）⇒ "初值不同导致两个不动点"这个猜想**不被支持**；
⇒ **两边都不是功率受限**（n1 余量更大，说明它发射功率更低）。

**③ 机制 B（读数随 MCS 缩放）**被同秒同宽度的对照否掉**（这是本轮最干净的一条）

| 腿 | 同一秒、同一宽度 | 各调制的 SINR 范围 |
|---|---|---|
| n78 s67 | 51 PRB，14:01:40 | QPSK 9.0–15.8、16QAM 11.1–24.5、64QAM 14.7–25.9、**256QAM 8.8–21.7** |
| n78 s67 | 51 PRB，14:01:42 | 16QAM 12.7–21.1、64QAM 14.3–19.3、QPSK 13.1–13.9 |
| n1 s66 | 25 PRB，14:00:11 | 16QAM 8.2–12.0、QPSK 5.1–12.6 |

⇒ 各调制**完全重叠**（连 256QAM 8.8 dB 与 QPSK 15.8 dB 都重叠）⇒ **读数不是被调制阶数缩放的信道量**，
§5.9.107 里"中位随调制上升"是**时间分布**造成的（高调制集中在好窗口），**不是**调制本身。

**④ 而"宽度"是**真的**（同秒受控对照，n1 s66 13:58:24）**

| 同秒、同小区、同手机 | 读数 |
|---|---|
| 窄授权 7 PRB / 5 PRB / 2 PRB（QPSK/16QAM）| **9.5 / 11.1 / 13.3 dB** |
| 宽授权 25 PRB（QPSK）| **3.5 / 5.1 / 4.9 dB** |

⇒ **宽度本身就带 6–10 dB**。这与"**部分路损补偿（alpha<1）+ 闭环功控**"的功率谱密度行为一致：
UE 的总功率随分配宽度**次比例**上升 ⇒ 每 PRB 功率随宽度下降（也解释了 n1 那 10–15 dB 的 PHR 余量）。
两份配置**都没有设**上行功控（`p0`/`alpha` 走默认、SIB 里下发），所以这个效应**两小区同源**，
它**不是** n1 与 n78 差 15 dB 的解释。

**⑤ 于是剩三个候选，判据一行就能分开（下次跑对旗标即可）**

| 候选 | 机制 | `LA UL MCS` 行会显示 |
|---|---|---|
| **A 功率谱密度** | 宽分配每 PRB 功率低 ⇒ 实测 SNR 真的低 | `pusch_snr` **随宽度**变化（窄高宽低）|
| **B 底噪/干扰** | n1 的 UL 底噪高（PUCCH 信号强 rsrp −6 dB 却只有 16–25 dB SINR）| 两小区 `pusch_snr` 分布整体错开，且与宽度无关 |
| **C OLLA 欠账** | 早期 Msg3 连 KO ⇒ 外环把偏移**往下压**，之后一路 MCS0 | `olla=` **显著为负**，而 `pusch_snr` 并不低 |

⚠ 候选 C 有直接线索：两条腿的**开头**都是 Msg3 反复 `crc=KO`（`iter=6`）⇒ 外环有理由欠账；
而 `olla=` 正是那行 debug 单独打印的量。**所以下一次跑对旗标，就能一次分开三者。**

```bash
# 只开 SCHED debug（LA 行在这里），日志比上一对小腿小得多
sudo -E bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu s68-la-n1b  --log.sched_level=debug
sudo -E LEG_CONFIG=configs/gnb_rf_b200_tdd_n78_20mhz.yml \
     bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu s69-la-n78b --log.sched_level=debug
```

#### 5.9.110 ★★★ 离线反演（s66/s67）：LA 环路**是灵敏的**，不是自锁；而"高调制只与窄授权配对"

**① 我的旗标错在哪（并已修工具）**：`--log.sched_level` **在本构建里不存在** —— gNB 只注册了
cu/du/fapi/gtpu/hal/lib/mac/ngap/nrppa/ntn/ofh/pdcp/phy/radio/rlc/rohc/rrc/sdap/sec/xnap 这些 level 选项，
`SCHED` 没有自己的选项。另：`gnb.cpp:133` 把 **`lib_level` 设给名为 `"ALL"` 的 logger**（不是 `all_level`）。
⇒ 那次腿的失败是**启动参数被拒**（`exit 109`），不是射频问题。

**工具已修（`run_leg.sh`，两条，都实测过）**：
* **预检**：把**同一套 argv** 交给二进制加 `--dryrun` 跑一遍——它**不碰射频**（本机无 USRP 也能 exit 0，
  实测 0 行提到 UHD/USRP），错选项则 `exit 109` + `The following argument was not expected`。
  预检不过就**拒绝启动**（今后这种错在启动前被拦，不再浪费一条腿）；
* **不再覆盖调用者的 `--log.all_level`**：原来脚本把 `--log.all_level info` 追加在用户参数**之后**，
  等于静默盖掉用户的选择（与脚本自己痛斥的"静默丢弃"同类）。

**② 离线反演：把"上一次读数"与"这一次实际选的调制"配对**（n1 s66 / n78 s67）

| 上次读数 | n1 s66 → 实际调制（宽度中位）| n78 s67 → 实际调制（宽度中位）|
|---|---|---|
| ≤ 6 dB | QPSK 95%（**25 PRB**）| QPSK 95%（**51 PRB**）|
| 6–10 dB | QPSK 93%（25 PRB）| QPSK 86% / 16QAM 14%（51 PRB）|
| 10–15 dB | QPSK 69% / 16QAM 19%（22 PRB）| **16QAM 64%**（51 PRB）|
| 15–20 dB | **64QAM 40% / 256QAM 26%**（11 PRB）| 16QAM 67% / **64QAM 26%**（51 PRB）|
| ≥ 20 dB | **256QAM 53%**（**5 PRB**）| **256QAM 66%**（**5 PRB**）|

⇒ **两条结论（都在两个小区上同时成立）**：
1. **环路灵敏**：MCS 随上一次读数**单调上升** ⇒ 候选 C（OLLA 自锁/欠账）**不是主因**，
   候选 B（读数被调制缩放）已在 §5.9.109 被否 ⇒ **剩下的就是"读数本身"**（候选 A 一族）；
2. **高调制只与窄授权配对**（两小区的宽度中位都从 25/51 PRB 一路降到 **5 PRB**）——这是**调度器策略**，
   不是缺陷；于是"n1 上传慢"与"n78 上传快"的差别**不在策略**，而在**同宽度下的读数**：
   **宽度 51/25 PRB 时 n78 读 11–24 dB，n1 只读 4–6 dB**；而窄授权两边都不差（n1 9–13 / n78 21 dB）。

⇒ **修正后的结论**：n1 的问题**特定于"宽带授权"**（25 PRB 铺满 5 MHz 时读数掉到 4–6 dB，窄授权则不），
而两小区的 LA 行为**逐项一致**。最可能的两条机制：**(i) n1 上行频段（1962.5 MHz，band 1 UL 是真 cellular 上行谱）
有外部干扰或前端凹陷，窄授权能躲开、宽带授权躲不开**；**(ii) 纯功率谱密度** —— 但 §5.9.109 ④ 的宽度效应
（同秒 7/5/2 PRB 9.5–13.3 dB vs 25 PRB 3.5–5.1 dB）只有 6–10 dB，**不足以解释 n1 与 n78 在 51 PRB 上的 10–18 dB 差**。

**③ 登记的下一步判别（两条，都很便宜）**
1. **换 n1 的上行频率**（改 `cell_cfg.dl_arfcn` 到另一个 band-1 频点）跑一条腿 ⇒ 若宽带授权的读数跳起来，
   **外部干扰**成立（前端凹陷不会因频点而变好）；
2. 或**空载腿**（gNB 开着、手机不发）读上行底噪 —— 需要先确认 `baseband metrics` 这项契约检查
   （"N symbols measured … metrics disabled"）能否给出底噪读数，**这一步要先读代码**，记为待办。
...
#### 5.9.115 ★★ C：§5.9.103 表里 13 处"未核实"的**逐条裁决**

| 行 | 项 | 裁决 | 依据 |
|---|---|---|---|
| L5443 | `burst dispatches` 的 `channel_estimator=0` 缺口 | **已闭合** | 当前腿：`dispatches=142575 (equalizer=114060 demapper=9505 channel_estimator=19010)` |
| L5449 | `evicted` 计数语义 | **已闭合（在用）** | §5.9.98 用 `handed/taken/evicted` 解释了 n78 上"车道只认领有 hop 的槽" |
| L5450 | `--self-test` 门的确切命令 | **关闭（不适用）** | 本构建没有该二进制（`find build -name "*self*"` 无结果）|
| L873 | `end_stage(..., WEIGHTS_STAGE)` 一次腿 16 次 | **很可能已解释** | `lib/` 全域 `end_stage(` 的**静态调用点正好 16 个**；**未按调用点打点坐实**（坐实要做的是运行时按点计数）|
| L2163 | "机制上说得通但尚未单变量隔离"的重传代价 | **已裁定关闭** | 该节末尾**用户裁定**："保留 `merged`，不回退"（2026-09-20）|
| L2406 | "每槽等待 ≈ 998 µs"未解释 | **已被取代** | §5.9.6x 的 `t2f` 分析：等的是**最后一个样点**，"前端提前做完"这个前提本身不成立 |
| L9681 | "该 PUSCH 失败 + 显式错误，待定" | **已闭合** | 同段的判据三条（数据面逐字节 + 契约 + 0 RF failure）**都过**；聚合 CRC 差已判为**流量结构**差 |
| L10797 | 前端栅栏 / "发布路径尚未接" | **已退役 / 已取代** | §5.9.65/§5.9.99：交棒成默认后**前端栅栏机制退役**（所有空中腿 signals=0 waits=0）；设备侧数值**已是生产路径** |
| L11144 | 离线代价 +32→+22 µs 的余账 | **已被取代（量纲已换）** | 代价现由 §5.9.92/§5.9.94 的**空中 A/B** 以 `cbs/lane` 与 residency 计量，不再用那次离线数 |
| L298 | 读侧 2.37 的构成"空中尚未逐点确认" | **已被取代** | 契约的 `host device data crossings` = **0.00 读 + 0.00 写每跳**，且四名 reporter（demapper/dft/equalizer/channel_estimator）在案 |
| L5437 | `not_found` 的小洞（无记录可等时直接读网格）| **仍开** | 当前腿 `not_found=1383 / taken=9505`（≈15%）；**与 §5.9.98 的机制同源**（车道只认领 hop），应一并看 |
| L9930 | §8.2 的"发散原因尚未定位"（设备 vs 宿主 LSE）| **仍开（低优先）** | 需要一次设备/宿主 LSE 的**逐点对照**（§8.2 记着代码形状，`OCUDU_CE_LS_CHECK` 仍在）；相关路径后来经过多次重构，先重测再谈 |

⇒ **C 的结论：8 处已闭合/已取代/已裁定、2 处仍开（`not_found` ≈15%、§8.2 发散）、1 处很可能已解释（静态 16 个调用点）、2 处关闭/不适用。**
"未核实"这个类别**清空了** —— 剩下的两处是**有读数、有下一步**的开放项，不再是"没人看过的状态句"。

#### 5.9.116 ★★ D（`#10` 下行方向同口径记账）：**盘点结论 = 下行侧是空的，这是一项工程而不是一次切片**

**① 盘点（证据）**
* **下行没有任何 `[dl_*]` 报告块**（`grep '\[dl' lib/` 只命中调度器的变量名，不是报告行）；
* **下行没有任何契约检查**（`register_phy_pipeline_check` 在 `lib/phy/lower/processors/downlink/` 下**零命中**）；
* 对比上行：契约 8 项 + `[ul_host]` + `[ul_pipeline]` + `[ul_gpu_pipeline]` + `[ul_rx]` + `[ul_rx_pool]` + `[metal_stats]` + `[mmse_time_sum]` + `[ul_gpu_lane]` + `[ul_by_size]` 等；
* 且**下行不在融合车道上**（启动横幅自报：`[lower_phy] DFT backend: rx=metal (GPU) tx=cpu`）⇒ "同口径"在目前的下行上**没有对应物**：
  上行那套账（每跳穿越 0.00、`cbs/lane`、residency）是**车道**的账，而下行还没有车道。

**② 因此 D 的正确形态（建议，不假装已做）**
1. **第一步（可测、最小）**：给下行加**穿越记账**——镜像上行的 `phy_pipeline_crossings.declare_reporter()`：
   下行 DFT 的输入是**宿主产生**的样本（`tx=cpu`），把它每槽的 host→device 字节数记下来，
   并加一条契约项 "dl host samples"（判据：0 拷贝，或明确登记允许值）。这一步**可单测**（现有 DFT/OFDM 测试就可驱动）；
2. **第二步**：给下行加一个与 `[ul_pipeline]` 同形的**延迟序列**（每槽 span），否则"下行有没有同样的问题"永远只能靠猜；
3. **第三步**：只有当**下行也上车道**（`tx=metal` + 一次提交）时，"`cbs/lane` 同口径"才有意义 —— 那是**另一个工程**（D1 的下行版），
   在它之前把 ①② 做好，正好是它的可行性证据。
⇒ **本轮不做**（按"不许半成品"的纪律）：它是一个新子系统，草率落地就是回归风险。**登记为下一项独立工作，前置 = 本节的盘点。**

#### 5.9.117 ★★ C 的最后一条：`not_found`（≈15%）**不是功能缺口** —— 它有一个另行计数的"不可信"分支，而那条恒为 0

**① 它是什么（代码）**：`shared_burst::claim_grid_production()`（`ocudu_metal_burst.mm:704-720`）按 `(grid 存储, slot)`
查交棒记录，**查不到**就 `++grid_not_found; return 0;`。代码注释把两种情况写明：
"要么这个 (存储, slot) 从未交棒过，要么它**早已产出并被 evict** —— 计数是因为**等不了**的迟到读者正是这个 key 存在的理由"。

**② 消费者拿到 0 之后做什么（同样有定义）**：`ensure_grid_produced()`（宿主读者）在 `generation == 0` 时
**直接 `return true`**（没有待完成的生产可等）；而**真正无法确认**的网格走**另一条路**：
`grid_ready_wait()` 超时（200 ms）⇒ `++ready_timeouts` 并 **`return false`**，
注释原文："*the caller then knows the grid is NOT trustworthy*"（调用方由此知道网格**不可信**）。

**③ 数据（五条腿）**

| 腿 | `late` | `not_found` | **`ready_timeouts`** |
|---|---|---|---|
| s62 / s63 / s64b / s65 / s67 | 2497 / 2684 / 1554 / 3839 / 1382 | 1477 / 1548 / **1554** / 2643 / **1383** | **0 / 0 / 0 / 0 / 0** |

⇒ **`not_found ≈ late`**（两条腿逐个相等）⇒ 它是"迟到/无记录"的**预期**读数，与 §5.9.98 的机制同源
（前端每槽交棒、车道只认领有 hop 的槽、其余被 evict）；而**潜在的那条**（网格不可信）是**另一个计数器**，
**五条腿全为 0**。

**④ 裁决**：§5.9.54 的第 2 项"`not_found` 的小洞（无记录可等时直接读网格）"**关闭** ——
它不是"直接读一个没人保证的网格"：无记录 ⇒ 没有待完成的生产；**不可信**的情形被单独计数并向上返回 false，
而那条路从未发生。**残留（写明）**：`not_found` 里"从未交棒"那一支的排序依赖前端自己的普通提交路径
（该路径的排序由前端栅栏/提取环节提供），本节只证明**它没有走到"不可信"那条分支**，没有单独测那一支的时序。

**③ 更正的盘点（本轮补查，它改了第一步的形态）**：下行的网格走的是**普通 `resource_grid_reader`**
（`ofdm_modulator_impl.cpp` 里**没有** `device_view` / `device_resource_grid` 路径），样本进的是**宿主 baseband 网关缓冲**。
⇒ **今天下行没有任何设备侧工作**，因此"给下行加穿越记账"会得到一个**恒为 0、没有意义的数** ——
上行的那个数是拿来"往 0 赶"的，下行连被赶的东西都不存在。
⇒ **第一步应改为**：给下行加一条与 `[ul_pipeline]` 同形的**延迟序列**（先定"从什么到什么"两个边界与判据 ——
...
**⑦ 判据脚本化（与 §5.9.101(1b) 的 `leg_gate.sh` 同一姿势）+ 它自己的三条臂**

`wip/a12_attribution_gate.sh`：把 C1–C5 机械求值，**"读不出"按红算**（§5.9.97 的教训）。它自己带三条臂自检
（`--self-test`：第三分支成立 ⇒ 5/5 绿；`plain_without_lane_slot` 很小 ⇒ C3 红；旧格式无此仪表 ⇒ C1–C3 红），
所以"它能红"是被证明过的，不是被假定的。

**并且它已经对两条既有 n78 腿把 C4 读到"恰好相等"**（这一步不需要新腿，它用的是 `[metal_stats] dft transforms`
那一列 —— 与"普通路由"是同一个总体，见 `ocudu_dft_metal_engine.mm:112/674`）：

| 腿 | T（由 `host sample assembly` 的符号数折算）| occasions = T/10 ms | 12×occ+1 | 读到 | 隐含周期 |
|---|---|---|---|---|---|
| s64b | 132.4 s | 13237 | **158845** | **158845** | 10.00 ms |
| s67 | 92.8 s | 9280 | **111361** | **111361** | 10.00 ms |
...
#### 5.9.120 ★★★ 里程碑全面审计（8 轴）：**目标达成（有界）**；但 `8/8` 必须连**名字**读、门必须从**正则**改成**标签**，且四条断言没有有效腿支撑

> 触发：用户问"我们是否在一个大 milestone 点上"。答案：**是**，但项目自己的教训（坑 #28、§9.2 撤回里程碑）
> 禁止"看到 `8/8` 就宣布"。所以本节做的是**逐项验判据**，不是复述结论。
> 全部读数在树 `29686076d2`（腿 `s69-a12-n78` 的二进制是 `ec8c3ebdd7`，两者之间只差文档/脚本/注释）。
> 交付物：**`wip/milestone_audit.sh`**（一条命令复跑下面全部机械判据，"读不出按红算"）+ 本节。

**① 审计范围（8 轴）与结果一览**

| 轴 | 方法 | 结果 |
|---|---|---|
| **A1/A5 契约八项 + 反向臂** | 逐条读判决表达式/计数点/`nullopt` 条件/臂的存在性 | **名字 8/8 齐、1:1 对应**；但六条可 `nullopt`、两条**永远适用且零分子也算过**、一条**既无臂也无配方**；`N` 今天是**可缩的** |
| **A2 目标属性重测** | 把 `00_goal_and_gap.md` §3.2 的旧差距表逐行映射到**今天的**计数器 + 最新腿 | 旧差距**逐行消失**；但目标里"**恰好两次穿越**"这半句**没有任何计数器**（见 ④） |
| **A3 离线门本身** | `ctest -N` 全量 vs 过滤器；跑 `-L phy` | **门既是瞎的又是宽的**：`-R` 大小写敏感（466 漏）、`o_du` 是子串陷阱（19 条噪声）；项目自己的 `-L phy` **175/175 全过（33 s）** |
| **A4 腿证据台账** | 129 条腿上跑 `leg_gate`，逐条读 provenance/commit/计数器 | 132 条可读；**没有一条是重上行腿**；"0 RF 实时失败"**在 6 条最近默认腿里有 4 条不成立**且**无判据** |
| **A6 文档/代码一致** | 抽查承重断言与行号键 | 4 处陈旧（§3.2 表、§20.6 的"7/8"、translator 注释、目标文档引用的已删拒绝串）——**今天全部更正** |
| **A7 构建/可运行/回滚** | dryrun × 3 模式、冲突规则、戳记、tag | `gpu` **仅靠配置即可选**（无拒绝）；冲突规则有单测；`--dryrun` **不跑配置校验器**（注释已更正）；**HEAD 上没有里程碑 tag** |
| **A8 性能代价账** | 同小区对照 §5.9.54 认下的数字 | n1 重腿 median **1820**（milestone 1844）、p95 **5214**（5423）⇒ **不劣化**；`cbs/lane` 1.00 → **2.00** 是已登记的代价 |
| **机械门全跑** | `wip/milestone_audit.sh` | **23 PASS / 0 FAIL / 0 RED / 1 INFO ⇒ `offline acceptance: GREEN`**（24 条判据，含"边缘块偶发"的复跑记录 `attempts=2`）|

**② 离线机械门（判据 + 读数，全部在 `29686076d2`）**

| 判据 | 读数 | 判定 |
|---|---|---|
| `value_net.py`（**门**）| `captures=47 problems=0`；`--self-test` **8/8**（能看见 P0 的 NaN 与 P5 的 5 个数量级）| ✅ |
| `neutral_vs_baseline.sh`（**信息，非判据**）| `25 捕获 / 131 字节`（全在 `_ce.txt` + 2 个 `_h.bin`）| ℹ 见 ⑦ |
| `ab_dumps.sh` 臂②/臂③ | 两条臂 **0 差异** | ✅ |
| §6.4 跨跳稳定（repeat 1 vs 20）| `h=0 llr=0` | ✅ |
| L1a 收棒臂 / L1b 跳臂 | **5 PASS** / **4× differing=0** | ✅ |
| 边缘块守卫双臂 | 默认 **6/6 绿**、`OCUDU_CE_CORR_FENCED=0` **6/6 红** | ✅ |
| MMSE 雷的速率（10 sweeps 冒烟）| `failing_sweeps=0/10 outliers=0 worst_drift=1.054` | ✅ |
| **`ctest -L phy`**（项目自己的标签门）| **100% passed out of 175**（1 条 x86-only 禁用），33 s | ✅ |
| 三条补齐（`phy` 标签漏标的 PHY 测试）| 3/3 | ✅ |
| `ul_pipeline_probe_test` / `lower_phy_test` / MMSE 单测 | 6/6 / 528/528 / All tests PASSED | ✅ |
| A1-2 归属门（s69）| **5 of 5** | ✅ |
| `leg_gate.sh` 9 条（s69/s67/s65）| **7/9** —— 两条红的都是**有效性**（UL 0.73/0.28 Mbit/s、占槽 2.2/11.0%）| ⚠ 见 ⑥ |

**③ A1/A5：`contract MET (8 of 8)` 到底证明了什么（逐项）**

* **名字对得上**：`phy_pipeline_checks()` 里正好 8 条，8 个字符串与打印的 8 个名字 1:1（**打印顺序不是契约**，按名字核）。
* **六条可以是 `nullopt`**（不适用、不进分母）：`zero-copy wraps`（空缓存）、`ce device estimates`（两侧都 0）、
  `dft radio inputs`（无变换 / 未发布 / cpu）、`radio sample continuity`（`blocks < 2`）、`host sample assembly`（未发布 / 无符号）、
  `host device data crossings`（未发布 / `hops == 0` / 非 gpu）。
...
* **第 5 条的申报范围**：打印出来的申报模块正好 4 个（`demapper, dft, equalizer, channel_estimator`）；
  `lib/phy/lower/`（含 OFDM 解调器的宿主网格回退）与 PUSCH 解调器自己**不在**其中，分母 `hops` **只来自一个模块**。
* **`N` 不是常数，而且可缩是实测的**：cpu 模式 dryrun 实测打印 `contract MET (0 of 4 checks applicable)`；
  历史腿里有 `MET 7/7`（6 条）、`7/8`（4 条 cpu_gpu）、`5/7`（20 条 cpu）、`0/4`（3 条启动被拒）；
  且 `ENABLE_METAL_STATS=OFF`（Apple 默认 configure）时**只有第 5 条**会登记 ⇒ `MET (1 of 1)`。
  ⇒ **宣布里程碑时必须把 8 个名字与第 5 条打印的申报模块一起引用**（坑 #28 的判读三件事）。
* §3.2 那张"逐项判读规则表"**在本节之前已经陈旧**（第 2 行的通过规则还是 §5.9.99 之前的旧式、若干行号漂移）——**今天已在该表下插入更正**。

**④ A2：目标属性的两半 —— 一半有读数，另一半**没有计数器****

旧差距表（`00_goal_and_gap.md` §3.2）逐行在最新腿上都**已经消失**：3 次标量回读 → `0 host read(s)` 且**分项表为空**；
1 组参数写回 → `0 host write(s)`；宿主建相关矩阵 → `device_corr_builds=12765`、`fb_blocks=0`、无写站点；
每跳 `waitUntilCompleted` → 估计器的等待**没了**（`mmse_ce commits=7807 waits=2`），剩下的那一次是
**车道 burst 的等待**（`burst commits=7805 waits=7805`）——**s47 也是每跳一次**（14525/14525）⇒ 不是漂移。

但目标原话是"**只有两次**穿越（IQ 上传、LLR 下载）"——**这"两次"本身没有任何计数器**：

| 目标项 | 今天怎么被量到 | 缺口 |
|---|---|---|
| IQ 上传 | 只有 `radio_inputs`（**变换**数）与 `wrap_copies=0`（无副本）| **没有**"几次/几字节"；而且它不是"每跳 1 次"：`368200 = handed 26300 × 14` 个**槽**上传 / 7805 跳 = **3.37 槽/跳**（"1/跳"是拿槽的事件去除以跳的分母）|
| LLR 下载 | `demapper=7805` 与 7805 跳**相等** + 两条暂存回退从未触发 ⇒ 1.00/跳**是推断** | 没有计数、没有字节数 |
| 两次之间的"零" | `0.00 read + 0.00 write / 跳`（4 个申报模块自审）| 未申报模块（LDPC、PUSCH 的 LLR 交接、lower PHY）**不在**这句话的范围内 |

⇒ **判读规矩**：可以说"**可测的那一半成立**（除允许的两次外，宿主对设备数据零读零写，且 4 个模块自审为空表）"，
**不可以**说"两次穿越被证明了"。另有 9 项属性只能靠读代码（相关矩阵的设备比值门、K4 的设备 CFO、
后 LLR 的宿主 `memcpy`、IQ 走的是引擎**私有** wrap 缓存而不在 `zero-copy wraps` 覆盖内等）。

**⑤ A3：门本身 —— 本次审计最重要的发现（比任何被测对象都重要）**

* **`ctest -R` 大小写敏感**：现过滤器（157）**大小写不敏感时是 466** —— 漏掉的是 `CsiReportPuschHelpersTest`、
  `PxschChain`、`LowerPhy*` 这一整类。这就是 §5.9.114"门是瞎的"的**同族**问题，只是换了一层。
* **`o_du` 是子串陷阱**：157 里有 **19 条**是 `sent_to_du` / `no_dus` / `zer`**`o_du`**`ration_is_no_op` 撞进来的噪声
  ⇒ 现门**同时**是瞎的（漏 309）和宽的（19 条噪声），真 PHY 面约 138。
* **项目自己的门是标签**：`ctest -L phy` = **176 条标签 / 175 条可跑**，本次实测 **175/175 全过、33 秒**。
  它覆盖了正则门漏掉的全部关键缺口：`phy_metrics_deferred_chain_test`（装饰器必须转发
  `submit()/wait()/supports_deferred_chain()`，"否则链会**静默退化成逐符号同步路径**"）、
  `resource_grid_device_view_test`（**整条零拷贝链的前提**）、`pxsch_*`、`ofdm_*_unittest`、`dft_processor_test`、
  `ldpc_chain_test`、`prach/pdxch_processor_test`、以及**同一个二进制里另外 17 条没被选中的 `UplinkProcessorFixture` 臂**。
* **三条 PHY 测试的标签不是 `phy`**（`du_low_phy_pipeline_test` = `apps;o_du_low`、
  `baseband_gateway_buffer_metal_smoke_test` = `gateways`、`pusch_processor_benchmark` = 无标签）⇒
  建议：给这三条各加一行 `PROPERTIES LABELS "phy"`，此后**裸 `-L phy` == 179**，里程碑门就一句话。
* **`lib/phy/metal/`（共享队列 / 共享命令缓冲的 burst / 车道探针）至今没有任何测试目标**（无 `add_test`、无 `test/`）。
* **30 个"构建了但从未注册"的二进制**（PHY 目录里 10 个）；其中 `dft_release_adopt_test` 是已被注册的 ObjC++ 版的**孪生**，
  `eq_handoff_probe` 自称"deferred-group stage hand-off 的最小复现"——正是要防的缺陷类。

**⑥ A4：腿证据台账（132 条可读腿）**

* **没有一条腿是"重上行"腿**：V1（≥2.0 Mbit/s）过 4 条（最好 3.8），V2（占槽 ≥50%）**0 条**（最好 25.9%）⇒ **交集为空**。
  名字里带 heavy 的三条实测 `0.36 / 0.28 / 1.24 Mbit/s`、`17.2% / 11.0% / 2.8%` 占槽，**等于或低于空载基线**（0.39 / 16.2%）。
  ⇒ 任何关于**余量、负载下的车道占用、饱和率、负载下 cbs/lane** 的说法**都没有有效腿**；硬门（契约/穿越/stale/gaps/cbs）与结构计数不受影响。
* **三条"可读"腿其实什么都没建立**：`s55-racefix_2314/_2315`、`s68-la-n1b` 是**启动被拒**
  （错选项），打印 `contract (mode=cpu)` / `MET (0 of 4)` ⇒ "有 contract 行"是必要的、**不充分的**。
* **provenance 只有 8 条腿宽**（`s63` 起）：其余 124 条的 `OCUDU_*` 状态**无法从腿文件判定**
  ⇒ 引用它们为"默认路径"是**假定**。
* **集合内两条反例**：`s64-heavywide_1455` = `NOT MET 1/8`（旧判据的红，§5.9.99 已解释）；
  `s66-la-n1` = `stale=1`（**不在** handoff §1 那"最近五条"里）。
* **`0 RF 实时失败` 站不住**：`s47=0`、`s67=0`，但 `s62=2 / s63=4 / s64b=21 / s65=8 / s66=40 / s69=33`
  （行文是 `[RF] [W] Real-time failure in RF: underflow|late`）；**`leg_gate` 里没有这条判据**，
  33 次 RF 告警的腿照样 7/9。而 `gaps=0` 是 131/132 ⇒ 两者**量的不是同一件事**（一个是 RF 线程的截止期，一个是样点连续性）。
...
| 位置 | 陈旧内容 | 处理 |
|---|---|---|
| `§3.2` 判读规则表 | 第 2 行通过规则仍是 §5.9.99 之前的 `radio*100 ≥ transforms*99`；多处行号漂移 | **已在该表下插入更正块** |
| `§20.6`（文件最后一节）| "契约现在在空口上是 **7/8**"（那是 P2 之前）| **已就地更正**（今天 8/8，7 条腿）|
| `du_low_config_translator.cpp:66` | "the mode itself is still refused by the validator ... (the fused lane is not implemented yet)" | **已改注释**（该占位拒绝已删除）|
| `00_goal_and_gap.md` §5 | 仍引用已删除的拒绝串（全库 0 命中）| **已加指针** |
| handoff `session_handoff_2026-09-23-3.md` | 引用 **§5.9.112 / §5.9.113 / §5.9.116③**，而设计文档里**没有这三节**（那两个提交只改了代码与测试）| **不改 handoff**（快照），在此登记：那三处的内容实际在 §5.9.114 与提交信息里 |
| 更正表用**绝对行号**作键（§5.9.103/§5.9.115）| 追加式插入会让键整体漂移（本节的插入亦如此）；且其中 4 个键在**提交态**就是死键：§20.6 那句在 L12589（不是 L11901）、§8.2 的在 L10618（不是 L9930）、§17.6 的在 L11485（不是 L10797）| 规矩：**按句子检索**，行号只作参考（本文件自身在"编辑坑"⑦ 里已记过同类错误）|
| handoff §8「关键坐标」| **2 处错误 + 6 处漂移**：`ocudu_dft_metal_engine.mm:270`（dft radio inputs）在它自己的提交上就已经是 L288、今天判决在 **L302**；`set_lane_slot` 在 1399 → 今天 **1446**（它自己的提交上是 1432）；证据行 267→**281**、`submit_slot_grid_write` 1311→**1325**、`claim_grid_production` 704→**707**、`uplink_processor_impl.cpp` 216→**219**（metrics）/ 249→**248**（assembly）、`phy_pipeline_crossings.h` 512→**514**（判决）| **不改 handoff**（快照）：本节即更正页。其余项（`ocudu_metal_queue.mm:191`、`pusch_demodulator_impl.cpp:819`、`lower_phy_baseband_processor.cpp:217`、`not_found 714`、LA `148`/`163`、`{0,28}`、池结构 39/WARN 139）**核对为正确** |
| §3 代码地图 + §4.1 行号（`:433/:1192/:1382/:2898/:3356/:3475/:3492`、`:829/:862`）| 逐条核对**全部错位**（真值：571 / 1648-1650 / 1900-1902 / 3742 / 4789 / 4987&5012 / 5012；解调器 852/885）| 导航表，不是证据 ⇒ 登记待重写 |
| §7 坑 #11 的"正解"式子 | 第 4 个谓词已加一项：`host_grid_pending = !host_grid_wanted \|\| !publish_all_grid`，`host_grid_wanted = host_grid_published() \|\| !device_reports`（`:4994/:5012`）| 登记待改 |
| §5.9.84 的 `644 vs 715` 判据 | 已在 §5.9.86 撤回，但 §5.9.85 仍把它当证据引用 | 登记待加指针；同族：§5.9.87/88 的"默认仍未翻"（§5.9.93 已翻）、§5.9.106 ② 的 n1 链路预算（§5.9.107 已撤）|
| §5.9.102 第 2 遍的 `ctest（36 项）36/36` | 36 就是 §5.9.114 证明**瞎**的那张旧过滤器，而这一行没有指针 | 登记待加指针（`-L phy` 才是门）|
| §5.9.100 ④ 的"无法用单测演示" | **过度断言**：池下限 8 是**调用点策略**（`lower_phy_factory.cpp:182`），处理器从公开字段建池、无断言 ⇒ 小池夹具**做得到**；且它自己预登记的"到 0 而没打 WARN 即为缺陷"**已经命中**（s63）| 已在 §5.9.100 ④ 就地加更正与裁决办法 |
| handoff 引用 `§5.9.112 / §5.9.113` | 设计文档里**没有这两节**（那两个提交只改代码与测试）⇒ 两条反向臂与"门漏 121"的记录只在提交信息里 | 建议：把内容并进 §5.9.99/§5.9.114 或补写两节 |

**⑨ 数值网/字节网的归属（回答了"131 字节是不是回归"）**

`value_net` **47/0** 是判据；逐字节网 **131 字节 / 25 捕获**是信息。归属实验（本次新做）：
**回滚臂 `OCUDU_CE_CORR_FENCED=0` 的字节差与默认臂完全相同（131/25）** ⇒ 这批漂移**不是**"雷的修复"的指纹；
而文档里早已登记过同一读数（§S18：*"differing-bytes=131，与 S15/S17 同数 ⇒ 235 个 dump 里没有一个字节因为本步而动"*）
⇒ **已知、稳定、有界**（且两条 `_h.bin` 只差 14 字节、LLR 判决级判据在 `value_net` 里全过）。§6.3 的"期望 0"已更正为"信息"。

**⑩ 里程碑判定：达成（有界）—— 宣布时必须带上边界**

**成立**（默认路径、离线门全绿、空口有读数）：`gpu` 融合车道；每跳**额外**数据穿越 `0.00 读 + 0.00 写`（4 模块自审为空表）；
契约 **8/8**（名字齐）；`stale=0`；`gaps=0`（131/132 腿）；`cbs/lane=2.00 (max=2) dropped=0`；`value_net 47/0`；`-L phy 175/175`。
**不成立 / 未建立**（宣布时**必须**排除）：
1. 目标里"**恰好两次**穿越"（无计数器；IQ 是 3.37 槽/跳，LLR 是推断）；
2. **任何负载结论**（无一条重上行腿；余量/占用/饱和率全部无有效腿）；
3. `cbs/lane=1.00`（D1 的旧里程碑数，已被 §5.9.91 ① 登记的代价改成 2.00）；
4. **`0 RF 实时失败`**（6 条最近默认腿里 4 条不成立，且无判据）；
5. 边缘块守卫的空口证据（**0 条腿**，只有未归档的离线臂）；
6. A1-2 的**第二条腿**（单腿 + 独立恒等式）；
7. keepalive 归还不变量（6/8 条差 14/28，无门）；
8. `host sample assembly` 的**可证伪性**（八条里唯一无臂无配方）。

**⇒ 建议的 milestone tag 注记（照 `gpu_phy_iq2llr_zero_data_crossings_p2` 的格式）**：写清**腿名 + 二进制戳 + 名字清单 + 申报模块 + 上面 8 条排除**，
并注明"本 tag 不声称负载行为与 RF 定时"。

**⑪ 未闭合项（按"每投入一分钟能拿回多少可信度"排序，全部附配方）**
...
| ~~8~~ | ~~`-L phy` 里重活的稳定性~~ **✅ 本轮登记** | 全套 179 项 **21.7 s** 跑完、无异常（`pxsch_bler_test` 0.64 s、两条 `radio_ssb_zmq_*` 在内）；尚无抖动读数——登记为"要重活时先看这条" | 离线 |
| ~~9~~ | ~~每条腿的 `Real-time failure in RF` 计数入库~~ **✅ 本轮完成** | `leg_gate.sh` 现在在 9 条判据之后打一行 **INFO**（s69 → 33、s67 → 0）；**故意不做成第 10 条判据**（阈值未登记，不能在门里现编）| 1 行脚本 |
| ~~10~~ | ~~§5.9.54 ① 那张 milestone 表本身~~ **✅ 本轮完成** | 已在表下就地标注：`cbs/lane` 1.00→**2.00**（已登记的代价）、`RF 失败=0` **只对 s47/s67 成立**且**无判据**、其余各行仍成立但须连"该腿是否自证无旋钮"一起读 | 文档 |

#### 5.9.121 ★★★ 两条新腿：**A1-2 复现**（s71）+ **第一条负载腿**（s70）——负载证据到手，同时量到**一道实时墙**

**① 时间线（先排除并行污染）**：本轮最后的离线 GPU 作业 23:50:30 结束、重建 23:50:57 结束；
腿 `s70` 起于 **23:55:32**、`s71` 起于 **00:01** ⇒ **没有重叠**，读数不是"并行跑"的产物（§6.3 的警告）。

**② A1-2 第二条腿（`s71-a12-n78b`，二进制 `4c4880012c`）：归属复现，5/5**

| 判据 | 读数 |
|---|---|
| C1 恒等式 | `126997 + 0 = 126997` |
| C2 子集 | `126997 ≤ 126997` |
| C3 预言 | `plain_with_block = 0`，比值 **1.0000** |
| C4 速率 | T=105.8 s ⇒ **10583** occasions ⇒ 期望 **126997**，读到 **126997**（隐含周期 **10.00 ms**）|
| C5 腿有效 | 契约 8/8、`stale=0`、crossings `0.00+0.00`（7963 跳）、gaps=0 |

⇒ 与 s69（17495 occasions）**occasion 数不同、恒等式同形** ⇒ §5.9.119 的归属（普通路由 = PRACH 实例 + 1 次预热）
由**两条独立腿**支撑，不再是单腿结论。附加旁证：s70 的普通路由 `274837 = 12×22903 + 1`（T=229 s）——**第三条**。

**③ 负载腿 `s70-heavy-n78`（229.04 s，n78，无旋钮）：先读它建立了什么**

| 项 | 读数 |
|---|---|
| **契约（在负载下）** | **8/8**；crossings **0.00 读 + 0.00 写 / 跳 over 46668 跳**；`ce device estimates: 513348 device, 0 host`；`host sample assembly: 6412938 of 6412938 就地读, 0 copied`；`zero-copy wraps: 0 failures, 0 misaligned`；`cfo: 0 round trips`；`gaps=0` |
| **车道代价（第一次有合法读数）** | `lanes=46668`、`cbs/lane=2.00 (max=2) dropped=0`；`busy split: ch_wt=36.4 / merged_hop=1152.7 µs`；residency med/p95/p99 = **1325.1 / 1439.5 / 1510.3 µs**；lane period med 1514.2 µs（一跳每 3.0 槽）|
| **余量（几何无关口径）** | 占用 **25.8%** ⇒ **余量 74.2%**；"可服务 **789 跳/s**，实际要求 **203.8 跳/s**"；**burst: residency/period = 87.5%** |
| 负载本身 | 46668 授权（**10.2%** 占槽）、**1.27 Mbit/s**、TBS 中位 **704 B**（≥1000 B 占 21.4%）、DL 20369（0.15 Mbit/s）|
| 延迟 | `[ul_gpu_pipeline]` med **3031.8** / p95 **4200.0** µs；`[ul_pipeline]` med 3076.0；`defer_wait` med 1445.9 µs；CE 均值 33.6 µs |
| RX 池 | `free_min=0`、`starved_takes=16`、`starved_events=11`、`held_max=8/8` ⇒ **DRAINED**，且一次性 WARN **恰好一次**（`leg_gate` 该判据 PASS）|
| CRC | OK/KO = 45632 / **1036**（KO 2.2%）|

**⇒ 三条"从无到有"**：① 车道/契约的那几条断言**第一次有了负载腿**（此前 132 条腿全部是轻上行，见 §5.9.120 ⑥）；
② **余量/占用/突发比**第一次有合法读数（`ul_load.sh` 的三个几何无关口径）；
③ **RX 池的第三种状态 `DRAINED` 与那条 WARN 的正向触发**同时被空口观测到（§5.9.120 ⑦.2 的"待观测"由此彻底关闭）。

**④ 但同一腿上量到三道逆向读数（全部新，且都要单独归因）**

1. **`stale=5`**：`stale=5 (span > 8000 us, the uplink HARQ round trip) mean=8397.4us max_stale=8767.0us`
   ⇒ 5/46668 = **0.011%** 的跳越过 8 ms 阈值（最多越 9.6%）。这是默认路径上**第一次** `stale > 1`（s66 是 1）。
2. **RF 实时失败 351 次**（235 underflow + 116 late），**全部集中在最后约 90 s**：15:56 仅 1 次，
   15:58:0x 起 3+2+36+28+20，15:58:5x **103**，15:59:0x–2x 67+67+24，最后一条在 `Stopping DU...` **前 11 ms**。
   而 `gaps=0` ⇒ **没有丢样点**（RF 线程错过截止期 ≠ 样点缺失，两者量的不是同一件事）。
3. **流量在 15:59:06 崩塌**：授权速率 600/s → **3–20/s**，18 s 后收尾。**全程只有 1 次 PRACH**（15:55:58 建链）
   ⇒ **不是 UE 重接**；结合"负载爬升"（15:55–15:58 约 119 授权/s → 15:58–15:59 约 350/s → 最后数秒 600/s），
   最像的形状是**业务端停止上传**，而 RF 失败在它之前就已随负载上升而出现。

**⑤ 判定更新（§5.9.120 ⑩ 的第 2 条排除项要改口径，不是删掉）**

* **可以说了**：契约的硬性质（8/8、`0.00+0.00`/跳、`ce … 0 host`、就地装配、0 wraps）在**一条 46668 跳的负载腿上同样成立**；
  车道在 203.8 跳/s 的报价下有 **74.2% 余量**、突发比 87.5%。
* **仍然不能说**：这条腿**没有达到预登记的有效性阈值**（1.27 < 2.0 Mbit/s、10.2% < 50% 占槽）⇒
  **不得把它当作"那条重上行腿"引用**，也**不得为了让它过关去改阈值**（阈值是预登记的；要改就得走一次单独的登记，并说明为什么旧阈值不适用）。
  正确写法是"一条**负载爬升**的腿"，并连它的负载剖面一起引用（119 → 350 → 600 授权/s → 崩塌）。
* **新登记（本轮）**：**负载下的实时墙**——RF 失败随负载上升出现、`stale` 同时越过阈值、随后业务崩塌；
  `gaps=0` 说明样点没丢，因此这是**实时调度预算**的问题，不是数据面缺陷。
  它需要自己的 A/B 才能归因（候选：机器热/其它进程、负载本身、GPU 与 RF 的争用），
  **在归因之前不得把"0 RF 失败"写进任何里程碑注记**（§5.9.54 表下已就地标注）。

**⑥ 顺带量到的一条仪表事实：门自己出现过一次不可复现的红**

`2026-09-24 00:08`，里程碑门 `wip/milestone_audit.sh` 的第一次 `value_net` 读数是 **`problems=3`**（该门今天此前每一次都是 47/0）；
紧接着复跑 **四次**（审计脚本自己的一次 + 显式连跑三次，其间 `ps` 确认**没有**任何 `ul_chain_replay`/`gnb` 在跑）读到 **47/0**，
且所有 `h` 相对偏差都是 `0.00e+00` ⇒ **不可复现**。按本文件 §6.3 自己记下的规则，第一件事是查"是不是在并行跑"
（`ul_chain_replay` 在并行实例下会给出错误结果，实测 980 条里 39 条同配置两次不同），本条符合那个形状。
**处置（本轮）**：`milestone_audit.sh` 的门现在**红了先单独复跑一次**，并在读数里印 `attempts=N`
...
#### 5.9.124 ★★★ 负载下"实时墙"的 A/B **预登记**（跑腿之前写死）：两条腿、四个判据、一个负载可比性前置

> 触发：§5.9.121 ④ 在 s70 上量到**唯一一处逆向行为**——RF 实时失败 **351 次**（235 underflow + 116 late）
> 全部落在最后约 90 s，`stale=5`（首次越过 8 ms），随后业务在 15:59:06 崩塌（600 → 3–20 授权/s，**全程仅 1 次 PRACH**
> ⇒ 不是重接），而 `gaps=0` ⇒ **没丢样点**。**目前只有"已登记"，没有"已解释"**，本节把解释它的判据先钉住。

**① 参考臂（s70-heavy-n78，就是被解释的对象）**

| 量 | s70 读数 |
|---|---|
| 时长 / 槽 / 授权 | 229.04 s / 458079 / 46668（10.2% 占槽）|
| 报价剖面（授权/秒，按 10 s 窗）| 前段 ≈119 → 15:58 段 ≈350 → 最后数秒 ≈**600** → **15:59:06 崩塌到 3–20**，18 s 后收尾 |
| RF 失败 | **351** 次 = 235 underflow + 116 late；**98% 落在运行的最后 25% 时间**内；最后一条在 `Stopping DU...` 前 11 ms |
| `stale` | **5**（`mean=8397.4us max_stale=8767.0us`，阈值 8000）|
| 硬门 | 契约 **8/8**、crossings `0.00+0.00` / 46668 跳、`cbs/lane=2.00 (max=2) dropped=0`、`gaps=0` |
| 其它 | CRC OK/KO 45632/1036（KO 2.2%）|

**② 两条腿（单变量、背靠背跑，**先 A-1 后 A-2**）**

```bash
sudo -E LEG_CONFIG=configs/gnb_rf_b200_tdd_n78_20mhz.yml \
  bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu s72-wall-gpu-n78     # A-1：与 s70 同模式
sudo -E LEG_CONFIG=configs/gnb_rf_b200_tdd_n78_20mhz.yml \
  bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh cpu s73-wall-cpu-n78     # A-2：唯一变量 = 管线模式
```
* **负载配方（两条腿尽量一致）**：手机**持续上行 ≥ 4 分钟**，中途不要碰手机；两次用**同一个 iperf3 命令**
  （例如 `iperf3 -c <server> -t 240 -P 4`，或你惯用的上传 App，但两次必须相同）。
* 停：**一次 Ctrl-C**，等收尾块打完（`run_leg.sh` 现在会在缺报告时报错退出）。
* ⚠ **背靠背**跑（中间不休息）是**故意的**：A/B 最怕的是机器热漂移被误读成模式效应；同一时段内跑完，
  漂移对两条腿的影响同向。**若判读落进灰区**，再加第三条 `s74-wall-gpu-n78b`（A-1 的重复）来分开"漂移"与"模式"。

**③ 预登记判据（腿跑完用 `wip/wall_ab.sh` 机械求值；"读不出"按红算）**

| # | 判据 | 阈值 | 依据 |
|---|---|---|---|
| **P0** | **负载可比性（前置）** | 两条腿的**峰值授权率相差 ≤ 2×**；否则 A-2 的对比**无效**（墙与负载相关，不可比就等于没做）| s70 的墙是随负载爬升出现的 |
| **R1** | 硬门 | 契约 8/8（mode=gpu 腿）/ crossings `0.00+0.00` / `gaps=0` / `cbs/lane ≤ 2.00 (max=2) dropped=0` | 与 s70 同形 |
| **R2** | **墙是否复现**（A-1 vs s70）| RF 失败 **≥100 ⇒ 复现**；**≤10 ⇒ 未复现**；11–99 ⇒ **灰区**（需第三条腿）| s70=351；同期轻载腿 s69=33、s71=**0** |
| **R3** | 形状 | **≥80% 的 RF 失败落在运行的最后 25% 时间**内 ⇒ 形状复现 | s70 = 98% |
| **R4** | `stale > 0` | A-1 的 `stale ≥ 1` ⇒ 与 s70 同类；`=0` ⇒ 该腿没有跨过 8 ms 的跳 | s70=5 |
| **R5** | **墙在哪一侧**（A-2 vs A-1）| **A-2 的 RF 失败 ≤ A-1 的 1/5 ⇒ "车道/GPU 侧"**；**≥ 1/2 ⇒ "机器/射频侧"**；之间 ⇒ 灰区 | 单变量 = 管线模式 |

**④ 判读表（先写死，免得事后挑解释）**

| 观测 | 裁决 | 下一步 |
|---|---|---|
| R2 复现 **且** R5 落在"车道/GPU 侧" | 墙与融合车道在同一负载下的主机/GPU 争用相关 | 用 `OCUDU_*` 把车道阶段逐个关掉继续二分（`OCUDU_GPU_*`/`OCUDU_CE_*` 的现有 A/B 旋钮），并把它登记为**里程碑外的新工作项** |
| R2 复现 **且** R5 落在"机器/射频侧" | 墙不在车道（cpu 模式同样撞墙）⇒ 指向射频/USB/机器实时预算 | 转去做**机器侧**归因（USB 帧数、CPU 亲和、热；与车道无关，不阻塞里程碑）|
| R2 **未复现** | s70 的墙**不是负载的可复现性质**（更可能是当时环境/热/客户端）| 记为"单次不可复现事件"，里程碑注记仍不得声称"0 RF 失败"；把 A-1 的读数归档 |
| R2 灰区 | 证据不足 | 跑 `s74-wall-gpu-n78b`（A-1 重复）再判 |
| P0 不成立 | 负载不可比 | **该对比作废**，重跑一条把负载钉住的腿（并说明为什么前一次不可比）|

**⑤ 与里程碑的关系（不因为这条腿而改口径）**：无论结果如何，**"0 RF 失败"仍然不得写进 tag 注记**
（§5.9.54 表下已就地标注）；本节只决定**它是不是一条可复现的、可归因的现象**。
另：**预登记阈值不许为了让某条腿过关而改动**（§5.9.121 ⑤ 同一条规矩）。

#### 5.9.125 ★★★ A-1/A-2 跑完：**墙在"车道/GPU 侧"（预登记 R5 通过）**，但同一构建下**短腿干净、长腿丢样点**——`gaps` 首次为非 0

**① 三条腿（同一构建 `36d43c54c9`，同一 n78 config，手机负载配方相同）**

| 腿 | 模式 | 时长 | 授权（峰值率）| RF 失败 | **gaps** | `stale` | 契约 |
|---|---|---|---|---|---|---|---|
| `s72-wall-gpu-n78_1934`（第一次尝试，短）| gpu | **81 s** | 8243（**600/s**）| **0** | **0** | 0 | **8/8** |
| `s72-wall-gpu-n78_1936`（A-1，长）| gpu | 357 s | 17719（81/s）| **1379**（792u/584l）| **3**（**565994** 样点）| 1 | **NOT MET: 1/8 失败 = 连续性** |
| `s73-wall-cpu-n78`（A-2）| cpu | 518 s | 17161（86/s）| **51**（28u/20l）| **3**（282199 样点）| 0 | **NOT MET: 1/5 失败 = 连续性** |

**② 预登记判据的裁决（`wip/wall_ab.sh` 机械求值，修正了解析器之后）**

| 判据 | 结果 |
|---|---|
| **P0** 负载可比性 | **PASS**（峰值 81.1 vs 85.8 授权/s，差 1.06×）|
| **R1** 硬门 | **两条都 FAIL** —— 但不是"读不出"：`radio sample continuity` **真的红了**（`gaps=3`）。**这是本线历史上第一次连续性检查进入 FAILED** |
| **R2** 墙是否复现 | **PASS**（1379 ≥ 100，且是 s70 的 3.9×）|
| **R3** 形状 | **FAIL**（最后 1/4 内 1032/1379 = **75%**，阈值 80%）⇒ 失败是"后段成簇"但更铺开（跨约 4 分钟，17→108→541→712 每分钟递增）|
| **R4** `stale ≥ 1` | **PASS**（1）|
| **R5** 墙在哪一侧 | **PASS ⇒ 车道/GPU 侧**（同样负载下 cpu 干净 **27×**：51 vs 1379）|

**③ 预登记没料到的形状（必须记下，因为它改变下一步）**

**同一个构建、同一个峰值负载（600 授权/s）：81 秒的短腿 `0 RF 失败 + 0 gaps + 8/8`，357 秒的长腿 `1379 RF 失败 + 3 gaps + 契约 NOT MET`。**
⇒ 墙**不是负载的确定性质**，它需要**时间/累积状态**（热、或某种随运行时间增长的东西）。而 A-1 又紧跟在
19:34 那次短腿之后 2 分钟起跑 ⇒ **A-1 是"热"状态下的运行**。
⇒ **R5 的对照仍然成立**（A-1/A-2 同一构建、负载可比、背靠背，cpu 侧干净 27×），所以"车道/GPU 侧"这个裁决**可以用**；
但 **R2 与 s70 的对照被一个混淆变量污染**：s70 在**合并之前**（`§5.9.123`：1513 个文件），s72/s73 在合并之后
⇒ "失败数从 351 涨到 1379、并且第一次出现丢样点"**不能**归因给负载，**必须再分一刀**。

**④ 新的事实：`gaps=3` 是硬门失败，不是软观察**

`radio sample continuity: 3 gaps over 707880 blocks (565994 samples missing or repeated) -> FAILED`
—— 连续性检查量的正是"电台送来的样点流有没有断"，**此前 131/132 条腿全 0**。它同时出现在 gpu 与 cpu 两条腿上，
所以**丢样点本身不是 GPU 特有的**；而 RF 告警数差 27× 说明**告警与丢样点是两件事**（cpu 腿 51 次告警也丢了 282199 个样点）。
最可能的机制（**登记为假设，未证**）：宿主侧 rx 消费者落后 → UHD underflow/迟到 → 流上出现跳变 → `gaps` 记账。
**这条一旦复现，里程碑的"`gaps=0`"就不能再作为默认路径的性质引用**（§5.9.54 表下已就地标注该表按腿读）。

**⑤ 第二轮预登记（下一步，先写死再跑）**

| 腿 | 配方 | 判据（跑后机械求值）|
|---|---|---|
| **A-3（冷）** | gpu，**机器静置 ≥10 分钟**后的第一条腿，负载同配方 ≥4 min | **冷腿干净**（RF ≤10 且 `gaps=0`）⇒ 墙需要累积状态；**冷腿就撞墙**（RF ≥100 或有 gaps）⇒ 与"热"无关 |
| **A-4（热）** | A-3 结束后**立即**再跑同一条 | A-4 ≫ A-3（≥5×）⇒ 时间/热累积成立；两者同量级 ⇒ 不是累积 |
| **A-5（决定性的那一刀）** | **合并前二进制**：worktree `/Users/jiachengwang/dev/ocudu_premerge`（= `38bfc6ce02`，已构建）跑同一配方 | 若合并前二进制在"热"状态下**不丢样点**（gaps=0）而 HEAD 丢 ⇒ **合并引入**；若同样丢 ⇒ 与合并无关（是更早就存在、只是从未跑够久）|

**⑥ 本轮修掉的两个解析器 bug（都属"读不出被当成没问题"的反面）**：`wall_ab.sh` 与 `leg_gate.sh` 的契约读取
只认 `contract MET (N of M checks applicable)`，遇到真正**失败**的腿印的
...
> 我在 `build/` 下找 `.metallib`，而它们住在**源目录**里；`ocudu_premerge` 的 `gnb` 构建（19:55:43–19:58:42）**早已把它们建好**
> （`ocudu_dft.metallib` 的 mtime 19:55:45 就在那次构建里）。我随后那次"补建内核"其实是 no-op。
> **预检本身保留**（一个**从未构建过 `gnb`** 的新 worktree 确实会缺内核，那时会静默回退），但**这次事故的归因作废**。

#### 5.9.126 ★★★ A-3/A-4 跑完：**"热/累积"假设不被支持**；同构建的离散度跨两个数量级；s76 出现新异常（RF 洪泛 + 收尾挂死）

**① 五条 HEAD gpu 腿 + 一条 cpu 腿（同构建 `36d43c54c9`/`7da6b2d953`，同 n78 config）**

| 腿 | 次序/状态 | 时长 | 授权（峰值）| RF 失败 | gaps | 契约 |
|---|---|---|---|---|---|---|
| `s72…_1934` | 第 1 条（冷）| 81 s | 8243（**600/s**）| **0** | 0 | 8/8 |
| `s72…_1936` | 第 2 条（热）| 357 s | 17719（81/s）| **1379** | **3** | NOT MET（连续性）|
| `s73`（cpu）| 第 3 条（热）| 518 s | 17161（86/s）| **51** | **3** | NOT MET（连续性）|
| `s74`（**冷**：静置后第一条）| 隔日 20:05 | 278 s | ?（84/s）| **166** | **0** | 8/8 |
| `s75`（**热**：紧接 s74）| 20:10 | 290 s | 14008（66/s）| **18** | **0** | 8/8 |
| `s76`（标 premerge，实为 HEAD）| 20:15 | 340 s | 13452（39/s）| **187210** | **0** | 8/8（但收尾不完整）|

**② A-3/A-4 的裁决（照 §5.9.125 ⑤ 预登记，不许事后改口径）**

* **A-3（冷）：RF = 166 ≥ 100 ⇒ 预登记里"冷腿就撞墙"那一支** ⇒ **与热无关**（且 `gaps=0`，形态比 s72 温和）。
* **A-4（热，紧接）：RF = 18**，比冷腿**少 9 倍** ⇒ 预登记的"同量级/不更大" ⇒ **累积假设被反向证伪**。
* R3 参照：s74 的 166 次失败**全部（100%）落在最后 1/4 时间**内 ⇒ "**晚期成簇**"是这个现象里唯一稳定的形状。
* ⇒ **冷/热不是自变量**；而同一构建下 6 条腿的 RF 计数是 `0 / 18 / 51 / 166 / 1379 / 187210` —— **跨两个数量级**，
  与模式（gpu/cpu）、时长、**报价负载**、冷热**都没有单调关系**。**"墙"目前是一条"每次跑都可能不同"的现象**，
  机制仍未定；唯一稳定的读数是**它出现在运行的后段**。

**③ s76：新异常（RF 洪泛 + 收尾挂死），而且它**不是**合并前那条腿**

* stdout 的横幅是 **`commit 7da6b2d953`**（= 主工作树 HEAD），日志也落在**主工作树**的 logs 里
  ⇒ 用户是在 `ocudu` 而不是 `ocudu_premerge` 下执行的 ⇒ **A-5（合并前对照）仍然缺**。
* RF 失败按分钟：`2 / 2 / 6 / 24 / 77081 / 110095` —— **最后两分钟洪泛**，而这条腿的负载很轻（39 授权/s、13452 个）。
* `Could not stop application after 5 seconds. Forcing exit.`（`lib/support/signal_handling.cpp:30`）+ 进程被 `Killed: 9`
  ⇒ **收尾没走完**：`.stderr` 里**有一次契约块（8/8）但没有 `[metal_stats]`** ⇒ **该腿的计数器不完整，不可用作证据**。
  **"应用在 5 秒内没停下来"本身是一条新发现**（有线程没退循环），登记待查。

**④ 本轮又修掉两个"工具缺口"（都由这次的数据暴露）**

1. **R5 只能在"两条腿模式不同"时适用**：冷/热这一对**都是 gpu**，脚本起初照样套用 R5 并"判出"车道侧
   （166 vs 18）——那是**跑间波动，不是侧别**。现在同模式对直接报"不适用"并说明原因。
2. **`run_leg.sh` 的"有报告"判据太弱**：它只要求 `[phy_pipeline] contract` 一行，而 **s76 恰好有那一行**、
   其后的 `[metal_stats]` 全部缺失 ⇒ 旧守卫会把它当可读。现在**同时要求 `[metal_stats]`**，
   并在检测到 `Could not stop application` 时**指名说出**这条腿是被强制退出的（自检：s76 判为不完整、s74 放行）。

**⑤ 下一步（仍是 A-5，命令里的目录是关键）**

```bash
cd /Users/jiachengwang/dev/ocudu_premerge          # ← 必须是这个 worktree（横幅应显示 38bfc6ce02）
sudo -E LEG_CONFIG=configs/gnb_rf_b200_tdd_n78_20mhz.yml \
  bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu s77-wall-premerge-n78
```
判据（§5.9.125 ⑤）：**热状态下**（紧接一条 HEAD 腿之后跑）若合并前二进制**不丢样点、RF 显著更低** ⇒ **合并引入**；
若同量级或同样洪泛 ⇒ **与合并无关**。**可选**：把 s76 的形态（轻负载、约 5 分钟、gpu）**重复一次**，
看"洪泛 + 收尾挂死"是否可复现——那是眼下最不受控、也最值得单独登记的一条。

#### 5.9.127 ★★★ s76/s77 的真身：**下行饱和 → UL 反馈丢失 → 下行 HARQ 超时 → 客户端挂住 → RF `late` 洪泛 + 收尾挂死**

> 触发：用户报告"iperf3（**CN → Phone**，即**下行**）挂住、不自动退出"，并指出**上一次第 3 腿（s76）也是这样**。
> 两者共同点一查就出来了——**这两条腿的负载是下行**，而 §5.9.124–126 追的"墙"全部是**上行**负载腿。
> **这是两个不同的 regime，不能并池。**

**① s76 的逐分钟读数（决定性的一张表）**

| 分钟 | PDSCH 授权 | PUSCH 授权 | RF 失败 | 失败类型 |
|---|---|---|---|---|
| 12:15 | 33376 | 1636 | 2 | 混合（15 late / 19 underflow，风暴前）|
| 12:16 | 83999 | 3603 | 2 | |
| 12:17 | 83993 | 3335 | 7 | |
| 12:18 | 83997 | 3384 | 24 | |
| **12:19** | **39216**（腰斩）| 1494 | **78015** | |
| **12:20** | **0** | **0** | **110385** | **187160 late / 16 underflow（风暴中）** |

* 下行满负载 ≈ **84000 授权/分 ≈ 1400/s ≈ 70% 的槽**；上行只有 ≈3400/分（**轻**）。
* **12:19:28 的触发行**（比"下行完全停摆"早约 1 分钟）：
  ```
  [SCHED] [W] rnti=0x4601 h_id=7: Discarding DL HARQ. Cause: Timeout was reached (256 slots)
              to receive the respective HARQ-ACK indication from lower layers (HARQ-ACK slot=0.9)
  [SCHED] [W] rnti=0x4601: Forcing "NACK" for 1 DL HARQ processes ...
  ```
  ⇒ **UE 的上行反馈（HARQ-ACK/UCI）先断** ⇒ 下行 HARQ 连续超时 ⇒ 下行不再前进 ⇒ **TCP 流卡死 = 用户看到的 iperf3 挂住** ⇒
  下行停止调度（12:20 `PDSCH=0`）⇒ **RF 每槽报一次 `late`**（110385/60 ≈ 1840/s ≈ 30 kHz 的每槽）直到进程被杀。
* **失败类型把侧别钉死了**：风暴中 **187160 late / 16 underflow**（≈100% `late`）——
  `late` 是"流上的东西来晚了"，不是"宿主没还缓冲（underflow）"。

**② s77（用户刚跑的那条，= premerge worktree、`38bfc6ce02`）：同形**

抽样其日志尾部 20 MB：**270007 `late` / 6 `underflow`**；读到时 RF 失败已 **826685**；
`.stderr` 只有 1962 字节（**无收尾报告**）；进程 `R+`、**154% CPU**、运行 14 分钟仍在打 `late` ⇒
**卡在错误循环里、不会优雅退出**（与 s76 的 "Could not stop application after 5 seconds. Forcing exit." 同因）。
⇒ **2/2 复现**（两条都是下行 iperf3 饱和负载）。

**③ 因此要把前面几节的措辞收窄（同一天内第二次自我更正）**

| regime | 腿 | RF 失败 | 收尾 |
|---|---|---|---|
| **上行负载**（手机上传；我在 §5.9.124–126 追的"墙"）| s70 351、s71 0、s72_1934 0、s72_1936 1379、s73 51(cpu)、s74 166、s75 18 | 0 … 1379 | 正常 |
| **下行饱和**（iperf3 CN→Phone）| **s76 187210、s77 ≥826685** | 10^5–10^6，**≈100% `late`** | **挂死 + 强杀** |

⇒ **"墙"（上行、后段成簇、0–1379）与"洪泛"（下行饱和、10^5–10^6、挂死）是两件事**，
§5.9.126 里"跨两个数量级、与负载无单调关系"的说法**只适用于上行那一组**；混在一起比是**又一个"两个不同的量"**（§6 纪律 1）。

**④ 与融合车道的关系（说清边界，不揽也不推）**

* 车道**只管上行**（`tx=cpu`，下行不在融合路径里）；这两条腿的**上行是轻的**，且 s76 中场打印的契约是 **8/8**
  （`ce device estimates: N device, 0 host`、`crossings 0.00+0.00`、`gaps=0`、`wrap_copies=0`）⇒
  **这条链不是融合车道的性质**，而是**这台 testbed 在"下行饱和 + 射频传输"上的能力/健壮性边界**。
* 但它有两条**必须留下的结论**：① 任何"`0 RF 失败`"的措辞继续作废（§5.9.54 表下已标注）；
  ② **"应用在 5 秒内停不下来"是一条独立的健壮性缺陷**（`lib/support/signal_handling.cpp:30` 的看门狗被迫兜底），
  它值得自己的一张工单——它让**任何一条撞上洪泛的腿都拿不到收尾计数器**。

**⑤ 下一轮预登记（先写死）**

| 腿 | 配方 | 判据 |
|---|---|---|
| **D-1（限速）** | 同样的下行 iperf3，但**限速**（`iperf3 -c <phone> -R -b 40M -P 4 -t 240`）| **不再洪泛**（RF ≤ 1000 且能正常收尾）⇒ 是**饱和阈值**驱动；**照样洪泛** ⇒ 与速率无关（指向射频/传输本身）|
...
#### 5.9.128 ★★★ 最重要的性能发现：**在匹配的 ~16–19 Mbit/s 上行负载下，`gpu` 丢样点、`cpu` 完全干净**（R5 第二次独立命中）

**① 先更正一条我自己写错的指令**：§5.9.127 ⑤ 的 D-1 配方里我写了 `iperf3 -c <phone> -R -b 40M`，文字却说"同**下行**负载"。
`-R` 是**反转**（server 发、client 收）⇒ 手机发、CN 收 ⇒ **那是上行负载**。用户按指令执行并补充了这个细节。
⇒ 因此 D-1/D-2 实际测的是**上行**，"`-b 40M`"限的是**上行**速率；**下行饱和阈值（D-1 原本的问题）仍然未测**。

**② 这两条腿的负载（工具量出来的，不是我估的）**

| 腿 | 时长 | 上行授权（占槽）| **上行净荷** | TBS 中位 | 车道 |
|---|---|---|---|---|---|
| `s78-dlcap40-n78`（gpu）| 331.7 s | 145145（**21.9%**）| **15.97 Mbit/s** | 4867 B | lanes=145145、`cbs/lane=2.00 (max=2) dropped=0`、**占用 49.3%**、余量 50.7%、"可服务 888 跳/s，要求 437.6"、burst 86.1% |
| `s79-dlcap40-cpu-n78`（cpu）| 292.7 s | 144829（**24.7%**）| **19.04 Mbit/s** | 4867 B | 无（cpu 模式没有车道）|

⇒ **这是全纪录里最重的两条上行腿**（此前最好 s70 = 1.27 Mbit/s，**高 12–15 倍**）。
**预登记的"≥2.0 Mbit/s"终于被满足（8–9.5×）**；**占槽 22/25% 仍未达 50%** ⇒ 按纪律**不能**称它们为"预登记意义上的重上行腿"，
但净荷这一维已经远超阈值。**两条腿的负载匹配得很好**：上行峰值 600/600 授权/s、下行峰值 251.6/254.2、TBS 中位同为 4867 B。

**③ 结果（`wall_ab.sh` 机械求值，P0 已改成"按主导方向"）**

| 判据 | 结果 |
|---|---|
| P0 负载可比性（主导方向）| **PASS**：`uplink 600.0 vs uplink 600.0 grants/s`（ul 600/600，dl 251.6/254.2）|
| R1 硬门 | gpu 腿 **FAIL**（`contract NOT MET: 1 of 8` = 连续性；`gaps=1`）；cpu 腿 **PASS**（`MET (5 of 7)`、`gaps=0`）|
| R2 墙是否复现 | **PASS**（733 ≥ 100）|
| R3 形状 | **PASS**（613/733 = **84%** 落在最后 1/4）|
| R4 `stale ≥ 1` | **PASS**（`stale=5`）|
| R5 在哪一侧 | **PASS ⇒ 车道/GPU 侧**（cpu 干净 ≥5×：**0 vs 733**）|

**④ 为什么这条读数比前面所有"墙"的读数都重要**

* **失败类型指向宿主侧**：`s78` 是 **651 underflow + 81 late**（`underflow` = 宿主没及时把缓冲还给电台），
  而**下行饱和洪泛是 ≈100% `late`** ⇒ **两种现象的侧别不同**，这是继 §5.9.127 之后第二次用类型把 regime 分开。
* **车道本身没过载**：占用 49.3%、余量 50.7%、"可服务 888 跳/s 而只要求 437.6" ⇒ 瓶颈**不在 GPU 的算力**，
  而在**宿主侧的流水线**（提交/等待/交棒记账那一段）——这与 `defer_wait`/提交代价落在宿主上的旧读数一致。
* **这是 R5 的第二次独立命中**：s72/s73（爬坡负载）1379 vs 51；s78/s79（**限速、稳态、匹配负载**）**733+丢样点 vs 0**。
  ⇒ **在重上行下 `gpu` 路径跟不上电台、而 `cpu` 跟得上** —— 这是里程碑之后**第一个可复现的性能/正确性信号**，
  而且它落在"本线存在的理由"（把上行搬到设备侧）上。

**⑤ 必须写明的三个保留**

1. **仍是单次配对**：本现象此前**跑间波动极大**（同构建 0…187210）⇒ **必须重复一次**（D-1b/D-2b）才能把它从"一次观测"升为"性质"。
2. **单变量只到"模式"这一层**：cpu 腿把整条链都搬回宿主，所以"车道/GPU 侧"是预登记的裁决，
   **不是**"哪一个宿主阶段"的定位——那要靠下一步的**单旋钮二分**（`OCUDU_DFT_OPEN_BLOCK=0`、`OCUDU_CE_HOLD_EXTRACTION=0`、
   `OCUDU_DFT_PIPELINE_DEPTH` 等现成旋钮，一条腿一个变量）。
3. **占槽未达预登记的 50%** ⇒ 按 §5.9.121 ⑤ 的规矩**不许改阈值**；引用时必须带上真实负载剖面（16–19 Mbit/s、22–25% 占槽）。

**⑥ 下一轮预登记**

| 腿 | 配方 | 判据 |
|---|---|---|
| **E-1 / E-2（重复）** | 与 s78/s79 **完全相同**（`-R -b 40M -P 4 -t 240`，gpu 与 cpu 各一条，背靠背）| **gpu 又丢样点/gaps>0 而 cpu 干净（≥5×）** ⇒ 现象成立（2/2）；**gpu 干净** ⇒ 前一次是波动，本结论降级 |
| **D-1'（原本的下行问题）** | **去掉 `-R`**（CN→手机）并限速：`iperf3 -c <phone> -b 40M -P 4 -t 240` | 不再洪泛 ⇒ **下行饱和阈值**驱动（§5.9.127 ④ 的洪泛）；照样洪泛 ⇒ 与速率无关 |
| **F（二分，需要 E 成立才做）** | 在 gpu 腿上加**一个**旋钮（先 `OCUDU_DFT_OPEN_BLOCK=0`）| `gaps` 是否消失 ⇒ 把"宿主侧哪一段"定位下来 |

#### 5.9.129 ★★★ E-1/E-2 复现，而且**机制找到了**：融合车道在重上行下把**接收池抽干**，阻塞接收线程 ⇒ underflow

**① E-1/E-2 的裁决：`wall_ab.sh` 7 条判据全部 PASS**

| 腿 | 模式 | 上行净荷（占槽）| RF 失败 | 类型 | gaps | 契约 | `stale` |
|---|---|---|---|---|---|---|---|
| `s78-dlcap40-n78` | gpu | 15.97 Mbit/s（21.9%）| **733** | 651u/81l | **1**（73,661 样点）| NOT MET | 5 |
| `s79-dlcap40-cpu-n78` | cpu | 19.04 Mbit/s（24.7%）| **0** | — | 0 | MET | — |
| **`s80-ulcap40-n78`** | gpu | 19.06 Mbit/s（24.3%）| **555** | **471u/84l** | **0** | MET | 2 |
| **`s81-ulcap40-cpu-n78`** | cpu | 19.55 Mbit/s（26.6%）| **1** | 1u | 0 | MET | — |

* **负载匹配**（P0 PASS）：上行峰值 **600.1 / 600.1** 授权/s、下行 217.5/200.9、TBS 中位 5379/4612 B ⇒ 两条腿的报价几乎相同。
* **R5 第三次命中"车道/GPU 侧"**：**555 vs 1**（≈555×）与上一对的 **733 vs 0** 同形，且**失败类型签名一致（underflow 为主）**。
* **R3 PASS**：s80 的 555 次失败 **100%** 落在最后 1/4 时间（`stale=2`，R4 PASS）。
* **★ 但"丢样点"没有复现**：s80 `gaps=0`（s78 那次是 1 个）。而且 `gaps>0` 并非 gpu 专有——**s73（cpu）也丢过 3 个**（爬坡负载）。
  ⇒ **两个不同的量必须分开说**（§6 纪律 1）：
  **(a) "重上行下 gpu 的 RF 失败比 cpu 高 2–3 个数量级" = 已复现（2/2 对）；**
  **(b) "丢样点" = 间歇性的、且两种模式都出现过，不是模式性质。**

**② 机制：接收池（本节的实质发现）**

同样的 ~19 Mbit/s 负载下，四条腿的 `[ul_rx_pool]` 读数：

| 腿 | 模式 | `held_max`（池 = 8）| `free_min` | `starved_takes` | `starved_events` |
|---|---|---|---|---|---|
| s78 | gpu | **8 = 池上限** | **0** | 95 | 69 |
| s79 | cpu | **3** | 5 | **0** | **0** |
| s80 | gpu | **8 = 池上限** | **0** | 74 | 56 |
| s81 | cpu | **2** | 6 | **0** | **0** |

* 池是**每槽取一个**（`taken` == 槽数，且 `returned == taken` ⇒ 无泄漏）。
* **gpu 把缓冲持有得久得多**（held_max 8 vs 2–3）⇒ 池被抽到 0 ⇒ `pop_blocking()` **阻塞接收线程** ⇒ 电台 **underflow**（正是观测到的失败类型）。
* **持有时间有多长**（同一腿的其它仪表）：`[ul_gpu_lane] residency` 均值 **1126 µs**、`[ul_pipeline]` 跨度 **中位 2772 µs / 最大 7968 µs**。
  池 8 个 × 每槽 0.5 ms ⇒ **持有跨 8 个槽（≈4 ms）就把池吃光**，而尾部的 max 跨度（≈7.9 ms ≈ 16 槽）必然吃光。
* **为什么 cpu 干净**：宿主同步处理、处理完即还（held_max 2–3、**零饥饿**）。**这不是"cpu 更快"，而是"cpu 不把缓冲留着跨跳"。**
* ⇒ 这与融合车道的**设计本身**自洽：交棒的用意正是让变换**延迟到那一跳的命令缓冲里执行**，
  于是那批样点必须**活着直到该缓冲完成**（keepalive token 的注释写的就是这件事）。
  代价是**接收缓冲的持有期 = 流水线跨度**，而池只有 8 个 ⇒ **余量本来就薄**，**此前的腿从没重到能把它抽干**
  （最重的 s70 只有 1.27 Mbit/s，也已经 `free_min=0`）。

**③ 这条结论对里程碑的意义（照实说）**：里程碑的契约性质（8/8、`0.00+0.00`/跳、就地装配、0 wraps）在**四条重上行腿上都成立**，
**但"车道在高负载下不给电台添麻烦"这一条不成立**：在 ~16–20 Mbit/s、占槽 22–27%、**车道占用仅 ~50%（能力是需求的两倍）**时，
**gpu 腿已经让电台 underflow 几百次，而 cpu 腿是 0–1 次**。⇒ **这是里程碑之后第一个需要单独立项的工作项**，
而且它**不是**"GPU 算力不够"（占用一半、跨度假均未爆），而是**接收缓冲的生命周期 vs 池容量**的结构性余量问题。

**④ 修法候选（登记，不在本轮动手）**

| # | 修法 | 代价 / 判据 |
|---|---|---|
| 1 | **加大接收池**（8 → 16/24/32）| 最直接。判据：同负载下 `held_max < pool`、`starved_events == 0`、RF 失败 ≈ cpu 量级（≤ 10）。内存代价小（每缓冲 = 一个槽的样点）|
| 2 | **缩短持有期** | 与交棒的设计目的冲突（延迟执行正是车道存在的理由）⇒ 只能"在真有必要时才延迟"|
| 3 | **把池耗尽当"车道容量事件"而非阻塞**（拒掉那一跳 / 走严格模式已有的 fail 路径）| 保电台实时性、牺牲个别跳；需要判据：`gaps=0` 且被拒跳数被计数（而不是静默阻塞）|
| 4 | 提高池的**下限策略**只解决一部分 | 现在 `lower_phy_factory.cpp` 的 `std::max({8U, …})` 是**调用点策略** |

**⑤ 下一轮预登记**

| 腿 | 配方 | 判据 |
|---|---|---|
| **G-1（决定性）** | 把接收池**加大**（离线一行改动 + 一个单测），然后跑与 s80 **完全相同**的 gpu 腿 | `starved_events == 0`、`held_max < pool`、**RF 失败 ≤ 10**（≈ cpu 量级）⇒ **池就是瓶颈**（机制坐实）|
| **G-2（对照）** | 同负载、同池，跑 cpu 腿 | 仍为 0–1 ⇒ 排除"只是这次运气好"|
| **H（备选）** | 池不变，但让交棒路径**在池紧张时拒跳**（修法 3） | `gaps=0`、`dropped` 类计数 > 0、RF 失败下降到 cpu 量级 |

**⑥ 纪律备注**：G-1 的池大小改动要有**反向臂**（小池 ⇒ 应当更容易饿死；这正好可以用**当前 HEAD 的 8** 当反向臂，
因为 s78/s80 已经是那个读数）⇒ **不需要新造反向臂**，两条既有 gpu 腿就是它。

#### 5.9.130 ★★★ **用户裁定：加大池是治标不治本** —— 根因是"数据在 GPU 流水线里被阻塞"，主攻方向 = **缩短整条 GPU PHY 流水线的时延**

**① 裁定（记下来，作为本项的定性）**：用户明确否掉了"加大接收池"作为**修法**：
池之所以被累积，是因为数据在 GPU 流水线里停留太久；**最根本的办法是缩短整条流水线的时延**。
⇒ 本项（连同 §5.9.54 ③ 的 **#13「pipeline 时间延迟（用户点名）」**，一直 OPEN）从"池容量问题"**重新定性为"流水线时延问题"**，
池只是**症状**与**兜底**。**G-1 因此从"修法"降级为"机制确认的诊断"**（见 ⑤），真正的主攻是 ④。

**② 时延分解（既有重载腿，纯离线；单位 µs，取中位/均值）**

| 量 | s80（**重载**：19.06 Mbit/s、24.3% 占槽）| s75（**轻载**：66 授权/s 峰值）| 读法 |
|---|---|---|---|
| `[ul_gpu_pipeline]` 跨度（整条流水线）| **2687**（p95 3557、max 7995）| **1994**（p95 2494、max 4332）| 端到端；**轻载就已经 ~2 ms** |
| `[ul_gpu_lane] residency`（车道缓冲寿命）| **1125**（max 6253）| **1121**（max 2816）| **与负载几乎无关** ⇒ 这是**结构性**的一段，不是被负载压出来的 |
| `mmse_time_sum defer_wait`（宿主等延迟链）| **1227**（p95 1322、max 6423）| **1368**（p95 1832）| 与 residency **是同一个窗口的宿主视图** ⇒ **不可相加** |
| 车道 busy：`merged_hop` | **1030**（97% of busy）| 982（96%）| 车道真正在 GPU 上执行的那一段 |
| 车道 busy：`ch_wt` | 37（3%）| 44（4%）| 估计器权重那一段（小）|
| `[ul_gpu_lane] gap`（阶段入口→…）| 45 | 100 | 排队/空隙 |
| ⇒ **车道之外的部分**（跨度 − 车道窗口）| **≈ 1460–1560** | ≈ 630–870 | 重载时**这一块长了 ~0.7 ms**（rx 装配、前端、LLR/上报、排队）|

**③ 结论（回答"根因在哪"）**：
* **流水线时延 = 一段"与负载无关的 ~1.1 ms 车道窗口" + 一段"随负载增长的车道外开销（轻 ~0.7 ms → 重 ~1.5 ms）"**。
* 持有期（= 接收缓冲寿命）≈ **整个跨度**（因为交棒把变换延迟到那一跳的命令缓冲里跑，样点必须活到该缓冲完成）。
  池只有 8 个、每槽一个 ⇒ **跨 8 槽（4 ms）就吃光**，所以**只要跨度逼近/超过 4 ms 就开始饿死接收线程**。
* ⇒ **缩短时延 = 同时缩短那 1.1 ms 的车道窗口与那 1.5 ms 的车道外开销**；两者都要有读数支撑才动手。

**④ 主攻路线：先把三段拆开（`OCUDU_UL_PHASE_SEGMENTS=1`，仪表早已存在，§5.9.32 有语义与判读表）**

```bash
cd /Users/jiachengwang/dev/ocudu
# L-1：与 s80 完全相同的重载配方（手机发、CN 收、限速 40M），只加相位分段开关
#      手机侧/CN 侧命令不变：iperf3 -c <phone> -R -b 40M -P 4 -t 240
sudo -E LEG_CONFIG=configs/gnb_rf_b200_tdd_n78_20mhz.yml \
  bash doc_chinese/phy_pipeline_gpu/wip/run_leg.sh gpu s82-phases-heavy-n78 OCUDU_UL_PHASE_SEGMENTS=1
```
读法（§5.9.32 已登记的判读表，此处照抄并绑定到本项）：
三段为 `[ul_time_frequency]`（IQ 收完 → 前端交出该槽的块）、`[ul_channel_estimation]`（→ 估计器这段做完）、
`[ul_equalization_demod]`（→ 第一次 LDPC 解码调用前）；**三段之和应 ≈ `[ul_gpu_pipeline]`**。

| 观察 | 主攻方向 |
|---|---|
| 尾巴在 **`ul_time_frequency`** | 前端/交接侧（含"交出后由谁提交"）⇒ 缩前端提交与交棒次序 |
| 尾巴在 **`ul_channel_estimation`** | 估计器这段（含等自己那块被认领/提交）⇒ 与交棒时机/`fallback·late` 有关 |
| 尾巴在 **`ul_equalization_demod`** | 均衡/解调段（含车道排队）⇒ 车道内排队与共享池 |
| 三段都平、只有 `ul_ldpc_decode` 离群 | 与车道无关：解码器/调度抖动，单独记账 |
| **三段之和 ≪ `[ul_gpu_pipeline]`** | 差额落在**未被分段覆盖**处 ⇒ 先查 `[ul_rx_wait]` 与 `gpu_lane gap`（本项 ② 里那段 ~1.5 ms 就在这里）|

**⑤ 任何时延修法的**验收判据**（先写死，免得用"灯变绿"代替"时延变短"）**

| # | 判据 | 阈值 |
|---|---|---|
| V1 | `[ul_gpu_pipeline]` 中位 | **明显低于重载基线 2687 µs**（至少 −20%，即 ≤ 2150 µs）|
| V2 | 池压力 | `starved_events == 0` 且 `held_max < pool`（= 症状消失）|
| V3 | 电台 | RF 失败 **≤ 10**（≈ cpu 量级 0–1）、`gaps == 0` |
| V4 | **不许用提交数换时延** | `cbs/lane ≤ 2.00 (max=2)`、`dropped == 0`（保住 D1 的"每跳一次提交"）|
| V5 | 不回归 | 契约 8/8、`crossings 0.00+0.00`、CRC KO% 不劣化 |

**⑥ G-1 的新定位（诊断，不是修法）**：把池临时加大**只**用来确认因果链
（"持有 → 池干 → `pop_blocking` 阻塞接收线程 → underflow"）：若**时延不变而 underflow 消失**，链条成立；
若 underflow 照旧，则链条还有第三方（例如 `[ul_rx_wait]` 侧）。**它不解决时延，因此不作为交付。**
...
⇒ 这就是"设备路由 == 带正确对角加载的宿主路由"的**测量**，而不是论证；§5.3.9 第 3 条据此结案。
...
| 6 | **空中腿** | ⏳ 见 §5.3.8：契约 **8/8**、**2 PRB 分层 CRC > 0**、1 PRB 不劣化、其余宽度不回归 |
...
* ~~**CPU 参考链在窄跳上仍然全 KO**~~ ⇒ **已撤回**（§5.3.7 勘误）：那个"CPU 臂"是 `host` 臂的重复运行
...
* ~~`OCUDU_CE_CORR_DEV=0` 不是精确 A/B~~ ⇒ **已结案**（§5.3.7）：`OCUDU_CE_CORR_DEV=0` **加上**
...
> ⚠ **2026-09-23 审计更正：本节 ① 的"每批必过 / 期望 `differing-bytes=0`"是陈旧的，它**不是判据**。**
> 角色分工在本文件 §5.8.x（"迁移：两条网的角色"，正文行 *"`wip/value_net.py` = 门；`wip/neutral_vs_baseline.sh`（逐字节）= 信息"*）
> 已经定了：**门 = `value_net.py`（`captures=47 problems=0`，`--self-test` 8/8）**，逐字节网**降级为信息**（"移了多少字节"的诊断，**不要删**）。
> 实测（里程碑审计，`29686076d2` 树）：`value_net` **47/0**、两个 `ab_dumps` 臂 **0 差异**、跨跳网 **0 差异**，
> 而逐字节网 **25 个捕获 / 131 字节**（23 个 `_ce.txt` + 2 个 `_h.bin`，即重新结合允许移动的估计器字段与网格尾部）——
> **"逐字节红"在今天不构成回归**；真正的判据是 `value_net.py` + 空口腿（CRC/契约）。
> ⚠ 反过来也必须成立：**纯编排类改动（P1/P2/P3）本来就该报 0 字节** ⇒ 逐字节网仍然要跑、要读它的数字，只是不阻断。
> 历史教训保持有效：这条网抓到过本线**唯一两个真实数值缺陷**（P0 的 NaN、P5 的 5 个数量级）——它们正是 `value_net.py` 里那两条
> 硬判据（NO-NaN + 数量级绊线）的来源，所以"降级"不等于"可以忽略"。
...
其实跑在宿主上而看起来完全合理 —— 这正是 §5.3.7 勘误里那个假 CPU 臂的成因。
...
| **`gnb_gpu_s13p1b_0919_2322`** | **`650ba53d6b`** | ✅ **P1 剩余空中验证**（§5.3.5）：3748 跳，RTF 0，gaps 0，CRC 80.8%，**读侧 0.00**；**写侧 0.10/跳** —— 全部来自 `ce: A/R_hp staged (host)`（368 call(s)，`refusals=corr_geometry=368`），即**窄分配（1–2 PRB）**的相关矩阵宿主 staging；契约 7/8。**这条写是新仪表才看见的既有路径**，修法见 §5.3.5 |
| `gnb_gpu_narrow-cap_0920_0453` | `0fd884036c` | 取窄语料的腿（§5.3.7）：13,824 跳，RTF 1，gaps 0，CRC 95.2%，契约 7/8（写 0.04/跳 = 620 次 A/R_hp；读 59 次全部来自 `OCUDU_UL_DUMP` 的宿主网格实例化）；**按宽度分层：2 PRB 仍 0.0%（564 次）**。录到 20 条真实 1–2 PRB 捕获 |
| **`gnb_gpu_narrow-fix_0920_0508`** | **`6742e2f271`** | ✅ **S13-P2c 空中验证**（§5.3.8）：16,067 跳，**RF failure 0**，gaps 0，CRC **97.9%**（本线最高），**契约 8/8**（跨越多 0.00 读 + 0.00 写/跳、**两张分项表都空**、`refusals=<none>`）；**2 PRB `0.0% → 77.5%`**、SINR 中位数 `−10.7 → +13.0 dB`。992 条 FAPI busy 与话务速率逐桶对应（非本批回归，见 §5.3.8）|
...
> ⚠ **2026-09-23 里程碑审计：本表有 4 行的"默认"已经翻过，但表里没有指针（滚回/复现时会照错的默认值跑）。**
> 逐行读代码的现状：`OCUDU_CE_HOST_GRID` 默认 **0，不是 1**（`port_channel_estimator_metal_mmse_impl.cpp:573-576`；
> 翻转见 §17.10 / §5.9.93，空口腿 `5c-hostgrid_0919_1730` 读到读侧 `1.39 → 0.00`/跳）；
> `OCUDU_CE_LANE_ORDER` 默认 **`merged`，不是 `event`**（`:1467`；翻转见 §17.10/§5.9.93）；
> `OCUDU_UL_FRONTEND_FENCE` **已经不存在**（`grep -rn "OCUDU_UL_FRONTEND_FENCE\|front_end_wait" lib include apps tests` = 0 命中 ⇒
> 前端栅栏机制随交棒成为默认而退役，见 §5.9.65/§5.9.99）；
> `OCUDU_CE_RSRP_CHECK` **不是"已回退"，它是活的**（`ocudu_metal_mmse_engine.mm:1666`、`…mmse_impl.cpp:5171`）。
> **规矩**：本表是滚回/复现的参考，翻默认时必须在本行留一句指针（本次 4 行都没有）。
...
>
> **⚠ 适用范围（2026-09-19 晚，S13-P1 剩余部分，§5.3.5）**：紧接着的一条腿
...
> 新仪表才让它可见。修法（及其已证伪的朴素版本）见 §5.3.5。
>
> **✅ 适用范围已补全（2026-09-20，S13-P2c，§5.3.7/§5.3.8）**：窄跳也改由设备建矩阵（`b1ab17c937`）后，
...
> ⚠ **2026-09-23 里程碑审计更正（本节是文件最后一节，读数早已过期）**：上面这条"契约现在在空口上是 **7/8**"
> 是 **P2 之前**的读数（那条每跳往返是 §5.5 的题目，**已经做完**）。今天空口上是 **8/8**：
> s47/s62/s63/s64b/s65/s67/s69 七条默认路径腿逐条读到
> `host device data crossings: 0 host read(s) (0 bytes) and 0 host write(s) (0 bytes) ... = 0.00 read(s) + 0.00 write(s) per hop`
> + `contract MET (8 of 8 checks applicable)` + `[mmse_time_sum] hops_gpu=N hops_no_gpu=0 fb_blocks=0`。
...
#### 5.9.136 P0-1 的**真实缺口**：分组读数其实早就有了，缺的是"前端那一段"

**① 记录里已有的分组读数（§5.9.66 ②，同一 harness 上按路线标注）**

| 路线 | `busy split` |
|---|---|
| `merged`（生产） | `merged_hop=468.7us/lane (90%)` + `eq_demap=54.3us (10%)` |
| `event` | `ch_wt=482.6us (87%)` + `eq_demap=70.8us (13%)` |
| `wait` | `ch_wt=847.0us (90%)` + `eq_demap=95.8us (10%)` |

⇒ **两条独立结论已经成立**：(a) **均衡+解映射只占车道 busy 的 10–13%**（54–96 µs）；
(b) **估计器的权重缓冲是主体**（event 路线上 483 µs），而它与**离线账单**（§5.8.17 ② 空口几何：K1 197.5 + 抽取 117 + 重排 116.9 + K2 35.6 + corr 35.3 ≈ 500 µs）**互相印证**。

**② ⇒ P0-1 的剩余问题因此被收窄成一句话**：生产路线上那条 `merged_hop`（≈1030 µs）里，
**前端"交出"的那 14 个 transform 占多少**？它无法从 `event` 路线直接读（那是**不同的提交结构**），
按减法估 ≈ 1030 − 500 − 54 ≈ **476 µs**，但**减法是推断，不是测量**。

**③ 两条候选（按成本排序，**先做 A**）**

* **A（不写代码）**：飞一条加压腿同时开 `OCUDU_UL_PHASE_SEGMENTS=1` ⇒ 把 `[ul_gpu_lane] busy split` 与三段**配对**读
  （这也顺手完成 **P0-5**：两个探针的样本数不同，配对后 `busy/residency` 才能当预算用）；
  再以 `OCUDU_CE_LANE_ORDER=event` 跑同一配方，取 `ch_wt` / `eq_demap` 的**分组** GPU 时间在同一二进制上对照。
  判据：配对后 `busy split` 三项之和 ≈ `residency`（±10%）。
* **B（写代码，仅在 A 仍不足时）**：在命令缓冲**内部**插两个 `MTLSharedEvent` 信号（前端之后、估计器之后），
  宿主观察信号到达时刻并出分布 ⇒ **不改提交结构、不改输入缓冲寿命、不动 V4**（`encodeSignalEvent` 在本代码库已有用法：
  `ocudu_metal_queue.mm:464/528`）。⚠ 本机**不支持逐 dispatch 计数器**（§5.8.15 ①），所以 B 是"缓冲内分段"而不是"逐 kernel"。

**④ 一条工作流事实（本轮实测）**：**任何代码改动都会让"腿的提交证据"失效**。
P0-6 落了 5 个代码文件后，门的 `default leg … the commit it ran, vs HEAD` 与 `stress leg …` 两条判据双双转红
...
⇒ **"不新增提交"与"提前释放"在本平台不能同时成立**；P2-E 需要用户裁决（选项见 `phy_latency/gpu_phy_latency_optimization_design_and_implementation.md` §6.4.4）。
...
#### 5.9.141 `phy_latency/` 的文档整理（2026-09-25，用户要求）：**三类文档 + 一个开发文档**

**① 用户的规矩**：`session_handoff_*` 只作**新会话交接的状况快照**；开发过程中要记的东西**统一记入一个专门的开发文档**。
⇒ `doc_chinese/phy_latency/` 现在是**三类**：

| 类 | 文件 | 性质 |
|---|---|---|
| **design & implementation** | `gpu_phy_latency_optimization_design_and_implementation.md`（**新**，2026-09-25）| 开发文档：现象/机制/仪表/跑腿规范 + **追加式实施记录**（§6.1 P0-6、§6.2 P0-1、§6.3 P0-5、§6.4 P2-E、§6.5 缺陷与教训）+ 杠杆与候选（§7）+ 未决（§8）+ 证据索引（§9）|
| **session handoff memo** | `session_handoff_2026-09-24-1.md`（上一会话）、**`session_handoff_2026-09-24-2.md`（本会话，#1 之后）** | 一次性快照（新会话只读**序号最大的那一份**）。⚠ **用户 2026-09-25 的两条要求**：(a) memo 只是快照，**一条会话的结论与证据要并进开发文档**；(b) **命名 `<会话开始日>-<序号>`，跨午夜不改日期** ⇒ 本会话（09-24 开始）的交接是 `-09-24-2`，而 `session_handoff_2026-09-25-1.md` 是**下一个会话**的第一份（曾按 09-25 命名的那一份已并入开发文档并删除）|
| **high level status and plan** | `high_level_status_and_plan.md`（**新**）| 一句话现状、V1–V5、阶段进度、下一步、未决与待裁决 |

外加 `README.md`（三分工 + 文件命名 + **旧名映射**）。

**② 旧文件 → 新位置（本目录的 `00_status.md` … `05_p2e_platform_finding.md` 已删除，内容整体并入）**：

| 旧文件 | 现在 |
|---|---|
| `00_status.md` | 现象/机制/设备读数 → 开发文档 §2；判据与配方 → §3；未决 Q1–Q11 → §8；高层一览 → 高层文档 |
| `01_plan.md` | 判据/配方 → §3；归属式预算与 P1/P2 候选 → §7；阶段进度与下一步 → 高层文档 |
| `02_measurement.md` | 仪表手册 → §4；跑腿与门的规范 → §5 |
| `03_p0_instrumentation.md` | P0-6 → §6.1；P0-1 → §6.2；流程教训 → §6.5 |
| `04_p2e_plan.md` | 计划原文摘要 → §6.4.1 |
| `05_p2e_platform_finding.md` | 实现与平台证伪 → §6.4.2–§6.4.5 |
| `session_handoff_2026-09-25-1.md`（命名错误，内容已并入；该名留给下一个会话）| 仓库/构建状态 → §5.3；P0-5 → §6.3；P2-E 证据 → §6.4.3；`s84b-p0` 无日志 → §8 Q12 |

⇒ **本文更早章节里对 `phy_latency/00–05` 的引用，一律按上表读**（历史章节不改写，见本文开头的约定）。
**未决问题的编号已重排**：旧 Q1–Q8 保持原义，新增 **Q9**（5 秒收包停顿的成因）、**Q10**（n1 退化的残余）、
**Q11**（两条改前就红的网）——见开发文档 §8 与高层文档 §4。
