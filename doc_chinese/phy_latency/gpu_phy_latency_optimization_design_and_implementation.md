...
V2 ✅ `starved_events=0`、`held_max 9–18 < 32`、`dropped=0`；V3 ⏸ **另案暂停**（RF 841–1724 是逐腿天气，比较要成对）；
...
* **唯一还挂着的大项**：**前端那条 cb 的 ~452 µs**（占一跳窗口 87%，**整段在关键路径上**），且**只在空口出现**——
  交付几何下离线能构造的每一种形状都只要 **23–58 µs**（10–20×）。
* **可靠**：V1、`busy split`、F3、Q24/Q24b、阶段段（单个）、池/持有期。
...
* **已作废/更正**：§6.65（前端 10.6 µs）、§6.77（"37 µs 是平台地板"）、§6.80（"前端 = 真实执行且不重叠"）、
  §6.65③（"464 µs 是等样点"）、§6.71（H4）、§6.73/§6.77（两版 Q24/Q24b 的配对与聚合缺陷）。
* **未归属**：那 ~400 µs（前端 cb，空口特有）；下一步见 §6.84⑤ 的待飞清单（先做零 V4 代价的"持久 staging 缓冲"臂）。
