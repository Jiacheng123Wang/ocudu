...
#include "ldpc_gpu_decoder_metal_source.h" // generated by CMake (embedded .metal source)
...
struct engine_impl_t {
  ...
  id<MTLBuffer> buf_vn_total = nil; // zero-copy (CPU-written, GPU read-only)
  id<MTLBuffer> buf_ctrl     = nil; // shared (GPU writes, CPU reads)
  id<MTLBuffer> buf_s_hard   = nil; // private
  id<MTLBuffer> buf_h_pred   = nil; // private
  id<MTLBuffer> buf_err_eq   = nil; // private
  id<MTLBuffer> buf_suspect  = nil; // private
  id<MTLBuffer> buf_evidence = nil; // private
  id<MTLBuffer> buf_debug    = nil; // shared (VNStats)
...
bool decoder_engine::init(uint32_t n_logical, uint32_t m_logical, float alpha, const uint32_t* h,
                          const uint32_t* ht, uint32_t* col_weights_out)
{
  ...
  if (library == nil) {
    NSString* src = [NSString stringWithUTF8String:(const char*)ldpc_gpu_decoder_metal_source];
  ...
  // Shared (not private): the CPU recomputes the final syndrome from h_pred after the
  // kernels complete (the shader clears ctrl->error_count at the end of every iteration,
...
{
  engine_impl_t* engine = static_cast<engine_impl_t*>(impl);
  if (engine == nullptr || engine->p_init == nil) {
    return -1;
  }
  ...
  // Stage B: initial full syndrome computation (once).
  ...
  for (int it = 0; it < max_iter; ++it) {
    ...
    [enc setBytes:&engine->alpha length:sizeof(float) atIndex:8];
  ...
  if (error_count_out != nullptr) {
    // Recompute the final syndrome on the CPU: the shader clears ctrl->error_count at
    // the end of every full iteration, so it does not reflect the final state.
