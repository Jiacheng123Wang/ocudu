...
do {
    ...
    let params = LDPCParams(
        ...
        g0_words_per_row: UInt32(g0WordsPerRow),
        num_full_loops: numFullLoops,
        remainder: remainder
    )
    
    let iterations = 10000
    print(">>> 开始 LDPC 终极架构 PK (迭代次数: \(iterations), Z=\(z))")
    print("---------------------------------------------------------")
    
    let v1 = encoder.benchmark(version: "ldpc_v1", g: gBuffer, g0: g0Buffer, input: inBuffer, output: outBuffer, params: params, iterations: iterations)
    print("V1 暴力全量版 (1线程1比特):    \(String(format: "%.4f", v1)) ms")

    let v3ns = encoder.benchmark(version: "ldpc_v3_no_sync", g: gBuffer, g0: g0Buffer, input: inBuffer, output: outBuffer, params: params, iterations: iterations)
    print("V3 导航无同步 (1线程1比特):    \(String(format: "%.4f", v3ns)) ms")
    
    let v4 = encoder.benchmark(version: "ldpc_v4", g: gBuffer, g0: g0Buffer, input: inBuffer, output: outBuffer, params: params, iterations: iterations)
    print("V4 战术小组版 (32线程1比特):   \(String(format: "%.4f", v4)) ms")
    
    print("---------------------------------------------------------")
    print("M4 Pro 巅峰对决结果:")
    if v4 < v3ns && v4 < v1 {
        print("🏆 V4 战术小组版胜出！证明硬件归约和协作读取优于逻辑导航。")
    } else if v3ns < v4 {
        print("🏆 V3 导航版胜出！说明 G0 稀疏矩阵节省的计算量覆盖了逻辑分歧的开销。")
    }
} catch {
    print("错误: \(error)")
}
