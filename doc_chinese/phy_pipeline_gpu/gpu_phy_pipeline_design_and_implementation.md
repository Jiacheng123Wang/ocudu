...
#### 5.8.30 ★★ 收包粒度腿对（`s17-rxwait-slot` / `s17-rxwait-sym7`）：**"等样本"降了 66%，端到端只动 1.6%** ⇒ **收包粒度不是杠杆，议题收档，转 D1**

> 目的（§5.8.29 ⑦）：量出"结构性等待 ②"值多少。结论：**② 存在、可测、但不是杠杆**。

**① ⚠ 首先：腿里的 `OCUDU_UL_RX_SYMBOLS=7` 没有生效，两条腿实际比的是"2 slot 一块" vs "1 slot 一块"**

证据是**日志里的停机行**（`Uplink processing stopped: … (N samples)`），它直接报出真实块大小：

| 腿 | 停机行 | 真实块大小 | `[ul_rx]` 反算 |
|---|---|---|---|
| `s17-rxwait-slot` | `(7680 samples)` | **7680 = 2 slot** | 1001372153/130387 = **7680.00** |
| `s17-rxwait-sym7` | `(3840 samples)` | **3840 = 1 slot** | 680106785/177112 = **3839.98** |

⇒ **`nof_symbols_per_block` 无法把块压到一个 slot 以下**（有效的请求量被收包缓冲容量夹住）。
**这条实验顺带钉死了一个硬事实：当前缓冲策略下"块 < 1 slot"够不着**，所以整符号收包（S-7g-13）
那种"等到 slot 末尾"的等待，**在这套配置里根本不在可及范围内**。

**② 核心读数**

| 序列 | 腿1（块=7680，2 slot）| 腿2（块=3840，1 slot）| Δ |
|---|---|---|---|
| **`[ul_rx_wait]` mean** | **997.9 µs** | **498.7 µs** | **−499.2** |
| **`[ul_rx_wait]` median** | **1057.0** | **362.0** | **−695.0（−65.7%）** |
...
| | 腿1 | 腿2 |
|---|---|---|
| 总样点 / 3840 = **总槽数** | 260,774 | 177,112 |
| 运行时长 | 130.4 s | 88.6 s |
| **每槽等待 = mean(等待) × 块数 / 槽数** | 997.9 × 130387/260774 = **498.9 µs** | **498.7 µs** |

**⇒ 两条腿的每槽等待都是 ~499 µs，几乎逐位相同。**

**收包粒度只改变"多久等一次"，不改变"每槽一共等多久"。** 那个总量由 **ADC 的采样速率**决定
（一槽样点需要 500 µs 才产生完），**与块大小无关**。这是一个干净、与物理一致、可复算的结论。

**④ "块时长 − 处理时间"模型：单块等待对得上，但 `[ul_time_frequency]` 对不上**

| | 模型预测 median | 实测 median |
|---|---|---|
| 腿1：1000 − 33 | ~967 | **1057** ✅ |
| 腿2：500 − 66 | ~434 | **362** ✅（p95=720，分布更散）|

**但 `[ul_time_frequency]` 两条腿都停在 ~1080 µs 不动**，而按"等待 + 处理"它应当随块大小明显变化；
同时腿2 的 `[ul_rx_wait]` **min = 1.0 µs**（腿1 是 517 µs）⇒ **宿主的领先/落后是逐块变化的、不是常数**。
**⇒ 用中位数相减去拆 `[ul_time_frequency]` 不可靠。** 上一轮据此提出的"整符号收包能省 ~350–500 µs"
**被本条数据否掉**，**不再作为依据**。（要查清这个"不肯动的 1080 µs"需要**单槽时间线**，不是分布。）
...
1. ✅ **`[ul_rx_wait]` 仪表可用**（它把"等样本"从 `[ul_time_frequency]` 里拆出来了，这正是它的目的）；
2. ✅ **收包粒度不是延迟杠杆**：单块等待 −66%，端到端 −1.6%；
3. ✅ **每槽等待 ~499 µs 由采样率决定**，与块大小无关；
4. ✅ **当前策略下块无法小于 1 slot**（实验的副产品，硬事实）；
5. ✅ **用户裁定：这条议题收档，转 D1**。理由：D1 之后前端就是 GPU 在跑，宿主收包粒度问题**被设计消掉**；
   而 GPU 模式**硬性要求整 slot 收包缓冲**，所以这个优化在目标形态下**不存在**。
   §5.8.29 ④ 那个"不肯动的 1080 µs"**记档为遗留口径问题，不追**（要追需单槽时间线仪表）。
