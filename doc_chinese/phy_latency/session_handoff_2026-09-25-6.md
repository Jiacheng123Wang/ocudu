...
**宿主级诊断也做完了（§6.67）——不需要重建**：`mmse_phase_timer` 的开关是**运行期** `OCUDU_MMSE_DEBUG`（`OCUDU_CE_TIME` 是另一组计时器）；
`ul_chain_replay … --metal --repeat 20`（**空格**，`--repeat=20` 会报 unknown option）两臂读数：

| 相位 | 含义 | 直读 | scatter |
|---|---|---|---|
| `wrap` / `cb` | 缓冲映射 / 建命令缓冲 | 0.8 / 4.5 µs | 0.5 / 3.5 µs |
| **`encode`** | **整个 CE 权重阶段的宿主编码**（≈5 个派发）| **8.2 µs** | **6.5 µs** |
| `commit` | ⚠ **含 GPU 等待**（`end_stage` 里就 wait 了，`wait` 恒 0）——**别读成提交开销** | 6.1 µs | 3.8 µs |

⇒ **宿主 encode ≈1.3–1.6 µs/派发 ⇒ 宿主侧也排除了那 10–14 µs**。至此四个方向都量过：**GPU 执行 1.5–13.2、派发地板 1.3–1.4、边界 ≤0.3、宿主 1.3–1.6**（单位 µs/派发）。
⇒ §6.66② 的结论**维持并加强**：**配对 V1 增量是真的，但"每次派发 10–14 µs"没有仪器支持**（要么是**跳结构/调度**效应——两个仪器都把并发与别的 lane 排除在外；要么**部分来自逐腿天气**：`p39` 均值只降 14.4、中位降 21.5）。
**预算一律用 §6.66③ 的 ≈5–20 µs，不要再用 10–14 µs/派发。**

**仍未做（据实登记）**：**均衡/解映射的算力未进账单**（`ocudu_equalizer.metal` 的 `equalize_params`、`ocudu_demod.metal`）——
§6.65③ 里"均衡那条 741 µs 相位段有多少是算力"**仍未答**，这是补账的下一件；那 10–14 µs 若要坐实，需要**能看见并发/别的 lane** 的仪器（lane 级 GPU 时间戳或 §6.24 的 P0 dump），**不是更多孤立基准**。

**开工顺序**：(1) 把**均衡/解映射**补进 `ce_kernel_cost.mm`（补全账单——唯一还没做的便宜事）；
(2) ★ **A 项 / `P1-7` 的裁决**——**派发/边界这条线已见底（≤20 µs），它是唯一还有量级差的东西**（触 V4、其代码注释写明时延收益目前无法判读）。
