...
| **Q16** | 去掉 4 次派发后，**V1 降 52.2 µs**（≈13 µs/派发）而**设备占用窗口 `merged_hop` 只降 26.2 µs**（≈6.5 µs/派发）——差的那一半是"小派发被遮住"还是"直读更宽的步长稍贵"？ | **开放**（§6.49 ③）。两种解释本腿分不开，且**不影响交付读数**（V1 是判据）。要分开：`metal_chain_probe` 的 device-slice 臂（比 gather-读 vs 网格直读的 kernel 自身耗时），或照 `wip/dft_kernel_cost.mm` 的形状给均衡 kernel 做离线微基准。⚠ 在此之前，**"12 µs/派发"只用于 V1/跨度口径**，不要用 `merged_hop` 的降幅反推派发数 |
