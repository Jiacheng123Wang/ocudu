...
2. **zmq radio slow motion.** The cell slot counter advances at only ~0.2-2.6 slots/s (should be 1000/s); the E2E
   round-trip on macOS is 600-800 ms versus 300-400 ms on Ubuntu because of it. Suspects: clock drift between the
   two hosts and the zmq `tx_time` pacing. Needs its own investigation with captures on both sides.
