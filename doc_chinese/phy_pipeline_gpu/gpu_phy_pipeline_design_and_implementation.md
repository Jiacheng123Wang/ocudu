...
| **S13-P3 (G3a)** | 稀疏 RB 掩码：设备侧支持"有洞"的分配（K0-a / K3 / 等化器）| ❌ **决策：不做**（§5.7）。空口 **103,056 次上行授权 0 次非连续**（`vrb_alloc` 只用了 type1 区间）、四个门**能响且从未响**（阳性对照 `corpus_sparse/`：稀疏 ⇒ `ls_geometry/y_no_ls/ta_stride/k3_geometry`、穿越 2.00 读 + 1.00 写/跳）⇒ 收益是**适用范围**而不是修正在发生的错误；回归件与判据已就位 |
...
* 收益是**适用范围**（把 `8/8` 的措辞从"连续分配"扩到"任意分配"），**不是修一个正在发生的错误**：
  103,056 次空口授权里没有一次能触发它（S13 方案 §5 的 R4 预先写明了这个可能）；
* 真出现时**后果可见且安全**：四个门报数、宿主兜底出结果、契约穿越计数变红 —— **不是静默错误**；
* **回归件与判据已经就位**，要回头做随时可做：`corpus_sparse/` 四条 + 判据"稀疏两条的 `refusals`
  回到 `<none>`、穿越回到 1.00 读 + 0.00 写"。

**⚠ 措辞边界（本节的主要产出）**：`8/8` 的适用范围是**连续分配**。这句话以前只写在 S13 方案里，
现在写进设计文档，而且**不再是悬而未决的**：条件是什么、何时不成立、不成立时会发生什么，三条都有量。
