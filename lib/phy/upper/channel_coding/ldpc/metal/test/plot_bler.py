...
def load_series(path):
    """Returns (snrs, bler_cpu, bler_gpu, cpu_ci, gpu_ci)."""
    snrs, cpu, gpu, cpu_ci, gpu_ci = [], [], [], [], []
    with open(path) as f:
        for line in f:
            line = line.strip()
    ...
    return snrs, cpu, gpu, cpu_ci, gpu_ci
...
def plot_group(bg, z, files, outdir, show, with_ci):
    ...
    for f in sorted(files):
        ...
        snrs, cpu, gpu, cpu_ci, gpu_ci = load_series(f)
        by_rate.setdefault(float(m.group(4)), (snrs, cpu, gpu, cpu_ci, gpu_ci))

    legend_handles = []
    for slot, rate in enumerate(sorted(by_rate)):
        color = RATE_COLORS[slot % len(RATE_COLORS)]
        snrs, cpu, gpu, cpu_ci, gpu_ci = by_rate[rate]
        ...
        (l_gpu,) = ax.plot(snrs, gpu, color=color, lw=2, ls="--", marker="^", ms=9,
                           ...
                           label=f"R={rate:.3g} GPU layered-NMS", zorder=3)
    ...
    ax.set_title(f"LDPC BLER — BG{bg} Z{z} — CPU vs Metal GPU (layered NMS)", color=INK, fontsize=12)
