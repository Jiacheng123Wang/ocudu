...
| **CE** | **K1 矩阵反演 `A⁻¹`** | **CPU（主机）**——**临时**，见 §48.88 | 它**应该**在 GPU，且**算法已验证正确**（设备 K1 的 `w`/`h` 与主机一致，差 2.5e-4 = 预期的 `W` 误差）。回退 CPU 的原始理由是**性能**（设备 555µs vs 主机 ~10µs，§48.68），后来被我**用错误的精度论据**维持（§48.85）。**当前唯一障碍是一个同步缺陷**：设备 K1 使批次变长后，某处未等到批次完成就消费结果（§48.88(c)）。`OCUDU_CE_GPU_INVERT=1` 复现 |
...
**一句话**：从"频域网格"到"LLR"，**除 K1 外全部在 GPU 上**。
K1 **目标**在 GPU、**当前**仍在主机——原因不是精度，而是设备 K1 在内核执行层面没有生效（§48.86），
修好后即回 GPU，届时那 ~555µs/hop 是性能债。
