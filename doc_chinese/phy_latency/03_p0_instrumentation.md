...
### 2.1 ⚠ 已知缺陷（只读审计发现，**待机器空闲时修**）

`adopt()` 里拆分的控制流有一个**低概率但真实**的错误：**先提交旧缓冲、再尝试创建新缓冲**。
若 `[queue commandBuffer]` 返回 nil（资源耗尽），代码会**穿透到正常路径**并 `s.cb = cb`，
而那条缓冲**已经被提交** ⇒ 调用方随后往它里面编码、跳尾还会再 `commit()` 一次（Metal 硬错误）。
`mmse_engine` 那边的契约本身是安全的（`ocudu_metal_mmse_engine.mm:3665`：**只有 `adopt()` 返回 false 时**它才自己提交），
所以问题只在"创建失败"这条异常路径上。

**修法（一行顺序问题）**：把 `id<MTLCommandBuffer> nb = [q commandBuffer];` **提到提交之前**，
`nb == nil` 时**直接跳过拆分**（不 encode 信号、不提交、不 push outstanding），再走原来的 `s.cb = cb; return true;`。

**为什么现在不改**：改代码会移动 HEAD，而 `run_leg.sh` 的预检要求**二进制戳 == HEAD** ⇒ 会让用户的下一条腿被拒（本轮已经因为我的原因拒过一次）。
用户腿落地、机器空闲后再改，并重跑 `dft_release_adopt_metal_test`（OFF/ON 两侧）与 `ctest -L phy`。
