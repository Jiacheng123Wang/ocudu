# 交接（入口） — S14（**已被 `session_handoff_2026-09-20-6.md` 取代**）

> ⚠ **本文件不再是入口**。它描述的是 S14 **早期**的状态，有两处已被推进：
> ① 下面的候选表把"K1 的**数值契约**裁定"当成待决项——**那个裁定已经做了**（门已换成 `wip/value_net.py`，见设计文档 §5.8.19）；
> ② `merged`（P3）**已经翻成默认并保留**（用户裁定，§5.8.26/§5.8.27），不再是"未翻"。
> **新会话请从 `-6` 开始。**
