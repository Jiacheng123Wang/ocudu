...
static bool validate_expert_phy_unit_config(const du_low_unit_expert_upper_phy_config& config)
{
  ...
  if ((config.ldpc_decoder_type != "auto") && (config.ldpc_decoder_type != "generic") &&
      ...
      (config.ldpc_decoder_type != "metal_nms") && (config.ldpc_decoder_type != "metal_nms_layered")) {
    fmt::print(
        "Invalid PUSCH LDPC decoder type. Valid types are: auto, generic, neon, avx2, avx512, metal, metal_nms and metal_nms_layered.\n");
    valid = false;
  }
