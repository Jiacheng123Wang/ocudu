...
* ~~**CPU 参考链在窄跳上仍然全 KO**~~ ⇒ **已撤回**（§5.6.7 勘误）：那个"CPU 臂"是 `host` 臂的重复运行
  ...
  —— **CPU 侧没有留下任何疑点**；
* **1 PRB 的样本噪声大**（各腿 56–146 次、64–100%）⇒ 要断言它的好坏需要更多样本，本批不做结论；
* `OCUDU_CE_CORR_DEV=0` 仍然**不是**精确 A/B（宿主的 A 没有对角加载 ⇒ 那条臂在窄跳上也解不出来）。
  要让它成为精确 A/B，得让宿主在设备路由上拿到真正的 sigma2（`OCUDU_CE_HOST_SCALARS=1` 就是），
  这属于"旋钮语义"的收尾，不是本批的判据。
