...
- **双 UE 三腿 A/B（2026-09-01 凌晨，SINR 可比 ✓）**：cpu / helena incumbent /
  ...
  **复测结果（2026-09-01 凌晨）**：v2 曲线导致双 UE 均无法接入（OAI UE 的
  64QAM 在 21–33 dB 全崩，239 KO/16 OK）；二分定位（OCUDU_HELENA_FORCE_NN=1
  = alpha 全 1）→ 双 UE 正常接入跑通。结论：**部分混合（0<α<1）有害**——
  NN 与 classical 的增益/相位校准不匹配，混合产物破坏高阶 QAM 解调。
  v2 已回退（commit `5b5a90a724`）。FORCE_NN 腿（full-NN）成绩：手机
  98.2%、OAI UE 77.1%（64QAM 88%、256QAM 24.4% vs cpu 腿 79%）。
...
- **real3n A/B（2026-08-31 深夜，可比 ✓）**：incumbent **65.0%**（SINR 中位
  17.4）vs real3n（--norm 0.15 尺度修正版）**58.7%**（SINR 中位 20.2）——
  **real3n 仍输 6.3 pp**。尺度错配假说被证伪（或至少不是全部原因）：匹配
  尺度训练的真实标签微调在手机上仍劣于合成 incumbent。结合首轮 A/B
  （归一化修复前 real +30 pp、修复后 incumbent 大涨），结论方向：**归一化
  修复消除了 incumbent 的主要缺陷后，DD 标签微调所携带的标签噪声偏差使其
  净贡献为负**。real4（更大标签集）预期同向。待 classical 同场参照腿确认
  incumbent 与 classical 的差距后，决定是否停止"真实标签微调"方向。
- **三腿同场 A/B（2026-09-01 凌晨，SINR 可比 ✓ 17.4/20.2/18.8）**：
  incumbent 65.0% / real3n 58.7% / **classical 69.5%**。classical 高出
  incumbent **4.5 pp**——存在残余差距。按预案做最后一次尝试：real4 =
  手机标签 −13 严格门（31,881→23,305）+ --norm 0.15 + lr 1e-5；
  若 real4 仍低于 incumbent 则停止真实微调方向，实机结论 = 合成
  incumbent + 归一化（与 classical 差 4.5 pp 内）。
- **real4 完成（2026-09-01 凌晨）**：23,305 个 −13 门标签 + --norm 0.15，
  运行时尺度下离线全面最佳——合成 −17.42 / 真实 holdout −17.85（同尺度
  init −15.32、real3 −15.52、real3n −15.64）、45 dB 探针 −24.1/−28.4/−31.2、
  ANE p50 141 µs。入库 `ai_assets/helena_pusch52_real4_ml.mlmodelc`，
  待实机最后一腿 A/B 裁决（对照已有 incumbent 65.0% / classical 69.5%）。
- **OAI UE 8 小时采集标签重建完成（2026-08-31 深夜）**：134,181 TB →
  **99,598 个过门标签**（rank{0:99596,1:2}）。质量剖面：metric p50 −11.8 dB
  （低 SNR 13–20 dB 的 DD 标签明显更噪；−13 门只保留 15,503）；宽度
  25(33.9k)/50(25.5k)/51(24.6k)/46(14.4k)。**real4 若做**：建议 --gate -13
  + --norm 0.15；但在 real3n 实机回退之后，"更多标签"的边际价值存疑——
  等 classical 同场参照腿的结论再定（详见上一节）。
