...
| 判据 | 阈值 | 现状 | 出处 |
|---|---|---|---|
| **V1** `[ul_gpu_pipeline]` 中位 | ≤2150 µs | ✅ **1371.9 µs**（`p38`，新最好；基线 2675.1 ⇒ **−48.7%**）| §6.46/§6.48–§6.60 |
| **V2** 池 | `starved_events=0` 且 `held_max<pool` | ✅ **`p38`（512 帧 + 池 32）：`starved_events=0`、`held_max=10 < 32`、`free_min=22`**（`p37` 13/19）| §6.37–§6.39、§6.60 ① |
| **V4** 提交数 | `cbs/lane ≤2.00`、`dropped=0` | ✅ `p33` 仍 **2.00 (max=2)**、crossings `0.00+0.00`/跳（第四次确认）| §6.30/§6.46/§6.49/§6.52 ⑤ |
| **V5** 不回归 | 契约 8/8、0 crossings | ✅ **`p38`：契约 MET 8/8、`gaps=0`、`rx_overflows=0`**（`p37` 同）；深环把丢样换成迟到（`stale`）在 `p38` 回落到 **1 次（8.3 ms）** | §6.53、§6.57 ③、§6.60 ① |
| **V3** 电台 | RF 失败 ≤10 | ⏸ **另案暂停（用户裁决）**：`p38` 1546（`p37` 880、`p35` 710）⇒ **电台天气逐腿不同，比较要成对**；靶子已定为**宿主竞争/线程优先级**（§6.56 ③）| §6.40–§6.43、§6.56 ③ |
...
### 3.2 ✅（**已验收：预登记全部命中**）：把 `h_starts` 搬到 y 上 —— run 3→1、派发 6→4
...
**`p38-n78-wholehop` 已飞：预登记全部命中**（§6.60）——`runs` **1.00/跳**、`max_run` **12**、`first_break` **none**、
`y_direct` **1.00**、派发 **4.00/跳**（均衡 **1.00**）、`merged_hop` **541.1（−14.1）**、**V1 1371.9 µs（−36.3，新最好，基线 −48.7%）**、
契约 **8/8**、`gaps=0`、池 `held_max=10/free_min=22`。
**⇒ 下一步（§6.60 ②）**：剩下的派发只有 **CE 2 + 解映射 1**（要先读它们自己的编码路径）；`merged_hop` 里**非派发的 ~490 µs**
要靠"移除阶段"臂或**单 kernel 离线微基准**（`dft_kernel_cost.mm` 的形状）；**A 项（≈473 µs）仍需你裁决**（触 V4）。
