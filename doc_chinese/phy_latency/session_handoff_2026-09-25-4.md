...
| 判据 | 阈值 | 现状 | 出处 |
|---|---|---|---|
| **V1** `[ul_gpu_pipeline]` 中位 | ≤2150 µs | ✅ **1411.3 µs**（`p32`，新最好；基线 2675.1 ⇒ **−47.2%**）| §6.31/§6.39/§6.46/§6.48/§6.49 |
| **V2** 池 | `starved_events=0` 且 `held_max<pool` | ✅ `p32` 仍 `starved_events=0`、`held_max=12 < pool=16`、`free_min=4`、`pop_blocking` max **27 µs**、`dropped=0` | §6.37–§6.39、§6.49 ④ |
| **V4** 提交数 | `cbs/lane ≤2.00`、`dropped=0` | ✅ `p32` 仍 **2.00 (max=2)**、crossings `0.00+0.00`/跳（**合并派发不动它**，第三次确认）| §6.30/§6.46/§6.49 ④ |
| **V5** 不回归 | 契约 8/8、0 crossings | ⚠ `p32` **7/8**：唯一失败 = `radio sample continuity: 1 gaps`（**电台侧偶发**，p24/p28/p29 同类；池全程健康）| §6.49 ⑤ |
| **V3** 电台 | RF 失败 ≤10 | ⏸ **另案暂停（用户裁决）**：`p32` 1013（`p31` 792）；`transmit()` 672 次 >1 ms（max 85 ms）| §6.40–§6.42、§6.43 ⑤、§6.49 ⑤ |

**当前状态**：`HEAD = 戳 = e7c5ddf71e`（+ 本节腿结果的文档提交），**二进制已携带该戳**，工作区干净，**没有腿在跑**。
**①后半（变体 A，去 `y_gather`）已完成、已离线取证、已空口验收**（§6.48/§6.49）：`p32` 机制逐项命中、**V1 −52.2 µs**。
**下一步 = §3.2**（可选先补一条对照腿 `p32b-n78-gather`）。
...
**`p32`（①后半验收 ⇒ 机制命中、`y_gather=0`、派发 6.0/跳、V1 1411.3、−52.2 µs）**。
...
| 45 | **V1 1463.5 → 1411.3 µs（−52.2，新最好）**；拆段后收益全在 **`eq_demap` −41.7**（`ce` −4.3、`t2f` −3.2，三段和 −49.2）| `p32` 与 `p31` 的配对相位段 | §6.49 ② |
| 46 | ★ **标尺要写口径**："≈12 µs/派发"成立在 **V1/跨度**上（本腿 4 次 ⇒ −52.2，≈13 µs/次），**不成立在设备占用窗口**上（`merged_hop` 只 −26.2，≈6.5 µs/次）⇒ **不要用 `merged_hop` 降幅反推派发数**；两种解释（小派发被遮住 / 直读步长变宽稍贵）本腿分不开 | `p32` | §6.49 ③、§8 Q16 |
...
### 3.1 ✅（已完成、已验收）：去掉 `y_gather` —— 变体 A

**已落地**（开发文档 **§6.48**，提交 `3b2b1b151d`）＋ **已空口验收**（开发文档 **§6.49**，腿 `p32-n78-directgrid`）：

| 读数 | 预登记 | `p31`（改前）| **`p32`（改后）** | |
|---|---|---|---|---|
| `sites(y_direct=)` / `(y_gather=)` / `(ch_gather=)` | ≈3.0 / 0 / 0 | — / 3.0 / 1.0 | **3.00 / 0 / 0** | ✅ |
| `burst dispatches`（均衡）| **6.0**（3.0）| 10.00（7.00）| **6.00（3.00）** | ✅ |
| `merged_hop` | ≈544 µs | 592.1 | **565.9**（只 −26.2，见 §6.49 ③）| ⚠ |
| **V1 中位** | ≈1415 µs | **1463.5** | **1411.3**（**−52.2**）| ✅ |
| 契约 / `cbs/lane` / gaps | 8/8 / 2.00 / 0 | 8/8 / 2.00 / 0 | **7/8** / **2.00** / 1 | ⚠ 唯一失败 = 电台 gap |

* **收益拆段**：`eq_demap` **−41.7 µs**、`ce` −4.3、`t2f` −3.2（三段和 −49.2 ≈ V1 的 −52.2）。
* ★ **标尺口径**：4 次派发 ⇒ V1 −52.2（**≈13 µs/派发，V1/跨度口径**），但 `merged_hop` 只 −26.2 ⇒
  **"12 µs/派发"不能用在设备占用窗口上**（§6.49 ③、§8 Q16）。
* **可选补一条对照腿** `p32b-n78-gather`（同配方 + `OCUDU_EQ_DIRECT_GRID=0`）把幅度做成对；
  机制方向已由计数器钉死，且本腿电台更差而 V1 仍降 ⇒ **收益偏保守**。
...
> 杠杆是**有口径的标尺**：**跳内每去掉 1 次派发 ⇒ V1 ≈ −13 µs**（⚠ **口径 = V1/跨度，不要用在设备占用窗口上**，§6.49 ③）。
> **①后半（`y_gather` 变体 A）已完成、已离线取证、已空口验收**（开发文档 **§6.48/§6.49**）：
> 离线 27 条语料 ×2 臂 = **135 个 dump 逐字节相同**；空口 `p32` `y_direct=3.00/跳`、`y_gather=0`、`ch_gather=0`、
> **派发 10.00 → 6.00/跳**、**V1 1463.5 → 1411.3（−52.2）**；V2/V4 保持、契约 7/8（唯一失败是**电台侧 1 gap**，见 §6.49 ⑤）。
> **下一步 = §3.2**：解开 `estimates` 断因让 run 覆盖整跳（3 → 1，再 ≈ −24 µs）——
> 主战场是 `ocudu_metal_mmse_engine.mm` 的**估计输出布局**；注意它与 ①后半的交互（见 §3.2 的 ⚠）。
> **可选先补一条对照腿** `p32b-n78-gather`（同配方 + `OCUDU_EQ_DIRECT_GRID=0`）把 −52 µs 做成对读数。
