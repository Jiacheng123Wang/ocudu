...
2. **Q9（~5 秒停顿）：仪器已就位，下一条腿就能判**（开发文档 §6.6 链条 + §6.7 仪器）。
   配方（写死）：**n1 默认工况 + `--expert_execution.threads.upper_phy.max_pusch_and_srs_concurrency=2`**，
   读三行：`[ul_rx_pool] pop_blocking wait (P0-2)`（max / `over 1s=`）、`[metal_stats] input hold (P0-2)`（max 与**最久持有的 slot**）、
   `[ul_gpu_lane] paired/phase account`。判读：`wait max ≈ 5 s` 且 hold 也是秒级 ⇒ 链条闭合；
   `wait` 只有 ms 级 ⇒ 5 s 不在池→收包链上（回电台/驱动侧）。
   ⚠ 这条腿同时恢复"腿跑的是哪个提交"的证据：**P0-2 是代码改动，`p05-pair` 已不再是 HEAD 的证据**。
...
6. ~~配对账输出 + "同 slot 覆盖"计数器~~ ✅ **已随 P0-2 一起做完**（开发文档 §6.7）：
   配对行新增 `overwritten by a later lane for the same slot=`，并有 `paired/phase account … -> EXACT MATCH/MISMATCH` 自解释行。
