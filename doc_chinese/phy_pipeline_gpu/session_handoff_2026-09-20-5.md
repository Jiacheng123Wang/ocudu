# 交接（入口） — S14：**离线 GPU 时间仪器定案 + `ch_wt` 空口分段 + K1 画像**——计数器永久关闭、等待点实测、`busy split` 有协议、**87.4% 的 `ch_wt` 已归属（K1 31.5% / 导频抽取 25.9% / 重排 18.6%）**、K1 的微调路线全部量掉

> **本文件是新会话的唯一入口**：读完它就能开工。上一份 `session_handoff_2026-09-20-4.md` 是上一会话的
> 过程记录（**不必读**）；技术细节都在常驻设计文档
> `gpu_phy_pipeline_design_and_implementation.md`（本会话新增 **§5.8.15**（仪器）与 **§5.8.16**（K1 画像），
> 并更正了 §5.8.14 的结尾与它开头那句"每一段都保值"）。过程证据与复跑命令：
> `wip/S14_instruments.md`；机制测量：`wip/metal_counter_caps.mm`；手机掉线的判读方法：`wip/S14_phone_drops.md`。
...
**K1 的两次改动都量输并已整块回退**（`ocudu_mmse_inv.metal` 与 HEAD 一致）。
...
| 候选 | 依据 | 第一步 |
|---|---|---|
| **① ★★ 下一个靶子：导频抽取（空口 `ch_wt` 的 25.9%，162.7 µs）** | §5.8.17：它**从来没被优化过**，而且一直被 **P2 的合并藏在同一条 GPU 跨度里**（`ch_est` 那项"按设计消失"）。三条独立证据同指一个数：`OCUDU_CE_CPU_LS=1` → **−162.7 µs**；`CPU_INVERT=1` 的分解闭合；两者相减 ≈161 | 先把它**逐 kernel 拆开**：`mmse_pilots_lse` / `cfo` / `sigma2` / `power` / `epre`——`lse` 是逐元素的（§19.6.3：可以加算术），`cfo`/`sigma2`/`power` 是**长累加/归约**（要当心，动它们要重跑 dump 网）。现成臂：`OCUDU_CE_CPU_LS=1`、`OCUDU_CE_DEV_SIGMA2=0`、`OCUDU_CE_DEV_TA=0`。**先量再动** |
| **② 第三块：重排 116.9 µs（18.6%），其中 TA 链 80.1** | §5.8.17；§18.7 那条"把 TA 移出关键路径"的实验现在有了价码 | 用 `REFORMAT_REPEAT=2` 与 `DEV_TA=0` 把它拆细（K3 / rSRP / 噪声方差 / TA 各占多少）|
| **③ ★ 裁定：K1 要不要为它改【数值契约】** | §5.8.16 ⑦ 已把"微调"这条路走死：几何最优（差 1.1%）、barrier 不是成本（两次否证）、算术量不是主导（n^1.27）、**并行度免费**（`nof_systems` 1→8 成本一动不动）⇒ 剩下的是 **Gauss-Jordan 逐 pivot 的串行临界路径**，一次 54×54 上 **≈3.7 µs/pivot、共 197 µs/跳**。在"逐元素次序不变"下它**砍不动** | **用户裁定**：换算法（如 Newton–Schulz：高度并行、barrier 极少，但同一 A 的**舍入不同**）⇒ 判据从"逐字节不变"变成"重定基线 + 空口腿按 CRC/契约判"。**先估收益再决定** |
| **④ 修 `OCUDU_CE_INVERT_FIRST`** | 它现在产出 **NaN**（§4.2）；修好后 K1 能单独占一条命令缓冲，比 `INV_REPEAT=3` 更干净（后者只对奇数有效，且 `_ce.txt` 差 5–7 字节）| 把 `engine_run()` 里那次 `engine->invert(gpu_a, …)` 移到**相关矩阵前缀之后**；判据：四个 dump 与默认臂逐字节相同，且 `busy split` 出现独立的求逆项 |
| **⑤ DFT（空中 ~40%）** | §5.8.12 的**空中**分段说 DFT ~40%，而它**从来没被离线量过** | 现在有协议与隔离台架两套工具了，先量 DFT 的分段再谈优化；它属 `ocudu_dft_metal_engine`（多模式共用）⇒ 改动面更大 |

**⚠ 无论走哪条：先读 §4（四个失败/禁用项），别重复它们。**
**⚠ 离线量这套东西用 `corpus/syn027_25`（25 PRB = 空口几何），不要用 `syn004_4`**（§5.8.16 ① 的口径修正）。
**⚠ K1 的微调不要再试了**：几何、barrier、BLK、并行度四条都已被量掉（§5.8.16 ②③⑥）。
**⚠ 报一个旋钮臂的数之前，先在代码里确认那个旋钮名字存在**（§5.8.17 ④：`OCUDU_CE_DEV_INVERT` 根本不存在，拼错的旋钮是静默空操作，读数看起来像"这段不花钱"）。
