...
1. **回退在 `mode=gpu` 里是缺陷，不是健壮性。** §5.6.1 的 18 个 `mmse_refusal` 理由与 §5.6.2 的站点表
   ...
   * **待做**（本裁定只定"应当报错"，不定"多响"）：把"可见"升级为"报错"。**响度是独立决定**
     （abort / 该 PUSCH 失败 + 显式错误 / 其它），见 §5.4。
...
- **回退的"响度"**（§1.8 第 1 条的待做项）：`mode=gpu` 下 refusal 触发即报错。**响度待定**
  （abort / 该 PUSCH 失败 + 显式错误），因为它改变运行期行为，属于要单独定的决定。
