...
- **real3 实机 A/B 回退（2026-08-31 晚，背靠背）**：A 腿 incumbent **81.1%**
  ...
  **real3 实机回退 13 pp，不晋升**。两点观察：① incumbent 相对首轮 A/B 的
  ...
  （离线 norm 评估 −18.84→−15.52）。教训：**训练/评估必须镜像运行时的输入
  归一化**（train_pad/eval_pad 已加 --norm）。解法：real3n（s56 + --norm
  0.15）→ real4（全量 134k OAI UE 标签 + --norm 0.15）。DD 标签噪声底
  （p50 −15.7 dB）已无法离线分辨模型优劣，实机 A/B 是唯一裁决。
