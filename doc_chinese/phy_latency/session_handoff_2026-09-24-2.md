...
1. **等用户裁决 P2-E**（开发文档 §6.4.4 的选项表）：建议保留开关（默认关）作仪器，然后二选一——
   **接受 +1 提交**把前端块做成独立提交（V4 需重裁），或转 **P2-D**——后者的输入**已经齐了**
   （P0-5 配对给出 n1 的真实持有期分母：`residency` 835.5 / `busy` 515.9，配对样本 73529）。
2. **查那次 ~5 秒收包停顿**（开发文档 §8 **Q9**，本线最大未决项）：下一个嫌疑是"同一 strand 上的邻居任务 /
   池与解码线程的互动"，或与并发度无关的第三个因。**`p05-pair` 的新线索**：~101 ms 的 `[ul_rx_wait]` 最大值
   在**四条** n1 腿上都存在（101591/101670/101319/101557 µs），只有这条腿把它变成 `gaps=7`/`stale=933`
   ⇒ 它是**周期性固定项**，落不落在跳上决定显不显形。
