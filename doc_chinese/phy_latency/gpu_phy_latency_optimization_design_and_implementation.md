...
| **Q20**（会话 #6 新增）| 均衡 / 解映射的**算力**是多少（`merged_hop` 里除了 CE 38 µs 与 A 项 464 µs 的其余部分）？ | **开放**：算力账单只补了 CE 的 6 个 kernel（§6.65②）；两 kernel 在各自 metallib（`ocudu_equalizer.metal` 的 `equalize_params`、`ocudu_demod.metal`），**补齐后**才能判 §6.65③ 里"均衡那条 741 µs 相位段"有多少是算力 |
