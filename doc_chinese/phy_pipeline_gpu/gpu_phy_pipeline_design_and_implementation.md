...
| **P1** | 消掉**尾组**那次提交（②）| `EDGE_FUSE` 那条路线转正（尾组的 corr build 编进标准组同一个缓冲）| 27 语料 + 20 窄捕获 **dump 逐字节不变**；每次接收提交数 4.00 → **3.00** | `cbs/lane` **3.73 → ≈3.00**；契约 8/8、0 穿越、RF failure 0、CRC 不劣化 | 旋钮默认关 ⇒ 一行回退 |
...
| **P3** | 前端与后端并成**一次**提交（①+③+④ → 1）| 估计器/权重与 eq_demap 编进**同一个**命令缓冲，开头提交一次 | 逐字节不变；提交数 2.00 → **1.00** | `cbs/lane` **1.00（max=1）**；上述四条 **+ `[ul_gpu_pipeline]` mean/median 与基线（3493.5/2799.6 µs）的差 ≤ 约定预算**（这一步理论上要付"估计器等整条 lane 编码完"的延迟）| 旋钮切回两段式 |
