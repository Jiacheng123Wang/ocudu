...
标签重建链**自洽但未与真实发射流对齐**：恢复的硬判定位流 vs 真实 TB 的吻合率
~0.59（随机 0.50）。已排除：配对（窗口内唯一候选）、E/调制（穷举无峰）、
RE 布局（实测修正）、CFO（每符号去旋转）、uint8 溢出、校验位窗口、Zc/CRC
开销。已定位的 bug 均修复后仍差最后一块——**某一处规范约定偏差**（交织器
空位/置换表/系统位窗口/映射顺序之一）。已交子代理对拍 OAI UE 发送链源码
（153 上）+ srsRAN 解速率匹配器，验收标准：三对样本吻合率 ≥0.90。
