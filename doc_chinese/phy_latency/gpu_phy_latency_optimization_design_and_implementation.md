...
### 6.61 **①开工（只读码 + 补仪器）：信道估计跳内其实是 6–9 次 kernel 派发，而车道只数到 2** —— 新的头号派发来源；尾巴（tail）组是最大的一刀
...
#### ② 规律：**分配宽度不是估计块（3 PRB）的整数倍 ⇒ 尾巴组再来一遍**

3 = 1 块 + 0；4 = 1 + **1**；6 = 2 + 0；8 = 2 + **2**；12 = 4 + 0；18 = 6 + 0；25 = 8 + **1** —— **完全吻合**。
⇒ **有多余尾巴的跳，`corr_a + corr_rhp + scatter` 各多跑一次 = +3 次派发**。
空口腿的授权宽度分布（`p31` 日志）正是混合的：`[0,51)`（51=17 块，无尾）、`[3,51)`（48=16 块，无尾）、
`[26,51)`（25，**有尾**）、`[8,51)`（43，**有尾**）、`[5,51)`（46，**有尾**）……
与腿上 `device_corr_builds=198724 / 145038 跳 = 1.37`（≈ 40% 的跳带尾巴）一致。

⇒ **真实派发账（每跳）**：CE **6（无尾）或 9（有尾）** + 均衡 1 + 解映射 1 = **8 或 11 次**，
而车道计数只写 4.00 —— **"还剩 4 次派发"这个说法要更正：里面的 CE 那一项其实是 6–9 次。**

#### ③ 候选杠杆（按"省下的派发 × 覆盖比例 ÷ 改动代价"排序）

| # | 杠杆 | 省 | 覆盖 | 代价 / 依据 |
|---|---|---|---|---|
| **A** | **把尾巴组并进主组**（标准块与尾巴块一次派发；`corr_stage`/`pilots_scatter` 结构里**已经有** `a_sys_stride`/`r_sys_stride`/`nof_systems` 这套"合并批次"字段，注释明说 *"a MERGED batch puts a narrower edge block into the standard slots"*）| **−3** | 带尾的跳（≈40%）⇒ 均值 **≈−1.2/跳** | **先验证"kernel 是否已支持合并批次"**；若支持 ⇒ **纯宿主改动**（像 §6.45 一样），若不然则要动 kernel |
| B | **R_hp 按几何缓存**（R_hp 由参考导频/协方差模型决定，**与数据无关**；§6.45 对 gather 表做过同一件事）| −1 | 分配重复的跳（空口约 40% 是 `[0,51)`）| 纯宿主 + 缓存键；需先确认 R_hp 的输入确实不含数据 |
| C | **`reformat`(K3) / `scatter` 用"改寻址"消掉**（§3.2 对 y 的同一招：纯搬运的 kernel 可以让下游直接按它的布局寻址）| −1 各 | 全部跳 | kernel + 宿主；每个都要逐字节不变量 |
...
**⏳ 待飞的腿**：**V1 的确认腿**（§6.31 ⑥：同配方 + 同流量窗口，最好同日交替 `batch=14` / `batch=1` 各一条）；
以及 **V2/V3 仍未达**（`starved_events=97`、RF 失败 1241，§6.31 ⑤）与 **D1/D2 的静默尾巴**（清扫缺入口点，§6.27 ④）。
