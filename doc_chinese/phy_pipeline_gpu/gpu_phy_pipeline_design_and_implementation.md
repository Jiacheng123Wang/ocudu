...
* **代价：延迟。** 中位端到端（IQ→LLR）**2016.5 → 3666.2 µs（+1650 µs，+82%）**，
  ...
  合并腿的 **3666.2 µs 比它还差**。
* **机制上为什么必然如此**：一次提交意味着"**整条 lane 编码完才 commit**"⇒ 这一跳的 GPU 工作
  （提取+权重+均衡+解映射）**无法与宿主编码均衡/解映射重叠**（`burst` 序历史上记的就是这个 ~125 µs 的
  latency debt，而在有负载的腿上它会随宿主的编码时间一起变大）。两条腿的 lane GPU 窗口
  （residency 704.5 vs 556.4 µs）只差 ~150 µs，而端到端差 1650 µs ⇒ **差额几乎全在"工作什么时候能开始"**。
* **⚠ 口径要说清楚**：两条腿的**链路/负载不同**（合并腿 9.06 slot/跳、两段腿 4.97）⇒ 绝对延迟不能逐点对读；
  但**最小延迟几乎相同**（1355.7 vs 1357.1 µs），**中位差 1.65 ms** 且**方向与机制一致**（gap 归零、
  一次提交），所以"P3 把宿主编码时间放上关键路径"这个结论是可信的。
