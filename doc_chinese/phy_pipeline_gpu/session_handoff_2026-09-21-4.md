...
| **10** | **★ 下一件事：上腿对**（`gpu` 对照 / `gpu` + 旋钮），判 PUCCH/PUSCH 的 `sinr` 中位数与五个计数 | 腿对 | ⬜ **下一步** |
...
**五条腿揭出五个缺陷，现在五个都修掉了**（接收池泄漏；PUCCH 是宿主消费者的结构性约束；锁内释放最后一个引用的自死锁；
...
**下一步就是再上腿对**——判 PUCCH/PUSCH 的 `sinr` 中位数（对照 +24/+26 dB）与五个计数。
