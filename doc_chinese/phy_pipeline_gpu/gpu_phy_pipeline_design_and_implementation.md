...
#### 5.8.5 ★ P0 结果（2026-09-21）：两件事，一件**挡住 P1**，一件**要你决策**
...
**P0-① 结果（2026-09-21）：判据做出来了，而且它直接指出了病灶——A 槽位的 pad 被写成 NaN。**
...
**P0-② 结果（2026-09-21）：病灶收窄到"求逆读到的边组 pad 是空的"，但**还没修**（本轮只做仪表）。**
...
**P0-③ 结果（2026-09-21）：按"设备自己拥有 pad"改了一版，**没修好，已回退**——而且这次尝试推翻了我上一条推论。**
...
**⚠ 那是假阳性。** 换一个参照（**基线 dump**，即改动前的默认路线）再比：**三个臂（默认 / TAIL_DEV / EDGE_FUSE）全都不同**，
15 个有余数的捕获 × 3 个 dump = **45/135**，症状都是**边组 `_h` 为 NaN**。
⇒ 我的改动**把原本正常的路线弄坏了**，同时让融合臂"与它们一致"。
...
| 32 | **★ 改过被广泛包含的头之后，只用 `--target` 增量构建** | 给估计器结果接口加了两个虚函数之后，只重建 `ul_chain_replay`：27 个捕获的 **`_llr.bin` 全部不同**，看起来像一次真实回归（连 `_h.bin` 都相同，只有 LLR 变）——**全量 `cmake --build build` 之后 0/135 差异**。陈旧对象会制造"有代码改动"的假象 | **动过头文件（尤其是被多个库包含的）之后做一次全量构建**，再判读任何差异；看到"只有一个产物变"时先怀疑构建，不要先怀疑代码 |
