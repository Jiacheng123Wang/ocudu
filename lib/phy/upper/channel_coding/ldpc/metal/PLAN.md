...
- **E2E 第一轮结果(2026-08-24 21:39,修复 ABI 后重跑,e976e6428f)**:
  ...
  - **E2E 工作点量化(2026-08-24,BLER 基准,100 块/点)**:E2E 失败 TB 为
    BG2 z=48(tbs=437)/z=56(tbs=512,528)、QPSK、R≈0.04-0.15(带重复);基准
    可复现的最低码率 R0.2 处:
    | BG2 | 码率 | LLS 冠军 | NMS |
    |---|---|---|---|
    | Z48 | 0.2 | 0/1/23/77/96/100 @0-10dB | **100%@0dB** |
    | Z48 | 0.3333 | 0/0/10/63/96/100 | **100%@0dB** |
    | Z48 | 0.5 | 0/0/0/48/95/100 | **100%@0dB** |
    | Z56 | 0.2/0.3333 | 15/77/96 @4/6/8dB | — |
    **LLS 在 E2E 工作点需要 8-10dB,而 NMS ≤0dB——差距 8-10dB**,与 E2E
    的 1.5-3.4dB 有效 sinr 对照,LLS 通过率 ≈ 0-10%。Phase 2/3 预期总收益
    2-4dB 无法弥合:**metal_lls 的 E2E attach 必须依赖 UL 信号质量修复**;
    若不修 UL,E2E attach 应使用 NMS(metal,已证 191/196 + 50 ping)。
- **Phase 2 结论(2026-08-24,全组合测量,已合并 wip-lls-phase2)**:
  - 多嫌疑人 k=3/4:无收益,甚至崩盘风险(E-peel/E-uniform 高 SNR 下给正确
    VN 投大额证据 → 全盘侵蚀;E-self-general 保守化后 A/B 中性、D 不动,
    延迟 2-3 倍)——**证伪**;
  - C4 停滞逃逸(stall_escape,θ∈{0.5,0.75,1.0}、rounds∈{2,3}):D 卡 74、
    E1 卡 77 全无变化——D 的失败是**振荡型**(syndrome 每轮变化,非停滞),
    stall 检测不触发——**证伪**(机制保留为可配置旋钮);
  - cooldown 振荡护栏(k=2):A 58/90/97(vs 52/87/97)、E2@6dB 52(vs 48)、
    B@8dB 97(vs 95),但 B@6dB 45(vs 49)、C 轮数 4.2→5.9(+40% 延迟)——
    收益小而混杂,**不作默认,保留旋钮**;
  - α>1.5:C 的 floor 回归(190-197/200)——α 上限 1.5 确认。
  - **Phase 2 净结论:侵蚀比特翻转族在 k=2 下已到实用天花板**;工厂默认
    维持 Phase 1 冠军(γ0/α1.5/k2/cooldown 0),新旋钮全部可测但不开。
...
### 4.18.1 实链复跑验证(2026-08-30):修复有效,余下尾部为既有内核代价

复跑(211 样本):`[ul_ldpc_decode]` max 48995 → **16845 µs**、p95 6251、p99 9305、
median 753(前次 979)。

**修复验证(全部通过)**:
- 启动期预热生效:`engine slot bg=2 z=2 ready in 32882.7us` 发生在 01:24:47(启动期),
  首个 PUSCH 在 01:25:07——首解码不再付 33ms JIT 税(49ms 峰值消失);
- 共享缓存正确:18 个唯一 (bg,z) 各构建一次(无重复构建行),`cache=1..18` 递增;
- 解码对拍 disagreements 0(修复未改任何解码行为)。

**剩余尾部构成(均为修复前已存在,非新 bug)**:
1. **大 z 内核解码代价**:本地复现 BG2 z=352 99dB 1 轮——persistent GPU 单次解码
   **3224µs**、layered **1248µs**(gpu_wait 3038/1061µs)。实链 16.8ms 样本 =
   z=352 首用槽构造 ~7ms + metal_t=5937.8µs(iter=2,persistent 大 z 内核 2 轮)。
   根因:persistent 单 threadgroup(1024 线程=32 simdgroup)每层 z/32=11 行**串行**
   处理(threadgroup_barrier 限死了多 TG 方案);layered 则每行一个 TG 并行但吃
   dispatch 链。小 TB 场景(实链主体)两者都 ~200-500µs,大 TB(528B)是代价区。
2. **18 个 (bg,z) 首用构建**:矩阵构建 0.01-6.8ms(z=352)+ 每实例引擎初始化/预热
   ~0.3-2ms——现在**每进程一次**(修复前每实例一次),且被 3× 管线过载放大。
3. **管线过载放大**:pipeline mean 2927µs > 1ms 预算,任务排队使一次性成本翻倍。

**下一步候选(待用户决策)**:
- (a) 大 z 时自适应选 layered(1.06ms < persistent 3.2ms),或做多 TG + 设备栅栏的
   persistent 变体(改动大、需逐位对拍);
