...
void radio_zmq_tx_channel::send_response()
{
  ...
  if (count == 0) {
#if defined(OCUDU_FLOW_PROBES)
    // [zmq-probe] instrumentation (compiled only with ENABLE_FLOW_PROBES): remember when the buffer became empty.
    if (!buffer_was_empty) {
...
#if defined(OCUDU_FLOW_PROBES)
  // [zmq-probe] instrumentation (compiled only with ENABLE_FLOW_PROBES): report how long the pending request waited for data.
  if (buffer_was_empty) {
    auto empty_wait_us =
...
#if defined(OCUDU_FLOW_PROBES)
  // [zmq-probe] instrumentation (compiled only with ENABLE_FLOW_PROBES): how long does the blocking zmq_send take?
...
#if defined(OCUDU_FLOW_PROBES)
  // [zmq-probe] instrumentation (compiled only with ENABLE_FLOW_PROBES): zmq_send blocking time (anomalies) and a per-64-replies round-rate
  // summary (replies/s == lockstep rounds/s).
  {
    auto send_us =
        std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - t_before_send).count();
    if (send_us > 2000) {
      logger.info("[zmq-probe] tx={} zmq-send-block={}us bytes={}", channel_id_str, send_us, nbytes);
    }
    static unsigned probe_reply_count   = 0;
    static int64_t  probe_send_total_us = 0;
    static auto     probe_last          = std::chrono::steady_clock::now();
    probe_reply_count++;
    probe_send_total_us += send_us;
    if ((probe_reply_count & 63) == 0) {
      auto now        = std::chrono::steady_clock::now();
      auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(now - probe_last).count();
      logger.info("[zmq-probe] tx={} reply-rate: 64 replies in {}ms = {:.1f} replies/s, mean zmq-send={:.1f}us",
                  channel_id_str,
                  elapsed_ms,
                  elapsed_ms > 0 ? 64000.0 / elapsed_ms : 0.0,
                  probe_send_total_us / 64.0);
      probe_last          = now;
      probe_send_total_us = 0;
    }
  }
#endif
...
void radio_zmq_tx_channel::transmit_samples(span<const cf_t> data)
{
  ...
  while (state_fsm.is_running() && (count != data.size())) {
    ...
    if (pushed == 0) {
#if defined(OCUDU_FLOW_PROBES)
      // [zmq-probe] instrumentation (compiled only with ENABLE_FLOW_PROBES): remember when the buffer became full.
      if (!buffer_was_full) {
      ...
      // [zmq-probe] instrumentation (compiled only with ENABLE_FLOW_PROBES): report how long the producer was blocked on the full buffer.
      auto full_wait_us =
