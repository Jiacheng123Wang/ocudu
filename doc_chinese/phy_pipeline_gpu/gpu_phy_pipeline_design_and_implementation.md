...
| **S13-P2c** | 窄跳（分配 < 1 个块）的相关矩阵：宿主建的 A **没有对角加载** ⇒ 2 PRB 恒 0% CRC | ✅ **完成并空中验证**（`b1ab17c937`，腿 `narrow-fix_0920_0508`，§5.6.7/§5.6.8）：**2 PRB `0.0% → 77.5%`**、SINR 中位数 **−10.7 → +13.0 dB**；契约 **8/8**、两张分项表都空、`refusals=<none>`、**RF failure 0**；27 捕获逐字节不变；机制由 `HOST_SCALARS=1` 逐值相同证明 |
...
|---|---|---|
| 宿主建 A（现状）| **KO × 16**，SINR **−8…−12 dB**，10 次迭代 | OK，SINR ≈ +15.6 dB |
| **设备建 A（修法）** | **OK × 14**（多数 **1 次迭代**），SINR **+10…+22 dB**；另 2 条 KO 但 SINR +13.7/+17.7 dB | OK，SINR ≈ **+33 dB** |
| CPU 参考链 | KO × 16，与宿主建 A **完全相同** | OK，+15.6 dB |

**★ 机制（用 `OCUDU_CE_HOST_SCALARS=1` 证明，不是推测）**：在**未修**的二进制上打开
`OCUDU_CE_HOST_SCALARS=1`（宿主于是读到设备的 sigma2/功率标量，`stats.sigma2` 变成真正的比值），
结果与"设备建 A"**逐值相同**（`cap_8611`: 两边都是 `crc=OK it=1 sinr=16.46`）。
...
**⚠ 一条未解释的旁证（诚实清单）**：**CPU 参考链也在这 16 条上全 KO**（与"宿主建 A"逐值相同）。
按机制它本不该如此（CPU 估计器自己算 `estimate_noise()`），所以要么 CPU 链在窄跳上另有问题，
要么这次比较里 CPU 与 Metal 的输入并不等价。**这一条不影响修法本身**（修法的效果是"能解出来了"，
14/16 条 1 次迭代即收敛），但**它是一个独立的问题**，记在这里不当作已解释。
...
* **CPU 参考链在窄跳上仍然全 KO**（§5.6.7 的旁证）：本批只修了 Metal 设备路由。
  `--cpu` 整链在同样的 16 条捕获上是 −9…−12 dB、全 KO —— **一个独立的问题**，未查；
